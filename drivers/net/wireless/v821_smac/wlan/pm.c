...
extern int suspend_dtim;
extern int listen_interval;
#else
int suspend_dtim = 3;
int listen_interval;
#endif
			       ...
			       struct cfg80211_wowlan *wowlan)
{
	...
	if (hw_priv->fw_txrxbuf_switch_buf_status == FW_TXRXBUF_SWITCH_BUF_ST_HOST_SWING) {
		pm_printk(XRADIO_DBG_WARN, "Don't suspend "
			   "because of epta buffer is switching.\n");
		return -EBUSY;
	}
#endif
				...
				struct cfg80211_wowlan *wowlan)
{
	...
	if (priv->join_status == XRADIO_JOIN_STATUS_STA &&
		...
		wsm_set_beacon_wakeup_period(hw_priv,
			priv->join_dtim_period * suspend_dtim,
			listen_interval, priv->if_id);
