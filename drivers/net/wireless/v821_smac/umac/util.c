...
void mac80211_iterate_interfaces(
...
void mac80211_iterate_active_interfaces_rtnl(
	struct ieee80211_hw *hw, u32 iter_flags,
	void (*iterator)(void *data, u8 *mac,
			 struct ieee80211_vif *vif),
	void *data)
					  ...
					  u32 flags)
{
	...
	skb = mac80211_probereq_get(&local->hw, src, ssid, ssid_len,
				     100 + ie_len);
	if (!skb)
		return NULL;
...
static int ieee80211_reconfig_nan(struct ieee80211_sub_if_data *sdata)
{
	...
	for (i = 0; funcs[i]; i++) {
		...
		if (WARN_ON(res))
			mac80211_nan_func_terminated(&sdata->vif,
						      funcs[i]->instance_id,
			    ...
			    struct ieee80211_noa_data *data, u32 tsf)
{
	...
	if (ret)
		mac80211_update_p2p_noa(data, tsf);
