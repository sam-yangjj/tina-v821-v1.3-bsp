...
int xradio_start(struct ieee80211_hw *dev)
{
	...
	if (wait_event_interruptible_timeout(hw_priv->wsm_wakeup_done,
	    atomic_read(&hw_priv->suspend_state) == XRADIO_RESUME, 3 * HZ) <= 0) {
		sta_printk(XRADIO_DBG_ERROR, "%s:driver is suspending\n", __func__);
		return -ETIMEDOUT;
	}
#endif /*CONFIG_PM*/
			     ...
			     struct ieee80211_vif *vif)
{
	...
	if (priv->if_id == 1 &&
	    (priv->mode == NL80211_IFTYPE_AP || priv->mode == NL80211_IFTYPE_P2P_GO)) {
			     ...
			     u64 multicast)
{
	...
	*total_flags &= FIF_PROMISC_IN_BSS |
	...
	priv->rx_filter.bssid = (*total_flags & (FIF_OTHER_BSS | FIF_PROBE_REQ)) ? 1 : 0;
	priv->rx_filter.fcs = (*total_flags & FIF_FCSFAIL) ? 1 : 0;
	priv->bf_control.bcn_count =
		(*total_flags & (FIF_BCN_PRBRESP_PROMISC |
		FIF_PROMISC_IN_BSS | FIF_PROBE_REQ)) ? 1 : 0;
...
int xradio_set_pm(struct xradio_vif *priv, const struct wsm_set_pm *arg)
{
	...
	if (memcmp(&pm, &priv->firmware_ps_mode, sizeof(struct wsm_set_pm))) {
		...
		sta_printk(XRADIO_DBG_NIY, "%s,mode=0x%x, Idle=%d, Change=%d\n",
			   __func__, pm.pmMode, pm.fastPsmIdlePeriod, pm.apPsmChangePeriod);
...
int __xradio_flush(struct xradio_common *hw_priv, bool drop, int if_id)
{
	...
	for (;;) {
		...
		if (unlikely(!xradio_queue_stats_is_empty(&hw_priv->tx_queue_stats, -1, if_id))) {
			     ...
			     struct ieee80211_channel *chan)
{
	...
	struct wsm_start start = {
		...
		.band = (chan->band == NL80211_BAND_5GHZ) ? WSM_PHY_BAND_5G : WSM_PHY_BAND_2_4G,
		...
		case WSM_EVENT_INACTIVITY:
			{
				...
				memcpy(deauth->sa, entry->mac/*priv->link_id_db[i].mac*/,
				       ETH_ALEN);
...
void xradio_join_work(struct work_struct *work)
{
	...
	down(&hw_priv->conf_lock);
	{
		...
		SYS_WARN(wsm_set_operational_mode(hw_priv, &mode, priv->if_id));
		SYS_WARN(wsm_set_block_ack_policy(hw_priv, 0, hw_priv->ba_tid_mask, priv->if_id));
...
void xradio_ba_timer(struct timer_list *t)
{
	...
	if (hw_priv->ba_cnt >= XRADIO_BLOCK_ACK_CNT &&
	    ...
	    hw_priv->ba_acc_rx / hw_priv->ba_cnt_rx >= XRADIO_BLOCK_ACK_THLD)))
		ba_ena = true;
	else
		ba_ena = false;
...
#endif
			WSM_EDCA_SET(&priv->edca, 2, 0x0002, 0x0003, 0x0007, 0, 0xc8, false);
			     ...
			     struct xradio_vif *priv, u8 *data)
{
	...
	for (i = 0; i < no_of_mac_addr; i++) {
		mac_addr_filter->macaddrfilter[i].address_mode = addr_info[i].address_mode;
		memcpy(mac_addr_filter->macaddrfilter[i].MacAddr, addr_info[i].MacAddr, ETH_ALEN);
		mac_addr_filter->macaddrfilter[i].filter_mode =  addr_info[i].filter_mode;
	}
		      ...
		      struct sockaddr *sa)
{
	...
	/*Change mac in fw*/
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_CHANGE_MAC, sa->sa_data, ETH_ALEN, address_id);
	if (ret)
		return ret;
...
int xradio_set_arpreply(struct ieee80211_hw *hw, struct ieee80211_vif *vif)
{
	...
	memcpy(snaphdr, (struct ieee80211_snap_hdr *)rfc1042_header, sizeof(*snaphdr));
 ...
 * Returns: 0 on success or non zero value on failure
 */
static int xradio_set_snap_frame(struct ieee80211_hw *hw, u8 *data, int len)
{
 ...
 * Returns: 0 on success or non zero value on failure
 */
static int xradio_set_txqueue_params(struct ieee80211_hw *hw, u8 *data, int len)
{
 ...
 * Returns: 0 on success or non zero value on failure
 */
static int xradio_tesmode_reply(struct wiphy *wiphy, const void *data, int len)
{
	int ret = 0;
	struct sk_buff *skb = NULL;
	sta_printk(XRADIO_DBG_TRC, "%s\n", __func__);
 ...
 */
static int xradio_test(struct ieee80211_hw *hw, void *data, int len)
{
...
int xradio_get_tx_power_level(struct ieee80211_hw *hw)
{
	...
	sta_printk(XRADIO_DBG_MSG, "%s: Power set on Device : %d", __func__, get_power);
 ...
 * Returns: 0 on success or non zero value on failure
 */
static int xradio_set_advance_scan_elems(struct ieee80211_hw *hw, u8 *data, int len)
{
 ...
 * Returns: 0 on success or non zero value on failure
 */
static int xradio_set_power_save(struct ieee80211_hw *hw, u8 *data, int len)
{
...
int xradio_get_tsm_params(struct ieee80211_hw *hw)
{
	...
	spin_unlock_bh(&hw_priv->tsm_lock);
	return xradio_tesmode_reply(hw->wiphy, &tsm_stats, sizeof(hw_priv->tsm_stats));
}
...
int xradio_get_roam_delay(struct ieee80211_hw *hw)
{
	...
	sta_printk(XRADIO_DBG_MSG, "%s: Roam delay : %u", __func__, roam_delay);
...
int xradio_testmode_cmd(struct ieee80211_hw *hw, void *data, int len)
{
	...
	sta_printk(XRADIO_DBG_MSG,  "%s: type: %i", __func__, nla_get_u32(type_p));

	switch (nla_get_u32(type_p)) {
	case XR_MSG_TEST:
		ret = xradio_test(hw, nla_data(data_p), nla_len(data_p));
		break;
	case XR_MSG_SET_SNAP_FRAME:
		ret = xradio_set_snap_frame(hw, (u8 *)nla_data(data_p), nla_len(data_p));
		break;
	...
	case XR_MSG_SET_ADVANCE_SCAN_ELEMS:
		ret = xradio_set_advance_scan_elems(hw, (u8 *)nla_data(data_p), nla_len(data_p));
		break;
	case XR_MSG_SET_TX_QUEUE_PARAMS:
		ret = xradio_set_txqueue_params(hw, (u8 *)nla_data(data_p), nla_len(data_p));
		break;
	...
	case XR_MSG_SET_POWER_SAVE:
		ret = xradio_set_power_save(hw, (u8 *)nla_data(data_p), nla_len(data_p));
		break;
	default:
		break;
	}
