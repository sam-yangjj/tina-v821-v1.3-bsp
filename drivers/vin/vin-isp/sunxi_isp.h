...
struct isp_dev {
	...
	enum isp_input_seq infmt;
	unsigned char use_bk_one_buffer;
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
	struct vin_mm isp_stat1;
	struct vin_mm isp_save_load1;
	struct vin_mm load_para1[2];
	struct vin_mm d3d_pingpong1[4];

	enum isp_input_seq infmt1;
	char load_shadow1[ISP_LOAD_DRAM_SIZE];
	bool load_select1;
	char load_flag1;

	bool isp_handle_embed;
#endif
#endif
	...
	struct work_struct isp_save_ae_task;
