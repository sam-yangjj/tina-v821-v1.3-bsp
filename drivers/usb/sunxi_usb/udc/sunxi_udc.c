...
extern int sunxi_usb_ctrl(int id, u32 data);
/* config enable dma */
...
static ssize_t usb_speed_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sunxi_udc *udc = dev_get_drvdata(dev);
	return sprintf(buf, "%d\n", udc->gadget.speed);
}
static DEVICE_ATTR_RO(usb_speed);
		...
		struct sunxi_udc_request *req)
{
	...
	if (is_udc_support_dma() && is_dma_enable()) {
		if ((ep->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK) == USB_ENDPOINT_XFER_ISOC && \
			...
			/* When USB2.0 uses DMA in UVC to capture data link packets,
			 * the interval for transmitting 512 bytes and small packet data in FS mode is 1ms,
			 * while the interval in HS mode is 120us.
			 * If the small packet is shorter than this time,it will be lost. */
			if (USBC_Dev_QueryTransferMode(g_sunxi_udc_io.usb_bsp_hdle)
					== USBC_TS_MODE_HS) {
				udelay(120);
			} else if (USBC_Dev_QueryTransferMode(g_sunxi_udc_io.usb_bsp_hdle)
					== USBC_TS_MODE_FS) {
				mdelay(1);
			}
		}
...
#if IS_ENABLED(CONFIG_USB_G_WEBCAM) || IS_ENABLED(CONFIG_USB_CONFIGFS_F_UVC) || \
		IS_ENABLED(CONFIG_USB_SUNXI_G_WEBCAM_LEGACY) || IS_ENABLED(CONFIG_USB_SUNXI_CONFIGFS_F_UVC)
#else
		if (dev->gadget.speed != USB_SPEED_UNKNOWN) {
			spin_unlock_irqrestore(&dev->lock, flags);
...
#if IS_ENABLED(CONFIG_ARCH_SUN300IW1)
static void sunxi_udc_set_speed(struct usb_gadget *g, enum usb_device_speed speed)
{
	struct sunxi_udc *udc = the_controller;
	...
	DMSG_INFO_UDC("%s_%d: udc set max speed 0x%x speed 0x%x\n",
			__func__, __LINE__, udc->gadget.max_speed, speed);
}
#endif
...
#if IS_ENABLED(CONFIG_ARCH_SUN300IW1)
	.udc_set_speed          = sunxi_udc_set_speed,
#endif
};
...
#if IS_ENABLED(CONFIG_ARCH_SUN300IW1)
	device_create_file(&pdev->dev, &dev_attr_usb_speed);
#endif
...
#if IS_ENABLED(CONFIG_ARCH_SUN300IW1)
	device_remove_file(&pdev->dev, &dev_attr_usb_speed);
#endif
