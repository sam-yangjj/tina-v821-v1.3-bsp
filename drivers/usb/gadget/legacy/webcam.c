...
#define WEBCAMERA_COMPOSITE_ENABLE_UAC    (0)

#if WEBCAMERA_COMPOSITE_ENABLE_UAC
...
#if WEBCAMERA_COMPOSITE_ENABLE_UAC
/* --------------------------------------------------------------------------
 * module parameters specific to the UAC1
 */
...
static int uac1_do_config(struct usb_configuration *c)
{
	int status;
...
static int uac1_bind(struct usb_composite_dev *cdev)
{
	...
	if (IS_ERR(fi_uac1)) {
		printk("%s:%d usb get func uac1_func fail!\n", __FUNCTION__, __LINE__);
...
static int uac1_unbind(struct usb_composite_dev *cdev)
{
