...
extern bool wlan_reg_ready(void);
...
wakeup:
	...
	/*set rccali enable and check.*/
	if (wlan_reg_ready() == true) {
		xradio_dbg(XRADIO_DBG_ALWY, "wlan run at rccali_32k");
	} else {
		xradio_dbg(XRADIO_DBG_ALWY, "wlan run at rcosc_32k");
		ret = xradio_apb_write_32(hw_priv, RCCALI_CTRL_ADDR, RCCALI_DISABLE);
		if (ret < 0) {
			xradio_dbg(XRADIO_DBG_ERROR, "%s:APB write RCCALI_CTRL_REG error!\n", __func__);
			goto out;
		}
		ret = xradio_apb_read_32(hw_priv, RCCALI_CTRL_ADDR, &val32);
		if (ret < 0) {
			xradio_dbg(XRADIO_DBG_ERROR, "%s:APB read RCCALI_CTRL_REG error!\n", __func__);
			goto out;
		}
		if (val32 != RCCALI_DISABLE) {
			xradio_dbg(XRADIO_DBG_ERROR, "%s: unable to initialise " \
				"RCCALI_CTRL_REG wrote 0x%.8X, read 0x%.8X.\n",
				__func__, RCCALI_DISABLE, val32);
			ret = -EIO;
			goto out;
		}
	}
