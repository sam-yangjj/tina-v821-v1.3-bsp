...
/***************************************************************************************/
// forcing turn on/off PHY to restart monitor rx. V821's monitor bug
#ifdef CONFIG_DRIVER_V821
#define MONITOR_RX_FIX
#define MONITOR_RX_TMO      (1*HZ - 1)
#endif
/***************************************************************************************/
...
#define XRADIO_USER_CONFIG_SUPPORT     1
#define POWER_LEVEL_TAB_USE_LENGTH     11
#define XRADIO_USER_CHANNEL_FEC        1
#define XRADIO_USER_POWER_LEVEL_TAB    1
#define XRADIO_USER_AUTO_POWER         1
#define XRADIO_USER_AUTO_SCAN          0
...
#define XRADIO_SUPPORT_AIF_CONFIG     1 /* switch of enbale AIF module or not */
#define XRADIO_AIF_DEFAULT_SEL        1 /* AIF default vaule: 1 for enable, 0 for disable */
#ifdef CONFIG_DRIVER_V821
#define XRADIO_USE_FW_SOFT_AGC_CCA_POLICY    1 /* FW SOFT AGC default vaule: 1 for enable, 0 for disable */
#endif
...
struct xradio_common {
	...
	* after the mechanism is clear */
	...
	struct hifbypass_priv	*hifbypass_priv;
#endif
#ifdef CONFIG_DRIVER_V821
#if XRADIO_SDD_DIFF_HOSC_FREQ_FILE_MERGE
	u32 hosc_freq;
#endif
	u32 new_tx_conf_status;
	u8 sdd_freq_offset;
#ifdef MONITOR_RX_FIX
	struct work_struct monitor_rx_restart_work;
#endif
#endif
#if XRADIO_USER_POWER_LEVEL_TAB
	u16 sdd_power_level_tab[POWER_LEVEL_TAB_USE_LENGTH];
#endif
	struct semaphore	device_wkup_lock;
#ifdef CONFIG_FW_TXRXBUF_USE_COEXIST_RAM
	u32                 fw_txrxbuf_epta_sec_start;
	u32                 fw_txrxbuf_epta_sec_end;
#define FW_TXRXBUF_EPTA_SECTION_RANG_START  (0x09000000)
#define FW_TXRXBUF_EPTA_SECTION_RANG_END  (0x09000000 + 72 * 1024)
	struct delayed_work fw_txrxbuf_switch_buf_work;
	struct semaphore    fw_txrxbuf_switch_buf_wq;
	u32                 fw_txrxbuf_switch_buf_status;
#define FW_TXRXBUF_SWITCH_BUF_ST_HOST_SWING  (0xFF)
	atomic_t            fw_txrxbuf_switch_buf_lock_cmd_tx;
	u16                 fw_txrxbuf_switch_buf_work_try_cnt;
#define FW_TXRXBUF_SWITCH_BUF_TRY_MAX  (300)
	u8                  fw_txrxbuf_switch_buf_ps_active;
#endif
};

#ifdef XRADIO_USER_CONFIG_SUPPORT
struct xradio_user_config_info {
	u8 user_retry_cnt;     /* max retry count */

	u8 pm_dtim_period_extend; /* pm dtim param, 1 ~ 10, in beacon_period */
	u16 pm_tx_null_period; /* the null data frame txed period */

	u16 listen_interval;   /* listen interval in (re)association request */

	u8 ps_disable;         /* ps mode disable/enable */
	u8 ps_idleperiod;      /* ps ide period, in ms */
	u8 ps_changeperiod;    /* ps change period, in ms */

	u8 force_B_rate_enable;/* disable/enable B rate when rssi is lower than threshold */
	u8 force_2m_rssi;      /* B rate 2M threshold */
	u8 force_1m_rssi;      /* B rate 1M threshold */

#if XRADIO_USER_POWER_LEVEL_TAB
	u16 power_level_tab[POWER_LEVEL_TAB_USE_LENGTH];
#endif
	/*
	 * scan parameters
	 */
	u8 scan_num_probes;    /* number of probe requests (per SSID) sent to one channel */
	u8 scan_probe_delay;   /* delay time (in us) before sending a probe request */
	u16 scan_min_dwell;    /* min channel dwell time (in ms), for active scan only */
	u16 scan_max_dwell;    /* max channel dwell time (in ms), for active scan only */
#if XRADIO_USER_AUTO_SCAN
	u8 scan_auto_enable;
	u32 scan_auto_interval; /* auto scan mode scan interval, in second */
#endif

#if XRADIO_USER_AUTO_POWER
	u16 auto_pwr_step;
	u16 auto_pwr_rssi_thres;
	u16 auto_pwr_max_rssi;
	u16 auto_pwr_min_rssi;
	u16 old_rssi;
	u16 auto_pwr_rate_sel;
#endif
};
#endif  //#ifdef XRADIO_USER_CONFIG_SUPPORT
...
struct xradio_vif {
	...
	*/
	...
	bool	is_mfp_connect;
#ifdef XRADIO_USER_CONFIG_SUPPORT
	struct xradio_user_config_info user_config;
#endif
};
...
extern struct timeval TES_P2P_0002_roc_end_tmval;
...
static inline bool is_hardware_xradio(struct xradio_common *hw_priv)
{
	return (hw_priv->hw_revision == XR819_HW_REV0 || hw_priv->hw_revision == XR819_HW_REV1 ||
			hw_priv->hw_revision == V821_HW_REV0 || hw_priv->hw_revision == V821B_HW_REV0);
}
