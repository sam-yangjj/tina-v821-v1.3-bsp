...
#ifdef XRADIO_RAW_FRM_SUP
	.ndo_do_ioctl		= xrmac_ndo_do_ioctl,
#endif
};
...
#ifdef XRADIO_RAW_FRM_SUP
	sema_init(&sdata->tx_raw_lock, 1);
	init_waitqueue_head(&sdata->tx_raw_wq);
	sdata->raw_state = SDATA_RAW_TX_STATE_TXED;
	sdata->raw_tx_mode = SDATA_RAW_TX_MODE_NORMAL;
	sdata->raw_tx_timeout = HZ / 10;
#endif
