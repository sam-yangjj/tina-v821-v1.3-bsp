...
static int isp_3d_pingpong_alloc(struct isp_dev *isp)
{
	...
	__maybe_unused struct vin_md *vind = dev_get_drvdata(isp->subdev.v4l2_dev->dev);
	...
	if (isp->use_bk_one_buffer == 2) {
		for (i = 0; i < VIN_MAX_DEV; i++) {
			if (vind->vinc[i] == NULL)
				continue;

			if ((vind->vinc[i]->isp_sel) == isp->id && (vind->vinc[i]->id / 4 == 0)) {
				vinc = vind->vinc[i];
		...
		if (vinc) {
			...
			if (vinc->vid_cap.special_active == 1) {
				if (vinc->dma_merge_mode && isp->id % 2)
					isp->d3d_pingpong[0].dma_addr = buf->paddr + (isp->d3d_lbc.line_stride << 2) - hgt;
				else
					isp->d3d_pingpong[0].dma_addr = buf->paddr - hgt;
			} else
				vin_err("use_one_buffer 2 only support special interface\n");
			isp->d3d_pingpong[0].vir_addr = NULL;
			isp->d3d_pingpong[0].size = 0;

			isp->d3d_lbc.line_stride *= 2;
		}
	} else {
		ret = os_mem_alloc(&isp->pdev->dev, &isp->d3d_pingpong[0]);
		if (ret < 0) {
	...
	if (isp->id == SOFTWARE_TDM_ID) {
		isp->d3d_pingpong1[0].size = hgt + isp->mf.height * (isp->d3d_lbc.line_stride << 2); /* d3d_byr_addr */
		isp->d3d_pingpong1[1].size = kb_wnum * kb_hnum; /* d3d_k0_addr */
		isp->d3d_pingpong1[2].size = isp->d3d_pingpong1[1].size; 	/* d3d_k1_addr */
		isp->d3d_pingpong1[3].size = isp->d3d_pingpong1[1].size; 	/* d3d_status_addr */

		ret = os_mem_alloc(&isp->pdev->dev, &isp->d3d_pingpong1[0]);
		if (ret < 0) {
			vin_err("isp 3d pingpong1 buf0-d3d_byr_addr requset failed!\n");
			return -ENOMEM;
		}

		ret = os_mem_alloc(&isp->pdev->dev, &isp->d3d_pingpong1[1]);
		if (ret < 0) {
			vin_err("isp 3d pingpong1 buf1-d3d_k0_addr requset failed!\n");
			return -ENOMEM;
		}

		ret = os_mem_alloc(&isp->pdev->dev, &isp->d3d_pingpong1[2]);
		if (ret < 0) {
			vin_err("isp 3d pingpong1 buf2-d3d_k1_addr requset failed!\n");
			return -ENOMEM;
		}

		ret = os_mem_alloc(&isp->pdev->dev, &isp->d3d_pingpong1[3]);
		if (ret < 0) {
			vin_err("isp 3d pingpong1 buf3-d3d_status_addr requset failed!\n");
			return -ENOMEM;
		}
	}
#endif
...
{
	if (isp->d3d_pingpong[0].vir_addr)
...
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
	if (isp->id == SOFTWARE_TDM_ID) {
		os_mem_free(&isp->pdev->dev, &isp->d3d_pingpong1[0]);
		os_mem_free(&isp->pdev->dev, &isp->d3d_pingpong1[1]);
		os_mem_free(&isp->pdev->dev, &isp->d3d_pingpong1[2]);
		os_mem_free(&isp->pdev->dev, &isp->d3d_pingpong1[3]);
	}
#endif
#endif
}
...
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
	if (isp->id == SOFTWARE_TDM_ID) {
		if (isp->isp_handle_embed) {
			addr = (vin_dma_addr_t)isp->d3d_pingpong1[0].dma_addr;
			bsp_isp_set_d3d_bayer_addr(isp->id, addr);
			addr = (vin_dma_addr_t)isp->d3d_pingpong1[1].dma_addr;
			bsp_isp_set_d3d_k0_addr(isp->id, addr);
			addr = (vin_dma_addr_t)isp->d3d_pingpong1[2].dma_addr;
			bsp_isp_set_d3d_k1_addr(isp->id, addr);
			addr = (vin_dma_addr_t)isp->d3d_pingpong1[3].dma_addr;
...
*/
	...
	if (isp->id == SOFTWARE_TDM_ID) {
		if (isp->isp_handle_embed)
			bsp_isp_set_input_fmt1(isp->id, isp->infmt1);
		else
			bsp_isp_set_input_fmt(isp->id, isp->infmt);
	} else {
		bsp_isp_set_input_fmt(isp->id, isp->infmt);
	}
#else
...
static int sunxi_isp_subdev_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		if (isp->id == SOFTWARE_TDM_ID) {
			if (isp->load_flag1) {
				memcpy(isp->isp_load.vir_addr, &isp->load_shadow1[0], ISP_LOAD_DRAM_SIZE);
			}
			memset(isp->isp_save_load1.vir_addr, 0, ISP_SAVE_LOAD_DRAM_SIZE);
		}
#endif
#endif
		memset(isp->isp_save_load.vir_addr, 0, ISP_SAVE_LOAD_DRAM_SIZE);
		...
		if (isp->large_image == 3)
			bsp_isp_irq_enable(isp->id, START_INT_EN);
#endif
		...
		/* load addr*/
		isp->infmt = isp->isp_fmt->infmt;
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
		if (isp->id == SOFTWARE_TDM_ID)
			isp->infmt1 = isp->isp_fmt->infmt;
#endif
...
int sunxi_isp_subdev_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if (val) {
		...
		if (!isp->have_init) {
			...
			if (isp->id == SOFTWARE_TDM_ID) {
				if (!isp->load_flag1) {
					memset(&isp->load_shadow1[0], 0, ISP_LOAD_DRAM_SIZE);
					isp->load_flag1 = 0;
				}
			}
#endif
...
static int __isp_set_load_reg(struct v4l2_subdev *sd, struct isp_table_reg_map *reg)
{
	...
	if (isp->large_image == 3 && glb_isp[0]) {
		if (isp->id == 0) {
			...
			spin_lock_irqsave(&glb_isp[0]->slock, flags);
			isp->load_buf_busy = 1;
			spin_unlock_irqrestore(&glb_isp[0]->slock, flags);
...
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
		if (isp->id == SOFTWARE_TDM_ID) {
			ret = copy_from_user(&isp->load_shadow[0], reg->addr, 4);
			if (isp->load_shadow[ISP_LOAD_BUF_SIGN0_OFFSET] == ISP_LOAD_BUF_SIGN0 && isp->load_shadow[ISP_LOAD_BUF_SIGN1_OFFSET] == ISP_LOAD_BUF_SIGN1) {
				ret = copy_from_user(&isp->load_shadow1[0], reg->addr, reg->size);
				isp->load_flag1 = 1;
				vin_log(VIN_LOG_TDM, "get load1 form user\n");
			} else {
				ret = copy_from_user(&isp->load_shadow[0], reg->addr, reg->size);
				isp->load_flag = 1;
				vin_log(VIN_LOG_TDM, "get load form user\n");
			}
			isp->load_save = 1;
			return ret;
		}
...
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
	if (isp->id == SOFTWARE_TDM_ID) {
		isp->isp_stat1.size = ISP_SAVE_LOAD_DRAM_SIZE + 2 * ISP_LOAD_DRAM_SIZE;
		ret = os_mem_alloc(&isp->pdev->dev, &isp->isp_stat1);
		if (ret < 0) {
			vin_err("isp statistic buf1 requset failed!\n");
			return -ENOMEM;
		}
		isp->isp_save_load1.dma_addr = isp->isp_stat1.dma_addr;
		isp->isp_save_load1.vir_addr = isp->isp_stat1.vir_addr;
		isp->load_para1[0].dma_addr = isp->isp_stat1.dma_addr + ISP_SAVE_LOAD_DRAM_SIZE;
		isp->load_para1[0].vir_addr = isp->isp_stat1.vir_addr + ISP_SAVE_LOAD_DRAM_SIZE;
		isp->load_para1[1].dma_addr = isp->isp_stat1.dma_addr + ISP_SAVE_LOAD_DRAM_SIZE + ISP_LOAD_DRAM_SIZE;
		isp->load_para1[1].vir_addr = isp->isp_stat1.vir_addr + ISP_SAVE_LOAD_DRAM_SIZE + ISP_LOAD_DRAM_SIZE;
	}
#endif
#endif
	return ret;
...
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
	if (isp->id == SOFTWARE_TDM_ID)
		os_mem_free(&isp->pdev->dev, &isp->isp_stat1);
#endif
}

#if IS_ENABLED(CONFIG_TDM_OFFLINE_HANDLE_RAW) || IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
void isp_update_load_for_embed(struct v4l2_subdev *sd, bool embed_mark);
#endif
...
static void __isp_tx_buf_handle(struct isp_dev *isp)
{
	...
	vin_log(VIN_LOG_TDM, "\n");
#else /* CONFIG_SOFTWARE_TDM_FOR_ISP */
	struct vin_md *vind = dev_get_drvdata(isp->subdev.v4l2_dev->dev);
	struct vin_core *vinc = NULL;
	struct tdm_rx_dev *tdm_rx;
	struct tdm_dev *tdm;
	struct tdm_buffer *buf = NULL;
	unsigned int delay_cnt = 10;
	bool open_tx = false;
	int i, j;
	...
	list_move_tail(&buf->list, &tdm->rx_active[tdm_rx->id]);
	tdm->tx_busy = false;
	vin_log(VIN_LOG_TDM, "isp%d frame end\n", isp->id);

	for (i = 0; i < TDM_RX_NUM; i++) {
		if (!tdm->tdm_rx[i].subdev.entity.stream_count)
			continue;
		if (!list_empty(&tdm->rx_done[i])) {
			open_tx = true;
			if (i == SOFTWARE_TDM_ID) {
				for (j = 0; j < VIN_MAX_DEV; j++) {
					if (vind->vinc[j] == NULL)
						continue;
					if (!vin_streaming(&vind->vinc[j]->vid_cap))
						continue;

					if (vind->vinc[j]->tdm_rx_sel == SOFTWARE_TDM_ID) {
						vinc = vind->vinc[j];
						break;
					}
				}
				...
				if (vinc) {
					buf = list_entry(tdm->rx_done[i].next, struct tdm_buffer, list);
					vin_log(VIN_LOG_TDM, "isp tdm rx%d send buf%d data%d\n", i, buf->id, buf->embed_mask);
					isp_update_load_for_embed(vinc->vid_cap.pipe.sd[VIN_IND_ISP], buf->embed_mask);
				}
			}
		}
	}
...
#elif (defined BK_ONE_BUFFER_MODE || IS_ENABLED(CONFIG_TDM_USE_BK_BUFFER)) && defined SUPPORT_ISP_TDM && defined TDM_V200
static void __isp_tx_buf_handle(struct isp_dev *isp)
{
	struct vin_md *vind = dev_get_drvdata(isp->subdev.v4l2_dev->dev);
	struct vin_core *vinc = NULL;
	...
	if (vinc == NULL || vinc->use_one_buffer != 2)
		return;

	tdm_rx = container_of(vinc->vid_cap.pipe.sd[VIN_IND_TDM_RX], struct tdm_rx_dev, subdev);

	if (tdm_rx->bk_cap_once_ready) {
		csic_dma_enable(vinc->vipp_sel);
		tdm_rx->bk_cap_once_ready = false;
		//vin_print("dma%d open\n", vinc->id);
	}
}

#if IS_ENABLED(CONFIG_TDM_USE_BK_BUFFER)
static void __isp_speed_handle(struct isp_dev *isp)
{
	struct vin_md *vind = dev_get_drvdata(isp->subdev.v4l2_dev->dev);
	struct vin_core *vinc = NULL;
	struct tdm_rx_dev *tdm_rx;
...
static int isp_probe(struct platform_device *pdev)
{
	...
	if (isp->isp_reserved_np) {
		...
		if (!ret) {
			...
			vin_log(VIN_LOG_VIDEO, "register memory isp start 0x%llx, size:0x%x.\n", (unsigned long long)isp->reserved_r.start, isp->reserved_len);
		} else
...
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
		if (isp->id == SOFTWARE_TDM_ID)
			bsp_isp_map_save_load_dram_addr1(isp->id, (vin_dma_addr_t)isp->isp_save_load1.vir_addr);
#endif
...
void isp_update_load_for_embed(struct v4l2_subdev *sd, bool embed_mark)
{
	...
	isp->isp_handle_embed = embed_mark;
	...
	//vin_log(VIN_LOG_TDM, "isp%d handle embed data%d\n", isp->id, embed_mark);
	...
	if (embed_mark) {
		if (isp->load_select1) {
			memcpy(isp->load_para1[1].vir_addr, isp->isp_load.vir_addr, ISP_LOAD_DRAM_SIZE);
			bsp_isp_set_load_addr(isp->id, (vin_dma_addr_t)isp->load_para1[1].dma_addr);
			isp->load_select1 = false;
		} else {
			memcpy(isp->load_para1[0].vir_addr, isp->isp_load.vir_addr, ISP_LOAD_DRAM_SIZE);
			bsp_isp_set_load_addr(isp->id, (vin_dma_addr_t)isp->load_para1[0].dma_addr);
			isp->load_select1 = true;
		}
		bsp_isp_set_save_load_addr(isp->id, (vin_dma_addr_t)isp->isp_save_load1.dma_addr);
	} else {
		if (isp->load_select) {
			memcpy(isp->load_para[1].vir_addr, isp->isp_load.vir_addr, ISP_LOAD_DRAM_SIZE);
			bsp_isp_set_load_addr(isp->id, (vin_dma_addr_t)isp->load_para[1].dma_addr);
