...
static s32 de_enhance_init_para(u32 disp, u32 chn)
{
	...
	if (g_cfg->lti_exist[disp][chn])
		de_lti_init_para(disp, chn);
	if (g_cfg->peak_exist[disp][chn])
...
s32 de_enhance_init(struct disp_bsp_init_para *para)
{
	...
	for (disp = 0; disp < g_cfg->device_num; disp++) {
		for (chn = 0; chn < g_cfg->vep_num[disp]; chn++) {
			...
			g_cfg->lti_exist[disp][chn] =
				de_feat_is_support_lti_by_chn(disp, chn);
			g_cfg->peak_exist[disp][chn] =
				de_feat_is_support_peak_by_chn(disp, chn);
			g_cfg->dci_exist[disp][chn] =
	...
	if (blks == NULL) {
		for (chn = 0; chn < chn_num; ++chn) {
			if (de_feat_is_support_vep_by_chn(disp, chn)) {
				if (g_cfg->lti_exist[disp][chn]) {
					de_lti_get_reg_blocks(disp, chn, blks,
							      &num);
					total += num;
				}
				if (g_cfg->peak_exist[disp][chn]) {
					de_peak_get_reg_blocks(disp, chn, blks,
							      &num);
					total += num;
				}
				if (g_cfg->dlc_exist[disp][chn]) {
