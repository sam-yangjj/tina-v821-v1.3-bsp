...
/**
 * sunxi_drm_atomic_helper_commit_hw_done - setup possible nonblocking commit
 ...
 */
static void sunxi_drm_atomic_helper_commit_hw_done(struct drm_atomic_state *old_state)
{
	struct drm_crtc *crtc;
...

{
	...
	sunxi_drm_atomic_helper_commit_hw_done(old_state);
...
static int sunxi_drm_atomic_commit_setup(struct drm_atomic_state *state)
{
	struct drm_crtc *crtc;
	struct drm_crtc_state *new_crtc_state;
	struct sunxi_crtc_state *cstate = NULL;
	unsigned int i;

	for_each_new_crtc_in_state(state, crtc, new_crtc_state, i) {
		cstate = to_sunxi_crtc_state(new_crtc_state);
		cstate->commit_setup_done = true;
	}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 11, 0)
	.atomic_commit_setup = sunxi_drm_atomic_commit_setup,
#endif
};
