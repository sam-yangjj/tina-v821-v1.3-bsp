...
FREE_DMA:
	...
	disp_free((void *)buf_addr_vir, (void *)(unsigned int)phy_addr, size);
#else
...
void *disp_malloc(u32 num_bytes, void *phys_addr)
{
	...
	if (num_bytes != 0) {
		...
		if (address) {
			DE_INFO
			    ("dma_alloc_coherent ok, address=0x%lx, size=0x%x\n",
			     (unsigned long)(*(unsigned long *)phys_addr), num_bytes);
			return address;
		}
...
void disp_free(void *virt_addr, void *phys_addr, u32 num_bytes)
{
	...
	if (phys_addr && virt_addr)
		...
		dma_free_coherent(g_disp_drv.dev, actual_bytes, virt_addr,
				  (dma_addr_t)(unsigned int)phys_addr);
#else
		dma_free_coherent(g_disp_drv.dev, actual_bytes, virt_addr,
...
#else
static int __disp_dma_heap_alloc_coherent(struct disp_ion_mem *mem) { return 0; }
#endif
...
static inline void disp_init_bsp_init_para(struct disp_bsp_init_para *para, const struct disp_drv_info *drv_info)
{
	...
	para->clk_de_hb = drv_info->clk_de_hb;
	para->clk_disp_ahb = drv_info->clk_disp_ahb;
	para->clk_disp_mbus = drv_info->clk_disp_mbus;
...
static int disp_clk_get_wrap(struct disp_drv_info *disp_drv)
{
	...
	sprintf(id, "clk_de_hb");
	disp_drv->clk_de_hb = devm_clk_get(dev, id);
	if (IS_ERR(disp_drv->clk_de_hb)) {
		disp_drv->clk_de_hb = NULL;
		DE_INFO("failed to get clk for %s\n", id);
	}

	sprintf(id, "clk_disp_ahb");
	disp_drv->clk_disp_ahb = devm_clk_get(dev, id);
	if (IS_ERR(disp_drv->clk_disp_ahb)) {
		disp_drv->clk_disp_ahb = NULL;
		DE_INFO("failed to get clk for %s\n", id);
	}

	sprintf(id, "clk_disp_mbus");
	disp_drv->clk_disp_mbus = devm_clk_get(dev, id);
	if (IS_ERR(disp_drv->clk_disp_mbus)) {
		disp_drv->clk_disp_mbus = NULL;
		DE_INFO("failed to get clk for %s\n", id);
	}
...
static void disp_clk_put_wrap(struct disp_drv_info *disp_drv)
{
	...
	devm_clk_put(dev, disp_drv->clk_de_hb);
	devm_clk_put(dev, disp_drv->clk_disp_ahb);
	devm_clk_put(dev, disp_drv->clk_disp_mbus);
...
handle_cmd:
	...
	case DISP_RTWB_SYNC_FD:
	{
		...
		if (info2.window.x != info2.out_frame.crop.x) {
			DE_WARN("window.x and out_frame.crop.x are inconsistent.\n");
			return -EFAULT;
		}

		if (info2.window.width != info2.out_frame.size[0].width || info2.window.width != info2.out_frame.crop.width) {
			DE_WARN("window.width, out_frame.size[0].width, and out_frame.crop.width are inconsistent.\n");
			return -EFAULT;
		}

		if (info2.window.y != info2.out_frame.crop.y) {
			DE_WARN("window.y and out_frame.crop.y are inconsistent.\n");
			return -EFAULT;
		}

		if (info2.window.height != info2.out_frame.size[0].height || info2.window.height != info2.out_frame.crop.height) {
			DE_WARN("window.height, out_frame.size[0].height, and out_frame.crop.height are inconsistent.\n");
			return -EFAULT;
		}

		if (mgr)
			ret = disp_mgr_set_rtwb_sync_fd(mgr, &info2);
#endif
		break;
	}
