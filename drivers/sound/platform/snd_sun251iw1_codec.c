...
static SOC_ENUM_SINGLE_DECL(sunxi_fminl_gain_enum, SND_SOC_NOPM, FMINL_GAIN_SHIFT, sunxi_input_gain_text);
static SOC_ENUM_SINGLE_DECL(sunxi_fminr_gain_enum, SND_SOC_NOPM, FMINR_GAIN_SHIFT, sunxi_input_gain_text);
static SOC_ENUM_SINGLE_DECL(sunxi_lineinl_gain_enum, SND_SOC_NOPM, LINEINL_GAIN_SHIFT, sunxi_input_gain_text);
static SOC_ENUM_SINGLE_DECL(sunxi_lineinr_gain_enum, SND_SOC_NOPM, LINEINR_GAIN_SHIFT, sunxi_input_gain_text);
/* Digital-tlv */
...
/* FMIN & LINEIN Gain FUNC */
static int sunxi_codec_get_input_gain(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct sunxi_codec *codec = snd_soc_component_get_drvdata(component);
	...
	switch (shift) {
	case FMINL_GAIN_SHIFT:
		regmap_read(regmap, SUNXI_ADC1_REG, &reg_val);
		ucontrol->value.integer.value[0] = (reg_val & 0x3 << FMINLG) ? 1 : 0;
		break;
	case FMINR_GAIN_SHIFT:
		regmap_read(regmap, SUNXI_ADC2_REG, &reg_val);
		ucontrol->value.integer.value[0] = (reg_val & 0x3 << FMINRG) ? 1 : 0;
		break;
	case LINEINL_GAIN_SHIFT:
		regmap_read(regmap, SUNXI_ADC1_REG, &reg_val);
		ucontrol->value.integer.value[0] = (reg_val & 0x3 << LINEINLG) ? 1 : 0;
		break;
	case LINEINR_GAIN_SHIFT:
		regmap_read(regmap, SUNXI_ADC2_REG, &reg_val);
		ucontrol->value.integer.value[0] = (reg_val & 0x3 << LINEINRG) ? 1 : 0;
		break;
	default:
		return -EINVAL;
	}
...
static int sunxi_codec_set_input_gain(struct snd_kcontrol *kcontrol,
				      struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct sunxi_codec *codec = snd_soc_component_get_drvdata(component);
...
static const struct snd_kcontrol_new sunxi_codec_controls[] = {
	...
	/* INPUT Gain Func */
	SOC_ENUM_EXT("FMINL Gain", sunxi_fminl_gain_enum,
		     sunxi_codec_get_input_gain, sunxi_codec_set_input_gain),
	SOC_ENUM_EXT("FMINR Gain", sunxi_fminr_gain_enum,
		     sunxi_codec_get_input_gain, sunxi_codec_set_input_gain),
	SOC_ENUM_EXT("LINEINL Gain", sunxi_lineinl_gain_enum,
		     sunxi_codec_get_input_gain, sunxi_codec_set_input_gain),
	SOC_ENUM_EXT("LINEINR Gain", sunxi_lineinr_gain_enum,
		     sunxi_codec_get_input_gain, sunxi_codec_set_input_gain),
	...
	SOC_SINGLE_TLV("ADC1 Gain", SUNXI_ADC1_REG, ADC1_PGA_GAIN_CTRL, 0xF, 0, adc1_gain_tlv),
	SOC_SINGLE_TLV("ADC2 Gain", SUNXI_ADC2_REG, ADC2_PGA_GAIN_CTRL, 0xF, 0, adc2_gain_tlv),
};
...
static void sunxi_codec_init(struct snd_soc_component *component)
{
	...
	regmap_update_bits(regmap, SUNXI_ADC1_REG,
			   0x3 << FMINLG, dts->fminl_gain << FMINLG);
	regmap_update_bits(regmap, SUNXI_ADC2_REG,
			   0x3 << FMINRG, dts->fminr_gain << FMINRG);
	regmap_update_bits(regmap, SUNXI_ADC1_REG,
			   0x3 << LINEINLG, dts->lineinl_gain << LINEINLG);
	regmap_update_bits(regmap, SUNXI_ADC2_REG,
			   0x3 << LINEINRG, dts->lineinr_gain << LINEINRG);
