	...
	if (vinc->vid_cap.pipe.sd[VIN_IND_ISP]) {
		...
		if (isp->id == SOFTWARE_TDM_ID) {
			count += scnprintf(buf + count, size - count,
			   ...
			   4,
			  PAGE_ALIGN(isp->d3d_pingpong[0].size) * 2, PAGE_ALIGN(isp->d3d_pingpong[1].size) * 2,
			  PAGE_ALIGN(isp->d3d_pingpong[2].size) * 2, PAGE_ALIGN(isp->d3d_pingpong[3].size) * 2,
...
static void set_sensor_expgain_handle(struct work_struct *work)
{
	struct vin_core *vinc = container_of(work, struct vin_core, set_sensor_expgain_task);
	struct vin_vid_cap *cap = &vinc->vid_cap;
	...
	v4l2_subdev_call(cap->pipe.sd[VIN_IND_SENSOR], core, ioctl,
						SET_SENSOR_EXP_GAIN_DELAY, NULL);
}
...
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
static int vin_get_embed_from_isp(struct vin_core *vinc)
{
	struct isp_dev *isp = v4l2_get_subdevdata(vinc->vid_cap.pipe.sd[VIN_IND_ISP]);
	...
	vin_log(VIN_LOG_TDM, "get isp embed mask is %d\n", isp->isp_handle_embed);
	return isp->isp_handle_embed;
}
#endif
...
static irqreturn_t vin_isr(int irq, void *priv)
{
	...
	__maybe_unused struct tdm_rx_dev *tdm_rx = NULL, *tdm_rx_bind = NULL;
#ifndef BUF_AUTO_UPDATE
	...
	if (status.vsync_trig) {
		...
		vin_vsync_isr(cap);
		if (vinc->ve_online_cfg.ve_online_support_flag && vinc->vid_cap.ve_online_vsync_pd) {
		// vin_err("vinc->vid_cap.ve_online_vsync_pd(vinc->id);\n");
			vinc->vid_cap.ve_online_vsync_pd(vinc->id);
		}

		schedule_work(&vinc->set_sensor_expgain_task);
		...
		if (vinc->use_one_buffer) {
			if (vinc->large_image == 3) {
				if (vinc->id % 2 == 0 && vin_core_gbl[vinc->id + 1]) {
					vinc_bind = vin_core_gbl[vinc->id + 1];
					if (vinc->frame_use_flag == 0 && vinc_bind->frame_use_flag == 0) {
						vinc->frame_use_flag = 1;
					} else {
						vinc->frame_use_flag = 0;
					}
					goto vsync_end;
				} else if (vinc->id % 2 == 1 && vin_core_gbl[vinc->id - 1]) {
					vinc_bind = vin_core_gbl[vinc->id - 1];
					if (vinc_bind->frame_use_flag == 1) {
						vinc->frame_use_flag = 1;
					} else {
						vinc_bind->frame_use_flag = 0;
					}

					if (vinc_bind->frame_use_flag == 1 && vinc->frame_use_flag == 1) {
						csic_dma_disable(vinc_bind->id);
...
#if !IS_ENABLED(CONFIG_FRAMEDONE_TWO_BUFFER) || IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
#if IS_ENABLED(CONFIG_FRAMEDONE_TWO_BUFFER) && IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
		if (vinc->id != SOFTWARE_TDM_ID) {
			if (cap->first_flag == 0)
				cap->first_flag++;
			goto vsync_end;
		}
...
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
					buf->vb.sequence = (vinc->vin_status.frame_cnt & 0x7fffffff) | (vin_get_embed_from_isp(vinc) << 31);
#else
					buf->vb.sequence = vinc->vin_status.frame_cnt;
#endif
					buf->vb.vb2_buf.timestamp = ktime_get_boottime();
...
vsync_end:
	...
	if (status.frame_done) {
...
#ifdef CONFIG_FASTBOOT_PRINT_BOOTTIME
		/* if offline -> 1, online -> 0 */
		if (vinc->id < ARRAY_SIZE(vinc_first_frame) && !vinc_first_frame[vinc->id]) {
			vinc_first_frame[vinc->id]++;
		...
		if (vinc->large_image == 5 && vinc->id % 4 == 0) {
			if (vinc->tdm_rx_sel != 0xff) {
				...
				if (tdm_rx->pic_err) {
					tdm_rx->pic_err = 0;
					csic_dma_top_disable(dma_virtual_find_logic[vinc->id]);
					for (i = 0; i < 20; i++)
						;
					csic_dma_top_enable(dma_virtual_find_logic[vinc->id]);
					vin_print("video%d top reset\n", vinc->id);
				}
			}
...
#else /* !CONFIG_FRAMEDONE_TWO_BUFFER */
		if (vinc->once_resume && info->frame_mode) {
			...
			if (cap->frame_delay_cnt > 0 && vinc->large_image != 3) {
				if (cap->frame_delay_cnt >= 5)
					cap->frame_delay_cnt = 5;
				list_add_tail(&buf->list, &cap->vidq_active);
				cap->frame_delay_cnt--;
			...
			} else if (vinc->large_image == 3) {
				if (vinc->id % 2 == 1 && vin_core_gbl[vinc->id - 1]) {
					vinc_bind = vin_core_gbl[vinc->id - 1];
					if (vinc->frame_use_flag == 1 && vinc_bind->frame_use_flag == 1 && cap->frame_delay_cnt == 0 && vinc_bind->vid_cap.frame_delay_cnt == 0) {
						if (cap->special_active == 1) {
							list_add_tail(&buf->list, &cap->vidq_done);
							need_callback = 1;
						} else {
						...
						vinc->frame_use_flag = 0;
						vinc_bind->frame_use_flag = 0;
						csic_dma_frame_drop_disable(vinc->id);
						csic_dma_frame_drop_disable(vinc_bind->id);
					} else {
						list_add_tail(&buf->list, &cap->vidq_active);
						vinc->vin_status.lost_cnt++;
						if (cap->frame_delay_cnt)
							cap->frame_delay_cnt--;
						if (vinc_bind->vid_cap.frame_delay_cnt)
							vinc_bind->vid_cap.frame_delay_cnt--;
						vinc->frame_use_flag = 0;
						vinc_bind->frame_use_flag = 0;
...
#endif /* end CONFIG_FRAMEDONE_TWO_BUFFER */
#else /* CONFIG_BUF_GET_FRAME_DONE */
		vinc->vin_status.frame_cnt++;
		vin_get_rest_buf_cnt(vinc);
	...
	if (status.stored_frm_cnt) {
		...
		timestamp_ns = ktime_get_boottime();
...
int vin_core_suspend(struct device *d)
{
	...
	struct vin_vid_cap *cap = &vinc->vid_cap;
	__maybe_unused struct vin_core *vinc_bind = NULL;
	__maybe_unused struct vin_vid_cap *cap_bind = NULL;
	__maybe_unused struct isp_dev *isp;
	__maybe_unused struct isp_dev *isp_bind;
	__maybe_unused struct tdm_rx_dev *tdm_rx;
	__maybe_unused struct tdm_rx_dev *tdm_rx_bind;
	...
	if (test_and_set_bit(VIN_LPM, &vinc->vid_cap.state))
		return 0;
	if (vinc->dma_merge_mode && vinc->id % 4 == 0)
		return 0;
	...
	if (vin_busy(cap)) {
		...
		if (vinc->dma_merge_mode) {
			if (vinc->id % 4  == 1 && vin_core_gbl[vinc->id - 1]) {
				vinc_bind = vin_core_gbl[vinc->id - 1];
				cap_bind = &vinc_bind->vid_cap;
				if (cap_bind->pipe.sd[VIN_IND_ISP] != NULL) {
					isp_bind = container_of(cap_bind->pipe.sd[VIN_IND_ISP], struct isp_dev, subdev);
					isp_bind->runtime_flag = 1;
				}
#if defined SUPPORT_ISP_TDM && defined TDM_V200
				if (vinc_bind->tdm_rx_sel != 0xff) {
					tdm_rx_bind = container_of(cap_bind->pipe.sd[VIN_IND_TDM_RX], struct tdm_rx_dev, subdev);
					if (vinc_bind->use_one_buffer)
						tdm_rx_bind->runtime_flag = 1;
				}
#endif
...
#ifdef CONFIG_FASTBOOT_PRINT_BOOTTIME
	if (vinc->id < ARRAY_SIZE(vinc_first_frame))
		vinc_first_frame[vinc->id] = 0;
#endif
	vin_log(VIN_LOG_POWER, "%s\n", __func__);
	if (!vinc->vid_cap.registered)
		return 0;
...
#endif
	INIT_WORK(&vinc->set_sensor_expgain_task, set_sensor_expgain_handle);
#if IS_ENABLED(CONFIG_PM)
