...
#define ION_HEAP_CUSTOM_START_MASK		(1 << 9)
...
extern int vin_mem_pool_dma_alloc(struct device *dev, struct vin_mm *mem_man);
extern void vin_mem_pool_dma_free(struct device *dev, struct vin_mm *mem_man);
extern int vin_mem_pool_dma_mmap(struct device *dev, struct vm_area_struct *vma, void *cpu_addr, dma_addr_t dma_addr, size_t size, unsigned long attrs);
