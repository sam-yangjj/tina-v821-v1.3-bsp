...
static struct drm_crtc_state *sunxi_crtc_duplicate_state(struct drm_crtc *crtc)
{
	...
	// commit_setup_done should be reset on next commit
	state->commit_setup_done = false;
...
{
	struct sunxi_crtc_state *cstate, *new_cstate;
	struct drm_atomic_state *state = s->state;
	struct drm_crtc_state *new_crtc_state = NULL;

	cstate = to_sunxi_crtc_state(s);

	if (cstate->backend_blob) {
		drm_property_blob_put(cstate->backend_blob);
	}
	if (cstate->sunxi_ctm) {
		drm_property_blob_put(cstate->sunxi_ctm);
	}

	/*
	 * if swap done, commit_setup_done would be set in atomic_commit_setup
	 * ops,if it is false  means atomic_commit is aborted, we should decrease
	 * the kref of commit which has not release by drm framework because of a
	 * BUG such foolow:
	 *
	 ...
	 *
	 * we should fix it by put the commit manaually ourself, and should set
	 * abort_completion to false to avoid double-free in crtc_state_destroy
	 * if abort_completion is true, means setup_commit is not abort in
	 * stall_checks(), need to decrease refcount got by new_crtc_state->event->base.completion
	 */
	if (state) {
		new_crtc_state = drm_atomic_get_new_crtc_state(state, crtc);
		if (new_crtc_state) {
			spin_lock(&crtc->commit_lock);
			new_cstate = to_sunxi_crtc_state(new_crtc_state);
			if (!new_cstate->commit_setup_done && new_crtc_state->commit) {
				if (new_crtc_state->commit->abort_completion == true) {
					drm_crtc_commit_put(new_crtc_state->commit);
					new_crtc_state->commit->abort_completion = false;
					drm_dbg_dp(crtc->dev, "Warnning: atomic has aborted early, need to dec commit's ref manaually!!!\n");
				} else {
					DRM_INFO("Warnning: atomic has aborted because of stall_check!\n");
				}
			}
...
static void sunxi_drm_crtc_vblank_work(struct kthread_work *base)
{
	struct drm_vblank_work *work = to_drm_vblank_work(base);
	struct sunxi_drm_crtc *scrtc =
		container_of(work, typeof(*scrtc), vblank_work);

	DRM_DEBUG_DRIVER("[SUNXI-CRTC] %s: in frame%llu", __func__,
			 drm_crtc_accurate_vblank_count(&(scrtc->crtc)));
	sunxi_de_vblank_work(scrtc->sunxi_de);
}
...
void sunxi_drm_crtc_vblank_work_init(struct sunxi_drm_crtc *scrtc)
{
	if (!scrtc) {
		DRM_ERROR("crtc is NULL\n");
		return ;
	...
	drm_vblank_work_init(&scrtc->vblank_work, &scrtc->crtc, sunxi_drm_crtc_vblank_work);
}

int sunxi_drm_crtc_vblank_work_schedule(struct sunxi_drm_crtc *scrtc, int frame)
{
	if (!scrtc) {
		DRM_ERROR("crtc is NULL\n");
		return -EINVAL;
	...
	DRM_DEBUG_DRIVER("[SUNXI-CRTC] %s: in frame%llu", __func__,
			 drm_crtc_accurate_vblank_count(&(scrtc->crtc)) + frame);
	return drm_vblank_work_schedule(&(scrtc->vblank_work),
					drm_crtc_accurate_vblank_count(&(scrtc->crtc)) + frame,
					false);
}
