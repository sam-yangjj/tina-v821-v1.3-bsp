...
#define HWLOCK_TIMEOUT_MS_DEFAULT	(1000)
...
struct sunxi_spif {
	...
	int hwlock_timeout_ms;
	atomic_t hwlock_locked;
	atomic_t hwlock_available;
	struct completion hwlock_unlock_done;
...
extern void sunxi_spif_update_sample_delay_para(struct mtd_info *mtd, struct sunxi_boot_param_region *boot_param);
extern int sunxi_spif_controller_ops(struct spi_device *spi, struct spi_transfer *t);
extern ssize_t sunxi_spif_nor_io_read(struct spi_nor *nor, loff_t from, size_t len,
				 u_char *buf);
