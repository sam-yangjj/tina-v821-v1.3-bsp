...
int xradio_bh_suspend(struct xradio_common *hw_priv)
{
	...
	wake_up(&hw_priv->bh_wq);
#endif
	return wait_event_timeout(hw_priv->bh_evt_wq, (hw_priv->bh_error || XRADIO_BH_SUSPENDED ==
				  atomic_read(&hw_priv->bh_suspend)), 1 * HZ) ? 0 : -ETIMEDOUT;
}
...
int xradio_bh_resume(struct xradio_common *hw_priv)
{
	...
	ret = wait_event_timeout(hw_priv->bh_evt_wq, (hw_priv->bh_error ||
	     atomic_read(&hw_priv->bh_suspend) == XRADIO_BH_RESUMED), 1 * HZ) ? 0 : -ETIMEDOUT;
...
{
	int len = SKB_RESV_MAX + WSM_TX_EXTRA_HEADROOM + 8 + 12;	/* TKIP IV + ICV and MIC */
