/*
 *  common_gpio_qualcomm.c - Linux kernel modules for sensortek w11xx
 *  ambient light sensor (Common function)
 *
 *  Copyright (C) 2019 Taka Chiu, sensortek Inc.
 *
...
int register_gpio_irq(stk_gpio_info *gpio_info)
{
	...
	for (i = 0; i < MAX_LINUX_GPIO_MANAGER_NUM; i++) {
		if (!linux_gpio_mgr[i].gpio_info) {
			linux_gpio_mgr[i].gpio_info = gpio_info;
			gpio_idx = i;
			break;
		} else {
			if (linux_gpio_mgr[i].gpio_info == gpio_info) {
				//already register
				return -1;
			}
		}
	...
	linux_gpio_mgr[gpio_idx].stk_wq = create_singlethread_workqueue(
		linux_gpio_mgr[gpio_idx].gpio_info->wq_name);
	...
	printk(KERN_INFO "%s: int pin #=%d, irq=%d\n", __func__,
	       linux_gpio_mgr[gpio_idx].gpio_info->int_pin, irq);
	...
	case TRIGGER_RISING:
		err = request_any_context_irq(
			irq, stk_gpio_irq_handler, IRQF_TRIGGER_RISING,
			linux_gpio_mgr[gpio_idx].gpio_info->device_name,
			&linux_gpio_mgr[gpio_idx]);
		break;

	case TRIGGER_FALLING:
		err = request_any_context_irq(
			irq, stk_gpio_irq_handler, IRQF_TRIGGER_FALLING,
			linux_gpio_mgr[gpio_idx].gpio_info->device_name,
			&linux_gpio_mgr[gpio_idx]);
		break;
	...
	case TRIGGER_LOW:
		err = request_any_context_irq(
			irq, stk_gpio_irq_handler, IRQF_TRIGGER_LOW,
			linux_gpio_mgr[gpio_idx].gpio_info->device_name,
			&linux_gpio_mgr[gpio_idx]);
		break;
	...
	for (gpio_idx = 0; gpio_idx < MAX_LINUX_GPIO_MANAGER_NUM; gpio_idx++) {
		if (linux_gpio_mgr[gpio_idx].gpio_info == gpio_info) {
			if (linux_gpio_mgr[gpio_idx].gpio_info->is_exist) {
				if (!linux_gpio_mgr[gpio_idx]
					     .gpio_info->is_active) {
					linux_gpio_mgr[gpio_idx]
						.gpio_info->is_active = true;
				}
			}
		...
		if (linux_gpio_mgr[gpio_idx].gpio_info == gpio_info) {
			if (linux_gpio_mgr[gpio_idx].gpio_info->is_exist) {
				if (linux_gpio_mgr[gpio_idx]
					...
					linux_gpio_mgr[gpio_idx]
						.gpio_info->is_active = false;
					free_irq(linux_gpio_mgr[gpio_idx]
							 .gpio_info->irq,
						 &linux_gpio_mgr[gpio_idx]);
					gpio_free(linux_gpio_mgr[gpio_idx]
							  .gpio_info->int_pin);
					cancel_work_sync(
						&linux_gpio_mgr[gpio_idx]
							 .stk_work);
