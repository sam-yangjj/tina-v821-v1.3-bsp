		...
		struct v4l2_mbus_framefmt *fmt)
{
	...
	/*fps check warning*/
	if (best_ws->fps_fixed != info->tpf.denominator) {
		vin_warn("frame rate is no match !!! real frame rate:%ufps  user frame rate :%ufps\n", best_ws->fps_fixed, info->tpf.denominator);
		ws = info->win_pt;
		for (i = 0; i < info->win_size_num; ++i) {
			vin_warn("frame rate support:%ufps width:%d height:%d\n", ws->fps_fixed, ws->width, ws->height);
			++ws;
		}
	}
