...
retry:
		...
		case NATIVE_AUX_NACK:
			...
			if (retry_cnt < 7)
				goto retry;
			EDP_ERR("%s retry 7 timies but still NATIVE_AUX_NACK, request(cmd:0x%x  addr:0x%x len:%d)\n",
				    __func__, request.cmd_code, request.address, request.data_len);
			ret = RET_AUX_RETRY_FAIL;
			goto OUT;
		case NATIVE_AUX_DEFER:
			...
			if (retry_cnt < 7)
				goto retry;
			EDP_ERR("%s retry 7 timies but still NATIVE_AUX_DEFER, request(cmd:0x%x addr:0x%x len:%d)\n",
				    __func__, request.cmd_code, request.address, request.data_len);
			ret = RET_AUX_RETRY_FAIL;
			goto OUT;
		}
...
retry:
		...
		case AUX_ACK:
			...
			break;
		case I2C_AUX_NACK:
			EDP_LOW_DBG("edp_aux_i2c_read recieve I2C_AUX_NACK, request(cmd:0x%x i2c_addr:0x%x addr:0x%x len:%d)\n",
				    request.cmd_code, request.address, (addr + j * 16), request.data_len);
			retry_cnt++;
			if (retry_cnt < 7)
				goto retry;
			EDP_ERR("%s retry 7 timies but still I2C_AUX_NACK, request(cmd:0x%x i2c_addr:0x%x addr:0x%x len:%d)\n",
				    __func__, request.cmd_code, request.address, (addr + j * 16), request.data_len);
			ret = RET_AUX_RETRY_FAIL;
			goto OUT;
		case I2C_AUX_DEFER:
			EDP_LOW_DBG("edp_aux_i2c_read recieve I2C_AUX_DEFER, request(cmd:0x%x i2c_addr:0x%x addr:0x%x len:%d)\n",
				    request.cmd_code, request.address, (addr + j * 16), request.data_len);
			retry_cnt++;
			if (retry_cnt < 7)
				goto retry;
			EDP_ERR("%s retry 7 timies but still I2C_AUX_DEFER, request(cmd:0x%x i2c_addr:0x%x addr:0x%x len:%d)\n",
				    __func__, request.cmd_code, request.address, (addr + j * 16), request.data_len);
			ret = RET_AUX_RETRY_FAIL;
			goto OUT;
		}
