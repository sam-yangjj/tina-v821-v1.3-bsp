...
#if IS_ENABLED(CONFIG_ARCH_SUN55IW3) || IS_ENABLED(CONFIG_ARCH_SUN300IW1)
//In new clk framework, we don't need too adjust pll rate
#define CONFIG_PLL_RATE_AUTO_ADJUST
#endif
...
static s32 lcd_parse_panel_para(u32 disp, struct disp_panel_para *info)
{
	...
	ret =
	    disp_sys_script_get_item(primary_key, "lcd_lvds_diff_amplifier_lv",
				     &value, 1);
	if (ret == 1)
		info->lcd_lvds_diff_amplifier_lv = value;
	else {
		info->lcd_lvds_diff_amplifier_lv = 4;
	}
	...
	ret = disp_sys_script_get_item(primary_key, "lcd_cpu_tri_mode", &value, 1);
	if (ret == 1)
		info->lcd_cpu_tri_mode = value;
	...
	ret =
	    disp_sys_script_get_item(primary_key, "lcd_hv_de_polarity",
				     &value, 1);
	if (ret == 1)
	...
	ret =
	    disp_sys_script_get_item(primary_key, "lvds0_lane_np_inv",
				     &value, 1);
	if (ret == 1)
		info->lvds0_lane_np_inv = value;

	ret =
	    disp_sys_script_get_item(primary_key, "lvds1_lane_np_inv",
				     &value, 1);
	if (ret == 1)
		info->lvds1_lane_np_inv = value;

	ret =
	    disp_sys_script_get_item(primary_key, "lvds_even_odd_dir",
				     &value, 1);
	if (ret == 1)
...
static s32 lcd_clk_config(struct disp_device *lcd)
{
	...
	struct lcd_clk_info clk_info;
#ifndef CONFIG_PLL_RATE_AUTO_ADJUST
...
#endif
{
...
#endif
	u32 hwdev_index;
	u32 tcon_counter_irq_status = 0;
	u32 tcon_finish_irq_status = 0;
	...
	if (disp_al_lcd_query_irq
		...
		tcon_counter_irq_status =
			disp_al_lcd_query_irq(hwdev_index, LCD_IRQ_TCON0_CNTR,
						&lcdp->panel_info);
		tcon_finish_irq_status =
			disp_al_lcd_query_irq(hwdev_index, LCD_IRQ_TCON0_TRIF,
						&lcdp->panel_info);

		if ((tcon_counter_irq_status | tcon_finish_irq_status) == 0)
			goto exit;
		...
		if (disp_al_lcd_tri_busy(hwdev_index, &lcdp->panel_info)) {
			...
			if (tcon_finish_irq_status) {
				if (lcdp->lcd_panel_fun.vsync_cmd) {
					lcdp->lcd_panel_fun.vsync_cmd(lcd->disp);
				}
			}
		}
...
#if defined(DE_VERSION_V21X)
		if (lcdp->panel_info.lcd_if == LCD_IF_CPU &&
			lcdp->panel_info.lcd_cpu_tri_mode == LCD_CPU_TRI_FINISH) {
			if (lcdp->panel_info.lcd_cpu_te == 0 && lcdp->panel_info.lcd_cpu_mode == 1) {
				lcdp->tri_finish_fail = 0;
				tcon_irq_query(hwdev_index, LCD_IRQ_TCON0_TRIF);
			}
		}
#endif
