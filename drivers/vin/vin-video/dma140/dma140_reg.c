...
int dma_virtual_find_ch[6] = {
	0, 1, 2, 3, 0, 1,
};
int dma_virtual_find_logic[6] = {
	0, 0, 0, 0, 4, 4,
};
#else
...
void csic_dma_set_ve_online_drop_frm(unsigned int sel, unsigned int drop_frm)
{
	if (dma_virtual_find_ch[sel] == 0)
		vin_reg_clr_set(csic_dma_base[sel] + CSIC_DMA_TOP_EN_REG_OFF,
			CSIC_VE_ONLINE_CH0_DROP_FRM_MASK, drop_frm << CSIC_VE_ONLINE_CH0_DROP_FRM);
	else if (dma_virtual_find_ch[sel] == 1)
		vin_reg_clr_set(csic_dma_base[sel] + CSIC_DMA_TOP_EN_REG_OFF,
			CSIC_VE_ONLINE_CH1_DROP_FRM_MASK, drop_frm << CSIC_VE_ONLINE_CH1_DROP_FRM);
}
