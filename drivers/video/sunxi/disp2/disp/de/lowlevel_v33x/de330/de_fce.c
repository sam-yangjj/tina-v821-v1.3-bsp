	...
	u8 __iomem **phy_addr, u8 **vir_addr, u32 *size)
{
	...
	*size -= DE_FCE_REG_MEM_SIZE;
	...
	spin_lock_init(&g_ce_status[disp][chn]->fce_lock);
	return 0;
}
		...
		u32 bwslvl)
{
	...
	if (ce_en == 1) {
		...
		if (auto_contrast_en) {
			...
			spin_lock_irqsave(&g_ce_status[disp][chn]->fce_lock, flags);
			g_ce_status[disp][chn]->clr_hist_res = 1;
			spin_unlock_irqrestore(&g_ce_status[disp][chn]->fce_lock, flags);
...
s32 de_ce_tasklet(u32 disp, u32 chn, u32 frame_cnt)
{
	...
	spin_lock_irqsave(&g_ce_status[disp][chn]->fce_lock, flags);
	if (g_ce_status[disp][chn]->clr_hist_res) {
		g_ce_status[disp][chn]->clr_hist_res = 0;
