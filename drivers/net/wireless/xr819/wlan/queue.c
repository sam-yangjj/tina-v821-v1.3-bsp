...
static inline void xradio_queue_parse_id(u32 packetID,
					 u8 *queue_generation,
					 u8 *queue_id,
					 u8 *item_generation,
					 u8 *item_id,
...
static void xradio_queue_post_gc(struct xradio_queue_stats *stats)
{
	...
	while (!list_empty(&stats->gc_list)) {
		...
		item = list_first_entry(&stats->gc_list, struct xradio_queue_item, head);
		list_del(&item->head);
...
static void __xradio_queue_gc(struct xradio_queue *queue, bool unlock)
{
	struct xradio_queue_stats *stats = queue->stats;
...
int xradio_queue_clear(struct xradio_queue *queue, int if_id)
{
	...
	if (XRWL_ALL_IFS != if_id) {
		for (i = 0; i < stats->map_capacity; ++i) {
			stats->num_queued[if_id] -= queue->link_map_cache[if_id][i];
			stats->link_map_cache[if_id][i] -= queue->link_map_cache[if_id][i];
			queue->link_map_cache[if_id][i] = 0;
		}
	} else {
		for (iter = 0; iter < XRWL_MAX_VIFS; iter++) {
			for (i = 0; i < stats->map_capacity; ++i) {
				stats->num_queued[iter] -= queue->link_map_cache[iter][i];
				stats->link_map_cache[iter][i] -= queue->link_map_cache[iter][i];
				   ...
				   u32 link_id_map)
{
	...
	if (likely(link_id_map == (u32) -1)) {
		ret = queue->num_queued_vif[priv->if_id] - queue->num_pending_vif[priv->if_id];
	} else {
		ret = 0;
		     ...
		     struct xradio_txpriv *txpriv)
{
	...
	if (!SYS_WARN(list_empty(&queue->free_pool))) {
		struct xradio_queue_item *item = list_first_entry(&queue->free_pool,
								  struct xradio_queue_item, head);
		SYS_BUG(item->skb);
		 ...
		 */
		if (queue->overfull == false && queue->num_queued >=
		     ...
		     struct xradio_txpriv **txpriv)
{
	...
	if (!SYS_WARN(ret)) {
		...
		*txpriv = &item->txpriv;
		...
		if (!--stats->link_map_cache[item->txpriv.if_id][item->txpriv.link_id])
			wakeup_stats = true;
...
#endif
{
...
#endif
		txrx_printk(XRADIO_DBG_MSG, "Requeued frame dropped for generic interface id.\n");
#ifdef CONFIG_XRADIO_TESTMODE
	...
	if (unlikely(queue_generation != queue->generation)) {
		...
		item->packetID = xradio_queue_make_packet_id(queue_generation, queue_id,
							     item_generation,
							     item_id, if_id, link_id);
...
int xradio_queue_requeue_all(struct xradio_queue *queue)
{
	...
	while (!list_empty(&queue->pending)) {
		...
		spin_lock_bh(&stats->lock);
		++stats->num_queued[item->txpriv.if_id];
		++stats->link_map_cache[item->txpriv.if_id][item->txpriv.link_id];
		spin_unlock_bh(&stats->lock);
		...
		if (hw_priv->start_stop_tsm.start) {
			if (queue_id == hw_priv->tsm_info.ac) {
				...
				if (tmval.tv_usec > item->mdelay_timestamp)
					media_delay = tmval.tv_usec - item->mdelay_timestamp;
				else
					media_delay = tmval.tv_usec +
					1000000 - item->mdelay_timestamp;
				hw_priv->tsm_info.sum_media_delay += media_delay;
