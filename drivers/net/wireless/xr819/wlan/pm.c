...
static int __xradio_wow_suspend(struct xradio_vif *priv, struct cfg80211_wowlan *wowlan);
...
static int xradio_pm_init_common(struct xradio_pm_state *pm, struct xradio_common *hw_priv)
{
	int ret;
...
int xradio_pm_init(struct xradio_pm_state *pm, struct xradio_common *hw_priv)
{
	int ret = 0;
	pm_printk(XRADIO_DBG_TRC, "%s\n", __func__);
...
int xradio_wow_suspend(struct ieee80211_hw *hw, struct cfg80211_wowlan *wowlan)
{
	...
	if (work_pending(&hw_priv->query_work)) {
		pm_printk(XRADIO_DBG_WARN, "Don't suspend because of query_work is working.\n");
		return -EBUSY;
	}
	...
	if (hw_priv->tx_queue_stats.num_queued[0] + hw_priv->tx_queue_stats.num_queued[1]) {
		pm_printk(XRADIO_DBG_WARN, "Don't suspend because of tx_queue is not empty.\n");
		return -EBUSY;
	}

	/* Make sure there is no configuration requests in progress. */
	if (down_trylock(&hw_priv->conf_lock)) {
		pm_printk(XRADIO_DBG_WARN, "Don't suspend because of configuration requests.\n");
		return -EBUSY;
	}

	/* Make sure there is no wsm_oper_lock in progress. */
	if (down_trylock(&hw_priv->wsm_oper_lock)) {
		pm_printk(XRADIO_DBG_WARN, "Don't suspen because of wsm_oper_lock.\n");
	...
	if (down_trylock(&hw_priv->scan.lock)) {
		pm_printk(XRADIO_DBG_WARN, "Don't suspend because of scan requests.\n");
		goto revert1;
	}

	if (delayed_work_pending(&hw_priv->scan.probe_work)) {
		pm_printk(XRADIO_DBG_WARN, "Don't suspend because of probe frames tx in progress.\n");
		goto revert2;
	}
	...
	if (wait_event_timeout(hw_priv->bh_evt_wq,
		...
		pm_printk(XRADIO_DBG_WARN, "Don't suspend because of there are frames not confirm.\n");
		goto revert3;
	}
	...
	if (check_scene_locked(SCENE_SUPER_STANDBY) == 0) {
		if (xradio_poweroff_suspend(hw_priv)) {
			pm_printk(XRADIO_DBG_WARN, "Don't suspend because of xradio_poweroff_suspend failed.\n");
			goto revert3;
		}
		return 0;
	}
	...
	xradio_for_each_vif(hw_priv, priv, i) {
		...
		if (ret) {
			...
			pm_printk(XRADIO_DBG_WARN, "Don't suspend  because of __xradio_wow_suspend failed!\n");
			goto revert3;
		}
	}
	...
	if (xradio_bh_suspend(hw_priv)) {
		pm_printk(XRADIO_DBG_WARN, "Don't suspend because of xradio_bh_suspend failed!\n");
		goto revert4;
	}
	...
	if (atomic_read(&hw_priv->bh_rx)) {
		pm_printk(XRADIO_DBG_WARN, "Don't suspend because of recieved rx event!\n");
		goto revert6;
	}
				...
				struct cfg80211_wowlan *wowlan)
{
	...
	if (work_pending(&priv->join_work)) {
		pm_printk(XRADIO_DBG_WARN, "%s:Do not suspend when join work is scheduled\n", __func__);
		goto revert1;
	}

	/* Set UDP filter */
	wsm_set_udp_port_filter(hw_priv, &xradio_udp_port_filter_on.hdr, priv->if_id);

	/* Set ethernet frame type filter */
	wsm_set_ether_type_filter(hw_priv, &xradio_ether_type_filter_on.hdr, priv->if_id);
	...
	if (!state) {
		pm_printk(XRADIO_DBG_WARN, "%s:Do not suspend alloc xradio_suspend_state failed.\n", __func__);
		goto revert2;
	}
	...
	if (ret) {
		pm_printk(XRADIO_DBG_WARN, "%s:Do not suspend mcast timeout timer_pending failed.\n", __func__);
		goto revert3;
	}
...
revert3:
	...
	xradio_resume_work(hw_priv, &priv->connection_loss_work, state->connection_loss_tmo);
