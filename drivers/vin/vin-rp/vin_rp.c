				    ...
				    void *data, int data_len, void *priv)
{
	...
	if (isp->large_image == 3 && glb_isp[0]) {
		if (isp->id == 0) {
			//ae stat region
			memcpy(&isp->ae_stat_save[0], buf_va + ISP_AE_REG_OFS, ISP_AE_REG_SIZE);
			//af stat region
			memcpy(&isp->af_stat_save[0], buf_va + ISP_AF_REG_OFS, ISP_AF_REG_SIZE);
			//awb stat region
			memcpy(&isp->awb_stat_save[0], buf_va + ISP_AWB_REG_OFS, ISP_AWB_REG_SIZE);
			//hist stat region
			memcpy(&isp->hist_stat_save[0], buf_va + ISP_HIST_REG_OFS, ISP_HIST_REG_SIZE);
			//nr msc table
			memcpy(&isp->nr_msc_load_save[0], buf_va + ISP_LOAD_REG_SIZE + ISP_FE_TBL_SIZE + ISP_RSC_TBL_SIZE + ISP_RSC_TBL_SIZE, ISP_MSC_NR_TBL_SIZE);
			//return ret;
...
int isp_tunning_save_buf(struct isp_dev *isp, char *path, int cfg_id)
{
	void *buf_va;
	const char *buf_name;
	char fdstr[128], tmp[10];
	unsigned offset = 0;
	struct file *file_fd = NULL;
	unsigned short *ushort_ptr = NULL;
	struct tunning_special_ctrl *special_ptr = NULL;
	loff_t pos = 0;

	if (!isp->tunning_buffer) {
		vin_err("tunning rpbuf is null\n");
		return -ENOENT;
	}

	buf_name = rpbuf_buffer_name(isp->tunning_buffer);
	buf_va = rpbuf_buffer_va(isp->tunning_buffer);

	if (!rpbuf_buffer_is_available(isp->tunning_buffer)) {
		vin_err("%s not available\n", buf_name);
		return -EACCES;
	}

	if (cfg_id == ISP_CTRL_GET_ISP_PARAM) {
		/* cfg */
		sprintf(fdstr, "%s/cfg", path);
		file_fd = filp_open(fdstr, O_RDWR | O_CREAT, 0);
		if (IS_ERR(file_fd)) {
			vin_err("open %s failed.\n", fdstr);
			return -1;
		} else {
			pos = 0;
			vfs_write(file_fd, (char *)buf_va, sizeof(struct isp_param_config), &pos);
			filp_close(file_fd, NULL);
			file_fd = NULL;
		}
		/* version */
		offset = sizeof(struct isp_param_config);
		ushort_ptr = (unsigned short *)(buf_va + offset);
		sprintf(tmp, "%d", *ushort_ptr);
		sprintf(fdstr, "%s/version", path);
		file_fd = filp_open(fdstr, O_RDWR | O_CREAT, 0);
		if (IS_ERR(file_fd)) {
			vin_err("open %s failed.\n", fdstr);
			return -1;
			...
			pos = 0;
			vfs_write(file_fd, tmp, strlen(tmp), &pos);
			filp_close(file_fd, NULL);
			file_fd = NULL;
		}
		/* ir, colorspace */
		offset = sizeof(struct isp_param_config) + sizeof(unsigned short);
		special_ptr = (struct tunning_special_ctrl *)(buf_va + offset);
		sprintf(tmp, "%d", special_ptr->ir);
		sprintf(fdstr, "%s/ir", path);
		file_fd = filp_open(fdstr, O_RDWR | O_CREAT, 0);
		if (IS_ERR(file_fd)) {
			vin_err("open %s failed.\n", fdstr);
			return -1;
		...
		sprintf(tmp, "%d", special_ptr->colorspace);
		sprintf(fdstr, "%s/colorspace", path);
		file_fd = filp_open(fdstr, O_RDWR | O_CREAT, 0);
		if (IS_ERR(file_fd)) {
			vin_err("open %s failed.\n", fdstr);
			return -1;
	...
	} else if (cfg_id == ISP_CTRL_GET_LOG) {
		offset = sizeof(struct isp_param_config) + sizeof(unsigned short) + sizeof(struct tunning_special_ctrl);
		/*LOG */
		sprintf(fdstr, "%s/LOG/Info", path);
		file_fd = filp_open(fdstr, O_RDWR | O_CREAT, 0);
		if (IS_ERR(file_fd)) {
			vin_err("open %s failed.\n", fdstr);
			return -1;
			...
			pos = 0;
			vfs_write(file_fd, (char *)(buf_va + offset), sizeof(struct isp_log_cfg), &pos);
			filp_close(file_fd, NULL);
			file_fd = NULL;
		}
	} else if (cfg_id == ISP_CTRL_GET_3A_STAT) {
		/*ae */
		offset = sizeof(struct isp_param_config) + sizeof(unsigned short) + sizeof(struct tunning_special_ctrl) + sizeof(struct isp_log_cfg);
		sprintf(fdstr, "%s/3a_stat/ae", path);
		file_fd = filp_open(fdstr, O_RDWR | O_CREAT, 0);
		if (IS_ERR(file_fd)) {
			vin_err("open %s failed.\n", fdstr);
			return -1;
			...
			pos = 0;
			vfs_write(file_fd, (char *)(buf_va + offset), sizeof(struct isp_ae_stats_s), &pos);
			filp_close(file_fd, NULL);
			file_fd = NULL;
		}
		/*awb */
		offset = sizeof(struct isp_param_config) + sizeof(unsigned short) + sizeof(struct tunning_special_ctrl) + sizeof(struct isp_log_cfg) + sizeof(struct isp_ae_stats_s);
		sprintf(fdstr, "%s/3a_stat/awb", path);
		file_fd = filp_open(fdstr, O_RDWR | O_CREAT, 0);
		if (IS_ERR(file_fd)) {
			vin_err("open %s failed.\n", fdstr);
			return -1;
			...
			pos = 0;
			vfs_write(file_fd, (char *)(buf_va + offset), sizeof(struct isp_awb_stats_s), &pos);
			filp_close(file_fd, NULL);
			file_fd = NULL;
		}
		/* d3d_k */
		offset = sizeof(struct isp_param_config) + sizeof(unsigned short) + sizeof(struct tunning_special_ctrl) + sizeof(struct isp_log_cfg) + sizeof(struct isp_ae_stats_s) + sizeof(struct isp_awb_stats_s);
		sprintf(fdstr, "%s/3a_stat/d3d_k", path);
		file_fd = filp_open(fdstr, O_RDWR | O_CREAT, 0);
		if (IS_ERR(file_fd)) {
			vin_err("open %s failed.\n", fdstr);
			return -1;
			...
			pos = 0;
			vfs_write(file_fd, (char *)(buf_va + offset), sizeof(struct isp_d3d_k_stats_s), &pos);
			filp_close(file_fd, NULL);
			file_fd = NULL;
		}
	} else {
		vin_err("Unknown ctrl. %d\n", cfg_id);
	}
	return 0;
}
...
int isp_tunning_rpbuf_send(struct isp_dev *isp, char *path, int cfg_id)
{
	...
	void *buf_va;
	int buf_len, offset;
	int data_len = ISP_RPBUF_TUNNING_LEN;
	char fdstr[128], tmp;
	struct file *file_fd = NULL;
	struct tunning_special_ctrl *special_ptr = NULL;
	loff_t pos = 0;
	...
	buf_name = rpbuf_buffer_name(isp->tunning_buffer);
	buf_va = rpbuf_buffer_va(isp->tunning_buffer);
	buf_len = rpbuf_buffer_len(isp->tunning_buffer);
	...
	if (cfg_id == ISP_CTRL_SET_ISP_PARAM) {
		/* cfg */
		sprintf(fdstr, "%s/cfg", path);
		...
		if (IS_ERR(file_fd)) {
			...
			pos = 0;
			vfs_read(file_fd, (char *)buf_va, sizeof(struct isp_param_config), &pos);
			filp_close(file_fd, NULL);
			file_fd = NULL;
		}
		/* ir */
		special_ptr = (struct tunning_special_ctrl *)(buf_va + sizeof(struct isp_param_config) + sizeof(unsigned short));
		sprintf(fdstr, "%s/ir", path);
		file_fd = filp_open(fdstr, O_RDONLY, 0);
		if (IS_ERR(file_fd)) {
			vin_err("open %s failed.\n", fdstr);
			...
			pos = 0;
			vfs_read(file_fd, &tmp, sizeof(char), &pos);
			special_ptr->ir = tmp - '0';
			if (special_ptr->ir > 1) {
				vin_err("ir = %d value is error.\n", special_ptr->ir);
				special_ptr->ir = 0;
			}
			filp_close(file_fd, NULL);
			file_fd = NULL;
		...
		if (IS_ERR(file_fd)) {
			...
			pos = 0;
			vfs_read(file_fd, &tmp, sizeof(char), &pos);
			special_ptr->colorspace = tmp - '0';
			if (special_ptr->colorspace > 5) {
				vin_err("colorspace = %d value is error.\n", special_ptr->colorspace);
				special_ptr->colorspace = 0;
			}
			filp_close(file_fd, NULL);
			file_fd = NULL;
		}
	...
	offset = 0;
	ret = rpbuf_transmit_buffer(isp->tunning_buffer, offset, data_len);
	if (ret < 0) {
		vin_err("%s: rpbuf_transmit_buffer failed\n", buf_name);
		return ret;
	}

	vin_log(VIN_LOG_RP, "isp%d send tunning buffer to melis\n", isp->id);
...
#endif
static int isp_tunning_rpbuf_rx_cb(struct rpbuf_buffer *buffer,
				    void *data, int data_len, void *priv)
{
	struct isp_dev *isp = priv;
	/* void *buf_va = rpbuf_buffer_va(buffer); */

	if (data_len > ISP_RPBUF_TUNNING_LEN) {
		vin_err("melis ask for 0x%x data, it more than isp tunning_data  0x%lx\n", data_len, (long unsigned int)ISP_RPBUF_TUNNING_LEN);
		return -EINVAL;
	}

	isp->isp_tunning_update = 1;
...
int isp_tunning_rpbuf_create(struct isp_dev *isp)
{
	...
	if (isp->tunning_buffer) {
		vin_err("rpbuf_alloc_buffer tunning repeated\n");
		return 0;
	}
	...
	tunning_len = roundup(ISP_RPBUF_TUNNING_LEN, 0x1000);
	isp->tunning_buffer = rpbuf_alloc_buffer(isp->controller, tunning_name, tunning_len,
						NULL, &isp_tunning_rpbuf_cbs, (void *)isp);
	if (!isp->tunning_buffer) {
		vin_err("rpbuf_alloc_buffer tunning failed\n");
		return -ENOENT;
	}
	return 0;
}
...
int isp_tunning_rpbuf_destroy(struct isp_dev *isp)
{
	...
	if (!isp->tunning_buffer) {
		vin_err("rpbuf_free_buffer tunning repeated\n");
		return 0;
	}

	ret = rpbuf_free_buffer(isp->tunning_buffer);
	if (ret < 0) {
		vin_err("rpbuf_free_buffer tunning failed\n");
		return ret;
	}

	isp->tunning_buffer = NULL;
	return 0;
}
#endif
						...
						void *priv, u32 src)
{
	...
	case ISP_SET_SYS_RESET:
		...
		break;
	case ISP_SET_MIPI_SWITCH:
		isp_sensor_set_mipi_switch(isp, data);
		break;
	case ISP_DQ_TDM_BUFFER:
		isp_tdm_dqbuffer(isp, data);
		break;
	}
