/*
 *  common_timer.c - Linux kernel modules for sensortek stk83xx
...
static timer_manager *parser_timer(struct hrtimer *timer)
{
	...
	for (timer_idx = 0; timer_idx < MAX_LINUX_TIMER_MANAGER_NUM;
	     timer_idx++) {
...
int register_timer(stk_timer_info *t_info)
{
	...
	for (i = 0; i < MAX_LINUX_TIMER_MANAGER_NUM; i++) {
		if (!linux_timer_mgr[i].timer_info) {
			linux_timer_mgr[i].timer_info = t_info;
			timer_idx = i;
			break;
		} else {
			if (linux_timer_mgr[i].timer_info == t_info) {
				//already register
				if (linux_timer_mgr[i]
					    .timer_info->change_interval_time) {
					linux_timer_mgr[i]
						.timer_info
						->change_interval_time = 0;
					printk(KERN_ERR
					       "%s: chang interval time\n",
					       __func__);
					switch (linux_timer_mgr[i]
							.timer_info
							->timer_unit) {
					case N_SECOND:
						linux_timer_mgr[i]
							.timer_interval = ns_to_ktime(
							linux_timer_mgr[i]
								.timer_info
								->interval_time);
						break;
	...
	printk(KERN_ERR "%s: register timer name %s\n", __func__,
	       linux_timer_mgr[timer_idx].timer_info->wq_name);
	linux_timer_mgr[timer_idx].stk_wq = create_singlethread_workqueue(
		linux_timer_mgr[timer_idx].timer_info->wq_name);
	INIT_WORK(&linux_timer_mgr[timer_idx].stk_work, timer_callback);
	hrtimer_init(&linux_timer_mgr[timer_idx].stk_hrtimer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);

	switch (linux_timer_mgr[timer_idx].timer_info->timer_unit) {
	case N_SECOND:
		linux_timer_mgr[timer_idx].timer_interval = ns_to_ktime(
			linux_timer_mgr[timer_idx].timer_info->interval_time);
		break;
	...
	for (timer_idx = 0; timer_idx < MAX_LINUX_TIMER_MANAGER_NUM;
		...
		if (linux_timer_mgr[timer_idx].timer_info == t_info) {
			if (linux_timer_mgr[timer_idx].timer_info->is_exist) {
				if (!linux_timer_mgr[timer_idx]
					     .timer_info->is_active) {
					hrtimer_start(
						&linux_timer_mgr[timer_idx]
							 .stk_hrtimer,
						linux_timer_mgr[timer_idx]
							.timer_interval,
						HRTIMER_MODE_REL);
					...
					printk(KERN_ERR
					       "%s: start timer name %s\n",
					       __func__,
					       linux_timer_mgr[timer_idx]
						       .timer_info->wq_name);
				} else {
					printk(KERN_INFO
					       "%s: %s was already running\n",
					       __func__,
					       linux_timer_mgr[timer_idx]
						       .timer_info->wq_name);
				}
