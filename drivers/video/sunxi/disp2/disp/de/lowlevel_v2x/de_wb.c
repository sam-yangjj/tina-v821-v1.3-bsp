...
static enum de_rtwb_mode wb_mode = TIMING_FROM_TCON;
...
s32 wb_ebios_set_para(u32 sel, struct disp_capture_config *cfg)
{
	...
	if (((out_fmt == DISP_FORMAT_YUV420_P)
		...
		return -1;
	/* gctrl */
	wb_dev[sel]->gctrl.dwval |= (0x20000000 | (in_port << 16) | (wb_mode << 5));
	/* sftm */
...
u32 wb_query_gctrl(u32 sel)
{
	return wb_dev[sel]->gctrl.dwval;
}
...
s32 de_wb_irq_query_and_clear(u32 wb, enum wb_irq_state irq_state)
{
	...
	reg_val = wb_ebios_queryint(wb);
	state = reg_val & irq_state & WB_IRQ_STATE_MASK;

	reg_val &= ~WB_IRQ_STATE_MASK;
	reg_val |= state;
	wb_dev[wb]->status.dwval = reg_val;

	return state;
}

/* maybe the same enable bit as write back */
s32 de_top_start_rtwb(u32 wb_id, u32 en)
{
	return wb_ebios_writeback_enable(wb_id, en);
}

s32 de_wb_set_rtwb_mode(u32 wb_id, enum de_rtwb_mode mode)
{
	wb_mode = mode;
	return 0;
}
