/*
 *  stk83xx_qualcomm.c - Linux kernel modules for sensortek stk83xx
 *  accelerometer sensor (Qualcomm)
 *
 ...
 * @return: ssize_t
 */
static ssize_t stk_value_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	stk83xx_wrapper *stk_wrapper = dev_get_drvdata(dev);
	stk_data *stk = &stk_wrapper->stk;
 ...
 * @return: ssize_t
 */
static ssize_t stk_delay_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	stk83xx_wrapper *stk_wrapper = dev_get_drvdata(dev);
	stk_data *stk = &stk_wrapper->stk;
	return scnprintf(buf, PAGE_SIZE, "%d\n", stk_get_delay(stk) * 1000);
}
				...
				size_t count)
{
	...
	r_offset[2] = (u8)data;
	STK_ACC_LOG("offset=0x%X, 0x%X, 0x%X", r_offset[0], r_offset[1],
		    r_offset[2]);
 ...
 * @return: ssize_t
 */
static ssize_t stk_fifo_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	stk83xx_wrapper *stk_wrapper = dev_get_drvdata(dev);
	stk_data *stk = &stk_wrapper->stk;
 ...
 * @return: ssize_t
 */
static ssize_t stk_fifo_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	stk83xx_wrapper *stk_wrapper = dev_get_drvdata(dev);
	stk_data *stk = &stk_wrapper->stk;
 ...
 * @return: ssize_t
 */
static ssize_t stk_range_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	stk83xx_wrapper *stk_wrapper = dev_get_drvdata(dev);
	stk_data *stk = &stk_wrapper->stk;
			       ...
			       size_t count)
{
	...
	if (stk->pid != STK8327_ID) {
		if (data == 16) {
			STK_ACC_LOG(
				" This chip not support 16G,auto switch to 8G");
			data = 8;
		}
	}
 ...
 * @return: ssize_t
 */
static ssize_t stk_cali_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	stk83xx_wrapper *stk_wrapper = dev_get_drvdata(dev);
	stk_data *stk = &stk_wrapper->stk;
 ...
 * @return: ssize_t
 */
static ssize_t stk_cali_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	stk83xx_wrapper *stk_wrapper = dev_get_drvdata(dev);
	stk_data *stk = &stk_wrapper->stk;
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	if (len) {
		STK_ACC_LOG("FIR count=%2d, idx=%2d", stk->data_log.count,
			    stk->data_log.idx);
		STK_ACC_LOG("sum = [\t%d \t%d \t%d]", stk->data_log.sum[0],
			    stk->data_log.sum[1], stk->data_log.sum[2]);
		STK_ACC_LOG("avg = [\t%d \t%d \t%d]",
			    stk->data_log.sum[0] / len,
			    stk->data_log.sum[1] / len,
			    stk->data_log.sum[2] / len);
	}
...
static DEVICE_ATTR(accel_enable, 0664, stk_enable_show, stk_enable_store);
static DEVICE_ATTR(value, 0444, stk_value_show, NULL);
static DEVICE_ATTR(accel_delay, 0664, stk_delay_show, stk_delay_store);
...
static struct attribute *stk_attribute_accel[] = { &dev_attr_accel_enable.attr,
						   &dev_attr_value.attr,
...
static int stk_input_setup(stk83xx_wrapper *stk_wrapper)
{
	...
	stk_wrapper->input_dev_accel->name = "stk83xx";
	stk_wrapper->input_dev_accel->id.bustype = BUS_I2C;
	input_set_capability(stk_wrapper->input_dev_accel, EV_ABS, ABS_MISC);
	input_set_abs_params(stk_wrapper->input_dev_accel, ABS_X, 32767, -32768,
			     0, 0);
	input_set_abs_params(stk_wrapper->input_dev_accel, ABS_Y, 32767, -32768,
			     0, 0);
	input_set_abs_params(stk_wrapper->input_dev_accel, ABS_Z, 32767, -32768,
			     0, 0);
	...
	if (err) {
		STK_ACC_ERR("Unable to register input device: %s",
			    stk_wrapper->input_dev_accel->name);
	...
	stk_wrapper->input_dev_amd->dev.parent =
		&stk_wrapper->i2c_mgr->client->dev;
...
static int stk_init_qualcomm(stk83xx_wrapper *stk_wrapper)
{
	...
	err = sysfs_create_group(&stk_wrapper->input_dev_accel->dev.kobj,
	...
	kobject_uevent(&stk_wrapper->input_dev_accel->dev.kobj, KOBJ_CHANGE);
	kobject_uevent(&stk_wrapper->input_dev_accel->dev.kobj, KOBJ_ONLINE);
	kobject_uevent(&stk_wrapper->input_dev_accel->dev.kobj, KOBJ_ADD);
	return 0;
...
static void stk_exit_qualcomm(stk83xx_wrapper *stk_wrapper)
{
	/*
    sysfs_remove_group(&stk_wrapper->i2c_mgr.client->dev.kobj,
                             &stk_attribute_accel_group);
    */
	sysfs_remove_group(&stk_wrapper->input_dev_accel->dev.kobj,
			   &stk_attribute_accel_group);
#ifdef STK_AMD
	input_unregister_device(stk_wrapper->input_dev_amd);
	input_free_device(stk_wrapper->input_dev_amd);
 ...
 *          others: Fail
 */
static int stk_parse_dt(struct device *dev, struct stk83xx_platform_data *pdata)
{
	...
	pdata->interrupt_int1_pin =
		of_get_named_gpio_flags(np, "stk83xx,irq-gpio", 0, &int_flags);
	...
	if (client->dev.of_node) {
		...
		stk_platdata =
			devm_kzalloc(&client->dev,
				     sizeof(struct stk83xx_platform_data),
				     GFP_KERNEL);
...
void read_data_callback(struct stk_data *stk)
{
	...
	for (ii = 0; ii < 3; ii++) {
		coor_trans[0] +=
			stk->xyz[ii] * coordinate_trans[stk->direction][0][ii];
		coor_trans[1] +=
			stk->xyz[ii] * coordinate_trans[stk->direction][1][ii];
		coor_trans[2] +=
			stk->xyz[ii] * coordinate_trans[stk->direction][2][ii];
	}
...
void stk_report_accel_data(struct stk_data *stk)
{
	...
	input_report_abs(stk_wrapper->input_dev_accel, ABS_X, stk->xyz[0]);
	input_report_abs(stk_wrapper->input_dev_accel, ABS_Y, stk->xyz[1]);
	input_report_abs(stk_wrapper->input_dev_accel, ABS_Z, stk->xyz[2]);
	input_report_abs(stk_wrapper->input_dev_accel, ABS_MISC,
			 stk->sensitivity);
...
int stk_i2c_probe(struct i2c_client *client, struct common_function *common_fn)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		...
		STK_ACC_ERR("i2c_check_functionality error, functionality=0x%x",
			    err);
		return -EIO;
	}
	...
	stk->gpio_info.trig_type = TRIGGER_RISING;
