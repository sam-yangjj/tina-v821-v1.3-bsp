...
int dw_mc_set_clk(dw_mc_clk_e index, u8 state)
{
	int i = 0;

	for (i = 0; i < ARRAY_SIZE(mc_clk); i++) {
		if (mc_clk[i].index == index) {
			dw_write_mask(MC_CLKDIS, mc_clk[i].mask, !state);
			return 0;
		}
	}
	hdmi_err("dw mc set %d clock %s failed\n",
			index, state ? "enable" : "disable");
	return -1;
}

u8 dw_mc_get_clk(dw_mc_clk_e index)
{
	int i = 0;
	u8 state = DW_HDMI_DISABLE;

	for (i = 0; i < ARRAY_SIZE(mc_clk); i++) {
		if (mc_clk[i].index == index) {
			state = dw_read_mask(MC_CLKDIS, mc_clk[i].mask) ? DW_HDMI_DISABLE : DW_HDMI_ENABLE;
			return state;
		}
	}
	hdmi_err("dw mc get %d clock state failed\n", index);
