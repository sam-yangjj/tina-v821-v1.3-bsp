...
static int get_vsync(struct device_node *np, const char *name,
			struct sensor_list *sc)
{
	return get_gpio_info(np, name, &sc->gpio[VSYNC]);
}

static int get_mipi_sw(struct device_node *np, const char *name,
			struct sensor_list *sc)
{
	return get_gpio_info(np, name, &sc->gpio[MIPI_SW]);
}
#endif
