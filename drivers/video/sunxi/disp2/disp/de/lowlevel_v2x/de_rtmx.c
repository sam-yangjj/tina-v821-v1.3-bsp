...
int de_rtmx_init(unsigned int sel, uintptr_t reg_base)
{
	...
	if (memory == NULL) {
		DE_WARN("malloc rtmx global memory fail! size=0x%x\n",
	...
	for (j = 0; j < vi_chno; j++) {
		...
		if (memory == NULL) {
			DE_WARN("malloc video overlay memory fail! size=0x%x\n",
			      (unsigned int)sizeof(struct __vi_ovl_reg_t));
			return -1;
		}
		DE_INFO("malloc memory ok(0x%p) for video channel\n",
		    (void *)memory);
		...
		memory = kmalloc(sizeof(struct __ovl_palette_reg_t),
			    GFP_KERNEL | __GFP_ZERO);
		if (memory == NULL) {
			DE_WARN("malloc vi_palette overlay memory fail! size=0x%x\n",
	...
	for (j = 0; j < ui_chno; j++) {
		...
		memory =
			...
			GFP_KERNEL | __GFP_ZERO);
		if (memory == NULL) {
			DE_WARN("malloc ui overlay memory fail! size=0x%x\n",
		...
		memory =
			kmalloc(sizeof(struct __ovl_palette_reg_t),
			    GFP_KERNEL | __GFP_ZERO);
		if (memory == NULL) {
			DE_WARN("malloc ui_palette overlay memory fail! size=0x%x\n",
			    (unsigned int)sizeof(struct __ovl_palette_reg_t));
			return -1;
		}
	...
	for (j = 0; j < vi_chno; j++) {
		memory =
			...
			GFP_KERNEL | __GFP_ZERO);
		if (memory == NULL) {
			DE_WARN("malloc lbc overlay memory fail! size=0x%x\n",
	...
	if (memory == NULL) {
		DE_WARN("malloc blending memory fail! size=0x%x\n",
				 ...
				 void *base)
{
	...
	DE_INFO("sel=0x%x, overlayer %d, addr=0x%p\n", sel, chno, base);
...
static int de_rtmx_ovl_check_lay_cfg(unsigned int sel, unsigned int chno, unsigned int layno,
					struct __lay_para_t *cfg)
{
	int i;
	int laynum = de_feat_get_num_layers_by_chn(sel, chno);
	int vi_chno = de_feat_get_num_vi_chns(sel);
	int lbc_en, fcolor_en, ulay_en, vlay_en, fmt, alpha;
	int cfcolor_en, culay_en, cvlay_en, cfmt, calpha;
	int not_enable_layer = 0;

	if (!cfg->fcolor_en && !cfg->en)
		return 0;

	/* get check layer info */
	if (chno >= vi_chno) {
		cvlay_en = 0;
		culay_en = de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].lay_attr.bits.lay_en;
		cfcolor_en = de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].lay_attr.bits.lay_fcolor_en;
		cfmt =  de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].lay_attr.bits.lay_fmt;
		calpha =  de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].lay_attr.bits.lay_alpha;
	} else {
		cfcolor_en = de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[layno].lay_attr.bits.lay_fcolor_en;
		if (de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[layno].lay_attr.bits.lay_en) {
			if (de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[layno].lay_attr.bits.ui_sel) {
				cvlay_en = 0;
				culay_en = 1;
			} else {
				cvlay_en = 1;
		...
		cfmt = de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[layno].lay_attr.bits.lay_fmt;
		calpha = de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[layno].lay_attr.bits.alpha;
	}

	/* traverse the current channel layer */
	for (i = 0; i < laynum; i++) {
		if (i == layno)
			continue;
		...
		if (chno >= vi_chno) {
			lbc_en = 0;
			vlay_en = 0;
			ulay_en = de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[i].lay_attr.bits.lay_en;
			fcolor_en = de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[i].lay_attr.bits.lay_fcolor_en;
			fmt =  de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[i].lay_attr.bits.lay_fmt;
			alpha =  de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[i].lay_attr.bits.lay_alpha;
		} else {
#if defined(SUPPORT_LBC)
			lbc_en = de200_rtmx[sel].lbc_ovl[chno]->lbc_ctl.bits.lbc_en;
#else
			lbc_en = 0;
#endif
			fcolor_en = de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[i].lay_attr.bits.lay_fcolor_en;
			if (de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[i].lay_attr.bits.lay_en) {
				if (de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[i].lay_attr.bits.ui_sel) {
					vlay_en = 0;
					ulay_en = 1;
				} else {
					vlay_en = 1;
			...
			fmt = de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[i].lay_attr.bits.lay_fmt;
			alpha = de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[i].lay_attr.bits.alpha;
		}

		if (!lbc_en && !vlay_en && !ulay_en && !fcolor_en) {
			not_enable_layer++;
			continue;
		}

		if (lbc_en) {
			DE_WARN("maybe something wrong, ovl and lbc are enabled at the same time");
			goto disable_lay;
		}

		if (((cfcolor_en || culay_en) && vlay_en) ||
		    (cvlay_en && (fcolor_en || ulay_en))) {
			DE_WARN("cannot set chn to ui & video formats at the same time");
			goto disable_lay;
		}

		/* ui layer overlay */
		if ((cfcolor_en || culay_en) && (fcolor_en || ulay_en)) {
			if (calpha != alpha) {
				DE_WARN("cannot set chn lay to two alpha value at the same time");
				continue;
				/* goto disable_lay; */
			}
		}
		...
		if (cvlay_en && vlay_en) {
			if (cfmt != fmt) {
				DE_WARN("cannot set vchn to two formats at the same time");
				goto disable_lay;
			}
	...
	/* only enabled one layer */
	if (not_enable_layer == laynum - 1) {
		if (chno >= vi_chno) {
			de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[0].lay_attr.
			    bits.lay_alpha = cfg->alpha;
		} else {
			de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[0].lay_attr.bits.
				alpha = cfg->alpha;
			de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[0].lay_attr.bits.ui_sel =
				de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[layno].lay_attr.bits.ui_sel;
			de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[0].lay_attr.bits.
					lay_fmt = cfmt;
		}
	}
...
disable_lay:
	if (chno >= vi_chno)
		de200_rtmx[sel].ui_ovl[chno - vi_chno]->ui_lay_cfg[layno].lay_attr.bits.lay_en = 0;
	else
		de200_rtmx[sel].vi_ovl[chno]->vi_lay_cfg[layno].lay_attr.bits.lay_en = 0;
	return -1;
}
/**
			...
			struct __lay_para_t *cfg)
{
	...
	if (lbc_support == 0 && lbc_en == 1) {
		DE_WARN("rtmx%d chno%d lay%d does not supported LBC\n", sel, chno, layno);
		return 0;
	}
	if (lbc_en == 1) {
		/* disable overlay */
		...
		de200_rtmx[sel].lbc_ovl[chno]->lbc_v_size.bits.lay_height = cfg->layer.h - 1;
		...
		de200_rtmx[sel].lbc_ovl[chno]->lbc_crop_size.bits.height_crop = cfg->lbc_crop.h - 1;
	...
	if (cfg->en || cfg->fcolor_en) {
		/*
		 * The alpha and video_ui_sel registers of the four layers are shared with layer 0.
		 * The four layers can only be configured as video or UI at the same time,
		 * and these bits are shared among the four layers.
		 * When configuring the other three layers, layer 0 should also be configured.
		 */
		de_rtmx_ovl_check_lay_cfg(sel, chno, layno, cfg);
	}
...
int de_rtmx_set_route(unsigned int sel, unsigned char pno, unsigned int zoder)
{
	...
	route =
	    ...
	    & (~(0xf << (pno << 2)));
	DE_INFO("sel%d, pno%d, zorder%d, route:0x%x\n", sel, pno, zoder, route);
	de200_rtmx[sel].bld_ctl->bld_route_ctl.dwval =
	    route | (zoder << (pno << 2));
	DE_INFO("addr=0x%p, reg=0x%x\n", &de200_rtmx[sel].bld_ctl->bld_route_ctl,
	      de200_rtmx[sel].bld_ctl->bld_route_ctl.dwval);
	...
	struct de_rect *bld_rect, unsigned int *ovlw, unsigned int *ovlh)
{
	...
	for (i = 0; i < laynum; i++) {
		if (lay_en[i]) {
			...
			layer[i].w = crop[i].w;
			layer[i].h = crop[i].h;
			DE_INFO("layernum=%d, i=%d\n", laynum, i);
			DE_INFO("crop=<%d,%d,%d,%d>\n", crop[i].x, crop[i].y,
			      crop[i].w, crop[i].h);
			DE_INFO("layer=<%d,%d,%d,%d>\n", layer[i].x, layer[i].y,
			      layer[i].w, layer[i].h);
		}
	}
	...
	for (i = 0; i < laynum; i++) {
		if (lay_en[i]) {
			...
			DE_INFO("ovl_rect=<%d,%d,%d,%d>\n", RCOVL.x, RCOVL.y,
			      RCOVL.w, RCOVL.h);
			DE_INFO("bld_rect=<%d,%d,%d,%d>\n", RCBLD.x, RCBLD.y,
			      RCBLD.w, RCBLD.h);
		}
	}
