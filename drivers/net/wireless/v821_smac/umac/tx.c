...
//	struct ieee80211_key *key;
	...
	if (tx->sta && (rcu_dereference(tx->sta->ptk[tx->sta->ptk_idx])))
...
static struct txq_info *mac80211_next_txqi;
...
struct ieee80211_txq *mac80211_next_txq(struct ieee80211_hw *hw, u8 ac)
{
 ...
 begin:
	mac80211_next_txqi = list_first_entry_or_null(&local->active_txqs[ac],
					struct txq_info,
					schedule_order);
	if (!mac80211_next_txqi)
		goto out;

	if (mac80211_next_txqi->txq.sta) {
		struct sta_info *sta = container_of(mac80211_next_txqi->txq.sta,
						struct sta_info, sta);

		if (sta->airtime[mac80211_next_txqi->txq.ac].deficit < 0) {
			sta->airtime[mac80211_next_txqi->txq.ac].deficit +=
				sta->airtime_weight;
			list_move_tail(&mac80211_next_txqi->schedule_order,
				       &local->active_txqs[mac80211_next_txqi->txq.ac]);
			goto begin;
		}
	}


	if (mac80211_next_txqi->schedule_round == local->schedule_round[ac])
		goto out;

	list_del_init(&mac80211_next_txqi->schedule_order);
	mac80211_next_txqi->schedule_round = local->schedule_round[ac];
	ret = &mac80211_next_txqi->txq;
