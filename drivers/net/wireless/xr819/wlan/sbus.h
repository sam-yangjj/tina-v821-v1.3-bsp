...
struct sbus_ops {
	int (*sbus_data_read)(struct sbus_priv *self, unsigned int addr, void *dst, int count);
	...
	int (*irq_subscribe)(struct sbus_priv *self, sbus_irq_handler handler, void *priv);
...
static inline unsigned xr_sdio_blksize_align(unsigned size)
{
	return (size + (SDIO_BLOCK_SIZE - 1)) & (~(SDIO_BLOCK_SIZE - 1));
}
#endif
...
static inline unsigned xr_sdio_align_broken_byte512(unsigned size)
{
	...
	return (align_size < 512 ? align_size : xr_sdio_blksize_align(align_size));
}

/*sbus init functions*/
struct device *sbus_sdio_init(struct sbus_ops  **sdio_ops, struct sbus_priv **sdio_priv);
