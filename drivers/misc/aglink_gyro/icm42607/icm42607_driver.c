...
int _inv_imu_get_frame_count(inv_imu_device_t *s, uint16_t *frame_count)
{
	int status = 0;
...
int inv_imu_get_frame_count(inv_imu_device_t *s)
{
	int      status = 0;
	uint8_t  int_status;
	...
	status |= _inv_imu_get_frame_count(s, &packet_count);
...
int inv_imu_get_data_from_fifo(inv_imu_device_t *s, int packet_count)
{
	int      status = 0;
...
int inv_imu_configure_fifo_wm(inv_imu_device_t *s, u16 fifo_wm)
{
	int status = 0;
	u8 low_wm, upp_wm;
	/* Configure FIFO WM so that INT is triggered for each packet */
	low_wm = fifo_wm & 0xff;
	upp_wm = (fifo_wm >> 8) & 0xf;
	status |= inv_imu_write_reg(s, FIFO_CONFIG2, 1, &low_wm);
	status |= inv_imu_write_reg(s, FIFO_CONFIG3, 1, &upp_wm);
	return status;
}
...
int inv_imu_config_apex(struct inv_imu_device *s, SENSOR_APEX_CONFIG_t apex_sel)
{
	int rc = 0;
	uint8_t data;
	rc |= inv_imu_read_reg(s, SENSOR_CONFIG3_MREG1, 1, &data);
	data &= (uint8_t)~SENSOR_CONFIG3_APEX_DISABLE_MASK;
	data |= (uint8_t)apex_sel;
	rc |= inv_imu_write_reg(s, SENSOR_CONFIG3_MREG1, 1, &data);
	return rc;
}
