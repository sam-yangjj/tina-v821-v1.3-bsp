...
#define CLEAR_PENDING_DELAY_TIME    10
/*
...
static inline void gprcm_reg_write(struct sunxi_rtc_dev *chip, u32 reg, u32 val)
{
	writel(val, chip->gprcm_base + reg);
}
static inline u32 gprcm_reg_read(struct sunxi_rtc_dev *chip, u32 reg)
{
	return readl(chip->gprcm_base + reg);
}
...
static int sunxi_rtc_gettime(struct device *dev, struct rtc_time *tm)
{
	...
	if (chip->gprcm_base) {
		int gprcm_status;
		val = gprcm_reg_read(chip, GPRCM_LPCLK_CTRL_REG);
		gprcm_status = gprcm_reg_read(chip, GPRCM_LPCLK_STATUS_REG);
		if ((val & GPRCM_RCCAL_32K_SEL) || ((val & GPRCM_XO32K_EN) == 0) \
			|| ((val & GPRCM_XO32K_CLK_SEL) == 0) || ((gprcm_status & GPRCM_XO32K_RDY) == 0))
...
static irqreturn_t sunxi_rtc_alarm0_isr(int irq, void *id)
{
	...
	delay_time = chip->data->clear_pending_time;

	val = rtc_reg_read(chip, ALARM0_IRQ_STA_REG);
	if ((val & ALARM0_IRQ_PEND) && alarm0_is_enabled(chip)) {
		val |= ALARM0_IRQ_PEND;
		rtc_reg_write(chip, ALARM0_IRQ_STA_REG, val);  /* clear the interrupt */

		if (delay_time) {
			do {
				udelay(CLEAR_PENDING_DELAY_TIME);
				delay_time = ((delay_time >= CLEAR_PENDING_DELAY_TIME) ? (delay_time - CLEAR_PENDING_DELAY_TIME) : 0);
				val = rtc_reg_read(chip, ALARM0_IRQ_STA_REG);
			} while ((val & ALARM0_IRQ_PEND) && delay_time);

			if (!delay_time) {
				val = rtc_reg_read(chip, ALARM0_IRQ_STA_REG);
				if (val & ALARM0_IRQ_PEND) {
					sunxi_err(chip->dev, "clear interrupt timeout, Interruption will trigger multiple times\n");
					return IRQ_HANDLED;
				}
			}
		}
...
static int rtc_class_access_check(void)
{
	extern struct class *rtc_class;  //check rtc_class
	if (rtc_class == NULL) {
		return -EPROBE_DEFER;
	}
	return 0;
...
static int sunxi_rtc_probe(struct platform_device *pdev)
{
	...
	/* Release the reset */
	chip->reset = devm_reset_control_get_optional(dev, NULL);
	if (IS_ERR(chip->reset)) { /*chip->reset == err */
	...
	/* Enable BUS clock */
	chip->clk_bus = devm_clk_get_optional(dev, "r-ahb-rtc");
	...
	/* Check if the gprcm base is present. If so,
	 * the address needs to be mapped for use in enabling the external 32kHz oscillator.
	 */
	chip->gprcm_base = map_gprcm_base(dev);
	...
	err = rtc_class_access_check();
	if (err) {
		sunxi_err(dev, "rtc_class_access_check is failed\n");
		goto err6;
	}
...
#if IS_ENABLED(CONFIG_AW_RTC_FREE_RUNNING_COUNTER)
	err = rtc_frun_counter_init(chip);
	if (err)
		sunxi_warn(dev, "clocksource not work: rtc free running counter\n");
#endif
...
{
	int err;
	sunxi_info(NULL, "Sunxi RTC Driver: Version 1.3.4\n");
...
core_initcall_sync(sunxi_rtc_init);
#else
