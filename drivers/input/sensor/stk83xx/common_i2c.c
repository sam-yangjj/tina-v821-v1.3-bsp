/*
 *  common_i2c.c - Linux kernel modules for sensortek w11xx
...
int i2c_reg_write_block(int i2c_idx, unsigned int reg, void *val, int length)
{
	...
	if (addr_type == ADDR_8BIT) {
		unsigned char reg_ = (unsigned char)(reg & 0xFF);
		error = i2c_smbus_write_i2c_block_data(_pi2c->client, reg_,
						       length, val);
	} else if (addr_type == ADDR_16BIT) {
		int i = 0;
		unsigned char *buffer_inverse;
		struct i2c_msg msgs;
		buffer_inverse = kzalloc((sizeof(unsigned char) * (length + 2)),
					 GFP_KERNEL);
	...
	if (error < 0) {
		dev_err(&_pi2c->client->dev, "%s: failed to write reg:0x%x\n",
			__func__, reg);
	}
...
int i2c_reg_read_modify_write(int i2c_idx, unsigned int reg, unsigned char val,
			      unsigned char mask)
{
	uint8_t rw_buffer = 0;
	int error = 0;
...
int i2c_reg_read_block(int i2c_idx, unsigned int reg, int count, void *buf)
{
	...
	if (addr_type == ADDR_8BIT) {
		...
		struct i2c_msg msgs[2] = { { .addr = _pi2c->client->addr,
					     .flags = 0,
					     .len = 2,
					     .buf = (u8 *)&reg_inverse },
		...
		if (2 == ret) {
			...
			for (i = 0; i < read_length; i++) {
				*(u8 *)((u8 *)buf + i) =
					((buffer_inverse[read_length - 1 - i]));
			}
		}
	}
