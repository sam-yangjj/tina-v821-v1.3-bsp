...
int wsm_hwt_cmd(struct xradio_common *hw_priv,
		void *arg,
		size_t arg_size);
...
static int xradio_status_show_priv(struct seq_file *seq, void *v)
{
	...
	if (priv->join_status == XRADIO_JOIN_STATUS_STA) {
		...
		seq_printf(seq, "Preamble:   %s\n",
			   xradio_debug_preamble[priv->association_mode.preambleType]);
	...
	xradio_debug_print_map(seq, priv, "Asleep map: ", priv->sta_asleep_mask);
	...
	for (i = 0; i < MAX_STA_IN_AP_MODE; ++i) {
		if (priv->link_id_db[i].status) {
			seq_printf(seq, "Link %d:     %s, %pM\n",
				   i + 1, xradio_debug_link_id[priv->link_id_db[i].status],
...
#if defined(CONFIG_XRADIO_USE_EXTENSIONS)
	if (SYS_WARN(!debugfs_create_file("hang", S_IWUSR, d->debugfs_phy, priv, &fops_hang)))
		goto err;
#endif
