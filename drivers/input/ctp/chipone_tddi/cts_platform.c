...
int cts_plat_reset_device(struct cts_platform_data *pdata)
{
    ...
    /* !!!can not be modified */
    /* !!!can not be modified */
    cts_info("cts_reset_device debug func=%s line=%d", __func__, __LINE__);
        ...
        struct device_node *dev_node)
{
    ...
    ret = of_property_read_u32(dev_node, CFG_CTS_OF_REVERT_X_NAME, &pdata->revert_x);
    if (ret) {
        cts_warn("Parse revert X from dt failed %d", ret);
        pdata->revert_x = 0;
    }

    ret = of_property_read_u32(dev_node, CFG_CTS_OF_REVERT_Y_NAME, &pdata->revert_y);
    if (ret) {
        cts_warn("Parse revert Y from dt failed %d", ret);
        pdata->revert_y = 0;
    }

    ret = of_property_read_u32(dev_node, CFG_CTS_OF_SWAP_X2Y_NAME, &pdata->swap_x2y);
    if (ret) {
        cts_warn("Parse swap-x2y from dt failed %d", ret);
        pdata->swap_x2y = 0;
    }
    cts_info("revert x %d revert y %d swap-x2y %d", pdata->revert_x, pdata->revert_y, pdata->swap_x2y);

    if (pdata->swap_x2y) {
        CTS_SWAP(pdata->res_x, pdata->res_y);
    }

    ret = of_property_read_string(dev_node, "chipone,firmware_name", &pdata->firmware_name);
    if (ret) {
        pdata->firmware_name = CFG_CTS_FIRMWARE_FILENAME;
    }
    cts_info("update firmware from %s", (char *)pdata->firmware_name);
...
int cts_deinit_platform_data(struct cts_platform_data *pdata)
{
    ...
    input_unregister_device(pdata->pen_input_dev);
    return 0;
}
