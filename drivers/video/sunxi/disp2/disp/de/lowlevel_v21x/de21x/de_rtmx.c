...
s32 de_rtmx_init(struct disp_bsp_init_para *para)
{
	...
	for (disp = 0; disp < disp_num; ++disp) {
		...
		de_lbc_init(disp, ctx->de_reg_base);
		...
		de_csc2_init(disp, ctx->de_reg_base);
	...
	struct disp_layer_config_data **const pdata, u32 layer_num)
{
	...
	de_vsu_calc_scale_para(disp, chn, fix_size_result,
		chn_info->px_fmt_space, chn_info->yuv_sampling,
		&(chn_info->scn_win),
...
static int de_rgb_pq_set_csc2(u32 disp, u32 chn, u32 csc_id,
			      struct de_csc_info *in_info,
			      struct de_csc_info *out_info)
{
	struct de_csc_info csc_out = {0};
	struct disp_csc_config csc_config = {0};
	...
	if (in_info->px_fmt_space == DE_FORMAT_SPACE_RGB) {
		...
		csc_config.disp = disp;
		csc_config.chn = chn;
		csc_config.csc_id = csc_id;
		csc_config.hw_type = CSC_TYPE_CHN;
		csc_config.flags = CSC_DIRTY_CS_CONVERT;
		csc_config.in_fmt = in_info->px_fmt_space;
		csc_config.in_mode = in_info->color_space;
		csc_config.in_color_range = in_info->color_range;
		csc_config.out_fmt = csc_out.px_fmt_space;
		csc_config.out_mode = csc_out.color_space;
		csc_config.out_color_range = csc_out.color_range;
		de_csc2_apply(&csc_config);
		memcpy(out_info, &csc_out, sizeof(csc_out));
	} else if (in_info->px_fmt_space == DE_FORMAT_SPACE_YUV) {
		/* csc bypass or disable */
		de_csc2_enable(disp, chn, csc_id, CSC_TYPE_CHN, 0);
	...
	struct de_rtmx_context *ctx, struct de_chn_info *chn_info)
{
	...
	if (de_feat_is_support_vep_by_chn(disp, chn)) {
		de_rgb_pq_set_csc2(disp, chn, 1, &icsc_info, &ocsc_info);
	...
	/* fcm_csc */
	if (de_feat_is_support_fcm_by_chn(disp, chn)) {
	...
	csc_config.hw_type = CSC_TYPE_CHN;
	csc_config.flags = CSC_DIRTY_CS_CONVERT;
	csc_config.in_fmt = chn_info->px_fmt_space;
	csc_config.in_mode = chn_info->color_space;
	csc_config.in_color_range = chn_info->color_range;
	csc_config.in_eotf = chn_info->eotf;
	csc_config.out_fmt = ctx->output.px_fmt_space;
	csc_config.out_mode = ctx->output.color_space;
	csc_config.out_color_range = ctx->output.color_range;
	csc_config.out_eotf = ctx->output.eotf;
	de_csc2_apply(&csc_config);
	...
	struct disp_layer_config_data *data, u32 layer_num)
{
	...
	if (!enable) {
		...
		de_csc2_enable(disp, chn, 0, CSC_TYPE_CHN, 0);
		return 0;
	}
	...
	if (chn_info->fbd_en || chn_info->atw_en || chn_info->tfbd_en || chn_info->lbc_en) {
		...
		if (chn_info->tfbd_en) {
			...
			if (de_feat_is_support_lbc_by_layer(disp, chn, 0))
				de_lbc_disable(disp, chn);
		} else if (chn_info->lbc_en) {
			de_lbc_apply_lay(disp, chn, pdata, pdata_num);
			if (de_feat_is_support_fbd_by_layer(disp, chn, 0))
				de_fbd_atw_disable(disp, chn);
			if (de_feat_is_support_tfbd_by_layer(disp, chn, 0))
				de_tfbd_disable(disp, chn);
