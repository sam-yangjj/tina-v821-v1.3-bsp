...
static u8 _dw_phy_i2cm_wait_irq_state(void)
{
	...
	do {
		udelay(10);
		state = dw_mc_irq_get_state(DW_IRQ_I2C_PHY);
	} while ((state == 0) && (times--));
	/* clear read state */
	dw_mc_irq_clear_state(DW_IRQ_I2C_PHY, state);
...
int dw_phy_initialize(void)
{
	...
	dw_mc_irq_clear_state(DW_IRQ_I2C_PHY, 0x0);
...
ssize_t dw_phy_dump(char *buf)
{
	...
	n += sprintf(buf + n, "     - mpll    : [%s]\n",
			_dw_phy_get_mpll_lock() ? "lock" : "unlock");
	n += sprintf(buf + n, "     - power   : [%s]\n",
			_dw_phy_get_power_state() ? "enable" : "disable");
	n += sprintf(buf + n, "     - rxsense : [0x%X]\n",
			_dw_phy_get_rxsense());

	n += sprintf(buf + n, " - [hpd]\n");
	n += sprintf(buf + n, "     - state : [%s]\n",
			dw_phy_get_hpd() ? "detect" : "undetect");
	n += sprintf(buf + n, "     - sense : [%s]\n",
