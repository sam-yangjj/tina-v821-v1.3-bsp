					...
					struct jadard_i2c_platform_data *pdata)
{
	...
	uint32_t coords[4] = {0};
	uint32_t ret, data;
	uint32_t revert_x, revert_y, swap_x2y;
	...
	if (of_property_read_u32(dt, "jadard,revert-x", &revert_x) == 0) {
		pdata->revert_x = revert_x;
	}

	if (of_property_read_u32(dt, "jadard,revert-y", &revert_y) == 0) {
		pdata->revert_y = revert_y;
	}
	if (of_property_read_u32(dt, "jadard,swap-x2y", &swap_x2y) == 0) {
		pdata->swap_x2y = swap_x2y;
	}

	JD_I("DT:revert_x = %d, revert_y =  %d, swap_x2y = %d\n", pdata->revert_x, pdata->revert_y, pdata->swap_x2y);
	...
	if (of_property_read_u32_array(dt, "jadard,panel-coords", coords, coords_size) == 0) {
		...
		if (pdata->swap_x2y)
			JAD_SWAP(pdata->abs_x_max, pdata->abs_y_max);
