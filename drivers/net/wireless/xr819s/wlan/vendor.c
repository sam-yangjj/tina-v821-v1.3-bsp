...
static const struct nl80211_vendor_cmd_info xradio_nl80211_vendor_events[] = {
	...
	[NL80211_VENDOR_RECV_SPEC_FRM_INDEX] = {
		.vendor_id = XRADIO_NL80211_VENDOR_ID,
		.subcmd = NL80211_VENDOR_SUBCMD_RECV_SPEC_FRM
	},
#endif
};
...
#ifdef XRADIO_RAW_FRM_SUP
static int xradio_vendor_rx_spec_skb_event(struct wiphy *wiphy, struct wireless_dev *wdev,
                                                    const void *data, u32 len)
{
	...
	skb = cfg80211_vendor_event_alloc(wiphy, wdev, len + 12, NL80211_VENDOR_RECV_SPEC_FRM_INDEX, kflags);
	if (!skb) {
		wiphy_err(wiphy, "skb alloc failed\n");
		return -ENOMEM;
	}

	ret = nla_put(skb, DEBUG_ATTRIBUTE_RECV_SPEC_FRM, len, data);
	if (!ret) {
		cfg80211_vendor_event(skb, kflags);
	} else {
		printk(KERN_ERR "%s: nla_put failed. ret = %d\n", __func__, ret);
		kfree_skb(skb);
	}
...
int xradio_rx_spec_frame(struct xradio_common *hw_priv, u8 *skb_data, u32 skb_len)
{
	...
	if (wiphy) {
		xradio_vendor_rx_spec_skb_event(wiphy, NULL, (const void *)skb_data, skb_len);
	}
