...
unsigned int vin_log_mask = 0xffff - VIN_LOG_ISP - VIN_LOG_STAT - VIN_LOG_VIDEO - VIN_LOG_TDM - VIN_LOG_RP;
...
int vin_get_ion_phys(struct device *dev, struct vin_mm *mem_man)
{
	...
	mem_man->phy_addr = (void *)(uintptr_t)sg_dma_address(sgt->sgl);
	mem_man->sgt = sgt;
...
int os_mem_alloc(struct device *dev, struct vin_mm *mem_man)
{
	...
	mem_man->buf = ion_alloc(mem_man->size, ION_HEAP_CUSTOM_START_MASK, 0);
	if (IS_ERR_OR_NULL(mem_man->buf)) {
		vin_err("cma alloc failed(use ion size poll heap)\n");
		goto err_alloc;
	}
#else
	/* CMA or CARVEOUT */
...
int vin_mem_pool_dma_alloc(struct device *dev, struct vin_mm *mem_man)
{
#if IS_ENABLED(CONFIG_VIN_USE_ION_SIZE_POOL_HEAP)
	int ret;

	mem_man->buf = ion_alloc(mem_man->size, ION_HEAP_CUSTOM_START_MASK, ION_FLAG_CACHED);
	if (IS_ERR_OR_NULL(mem_man->buf)) {
		vin_err("cma alloc failed(use ion size poll heap)\n");
		goto err_alloc;
	}
...
EXPORT_SYMBOL_GPL(vin_mem_pool_dma_alloc);

void vin_mem_pool_dma_free(struct device *dev, struct vin_mm *mem_man)
{
	os_mem_free(dev, mem_man);
}
EXPORT_SYMBOL_GPL(vin_mem_pool_dma_free);

int vin_mem_pool_dma_mmap(struct device *dev, struct vm_area_struct *vma,
			  void *cpu_addr, dma_addr_t dma_addr, size_t size,
			  unsigned long attrs)
{
#if IS_ENABLED(CONFIG_VIN_USE_ION_SIZE_POOL_HEAP)
	attrs |= AW_ANSC_DMA_ATTR_CACHED;
	return sunxi_ansc_dma_mmap(dev, vma, cpu_addr, dma_addr, size, attrs);
#else
	return 0;
#endif
}
EXPORT_SYMBOL_GPL(vin_mem_pool_dma_mmap);
