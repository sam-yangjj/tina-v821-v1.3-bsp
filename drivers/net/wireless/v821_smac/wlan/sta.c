...
*/
	...
	if (atomic_read(&hw_priv->num_vifs) < XRWL_MAX_VIFS) {
		...
		sta_printk(XRADIO_DBG_ALWY, "%s:vif_type:%d p2p:%d if_id:%d macaddr: %pM\n",
			    __func__, vif->type, vif->p2p, priv->if_id, vif->addr);
#else
	...
	*/
	...
	if (priv->mode == NL80211_IFTYPE_MONITOR) {
		...
		/* set monitor default channel */
		if (hw_priv->channel && hw_priv->channel->hw_value)
			chan.hw_value = hw_priv->channel->hw_value;
		else
			chan.hw_value = 3;
			     ...
			     int duration, enum ieee80211_roc_type type)
{
	...
	TES_P2P_0002_roc_end_tmval.tv_sec = TES_P2P_0002_roc_sec + duration / 1000;
	TES_P2P_0002_roc_end_tmval.tv_usec = TES_P2P_0002_roc_usec + duration * 1000;
#endif
...
static void xradio_cancel_rem_chan(struct xradio_common *hw_priv)
{
	int if_id;
	struct xradio_vif *priv;
	sta_printk(XRADIO_DBG_TRC, "%s\n", __func__);
...
int xradio_cancel_remain_on_channel(struct ieee80211_hw *hw, struct ieee80211_vif *vif)
{
	...
	if (atomic_read(&hw_priv->remain_on_channel)) {
		...
		/***********************************************************************************
		 * It is possible that roc_work_done was initiated due to timeout, but the work of
		 * umac happened to initiate the cancel_roc action before scheduling, which  could
		 * result in the same roc being processed twice by the wlan driver. Whether it is
		 * the roc_work_done initiated by timeout or the umac initiating the cancel_roc,
		 * the wlan driver has already processed it, so a status code indicating successful
		 * operation should be returned at this point.
		 ***********************************************************************************/
		return 0;
	}
	...
	if (hw_priv->hw_restart || hw_priv->bh_error) {
		...
		/***********************************************************************************
		 * The reason for returning a successful status code to UMAC here is that the WLAN
		 * driver has been restart, and related resources will be reinitialized.
		 * If UMAC receives an abnormal status code during the cancel_roc process, it will
		 * trigger a system warning and fail to properly destroy the current abnormal roc_work,
		 * resulting in a system warning being triggered again the next time roc_work is initiated.
		 ***********************************************************************************/
		return 0;
	}
...
void xradio_join_work(struct work_struct *work)
{
	...
	down(&hw_priv->conf_lock);
	{
		...
		if (priv->vif->p2p) {
			...
			sta_printk(XRADIO_DBG_ALWY, "USE FORCE JOIN\n");
			join.probeForJoin = 0;
			join.basicRateSet =
...
int xradio_vif_setup(struct xradio_vif *priv)
{
	...
	if (priv->if_id < 2) {
...
#if XRADIO_USER_CHANNEL_FEC
		{
			int16_t ch_1, ch_7, ch_13;
			if (xradio_get_channel_fec_from_efuse(&ch_1, &ch_7, &ch_13)) {
				sta_printk(XRADIO_DBG_MSG, "Get channel FEC efuse failed!\n");
			} else {
				sta_printk(XRADIO_DBG_ALWY, "FEC ch1:%d ch7:%d ch13:%d\n", ch_1, ch_7, ch_13);
				if (wsm_set_channel_fec(hw_priv, ch_1, ch_7, ch_13, priv->if_id)) {
					sta_printk(XRADIO_DBG_WARN, "Set channel FEC failed!\n");
				}
			}
		}
#endif
		...
		for (i = 0; i < POWER_LEVEL_TAB_USE_LENGTH; i++) {
			if (hw_priv->sdd_power_level_tab[i])
				priv->user_config.power_level_tab[i] = ((hw_priv->sdd_power_level_tab[i] * 10) / 16);
		}

		ret = wsm_set_output_power_tab(hw_priv, hw_priv->output_power * 10, priv->if_id);
#endif
...
#ifdef MONITOR_RX_FIX
static int xraido_rx_restart(struct xradio_common *hw_priv)
{
	int ret = -1;
	u32 reg_val = 0;
	u32 reg_addr = PHY_PHY_PHYSOFTRSTN;
	WSM_REG_W reg_wr;
	WSM_REG_R reg_rd;

	memset(&reg_wr, 0, sizeof(reg_wr));
	memset(&reg_rd, 0, sizeof(reg_rd));

	reg_rd.flag = (u16)(WSM_REG_RET_F);
	/* sizeof:flag + data_size + u32 */
	reg_rd.data_size = 8;
	reg_rd.arg[0] = reg_addr;
	ret = wsm_read_mib(hw_priv, WSM_MIB_ID_RW_FW_REG, (void *)&reg_rd, sizeof(WSM_REG_R), reg_rd.data_size);
	if (ret) {
		sta_printk(XRADIO_DBG_ERROR, "read reg is failed! addr = 0x%08x\n", reg_addr);
		return -1;
	}
	reg_val = reg_rd.arg[0];
	sta_printk(XRADIO_DBG_MSG, "reg_addr:0x%08x, val:0x%08x\n", reg_addr, reg_val);

	reg_wr.flag = (u16)(WSM_REG_RET_F | WSM_REG_RW_F);
	reg_wr.data_size = 12;
	reg_wr.arg[0].reg_addr = reg_addr;
	reg_wr.arg[0].reg_val = (reg_val & (~0x1));     /* reset phy data */
	sta_printk(XRADIO_DBG_MSG, "write reg_addr:0x%08x, val:0x%08x\n", reg_addr, reg_wr.arg[0].reg_val);
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_RW_FW_REG, (void *)&reg_wr, reg_wr.data_size, 0);
	if (ret) {
		sta_printk(XRADIO_DBG_ERROR, "write reg is failed! addr =0x%08x, val = 0x%x\n", reg_addr, reg_wr.arg[0].reg_val);
		return -1;
	}

	msleep(1);
	reg_wr.arg[0].reg_val = (reg_val | 0x1);
	sta_printk(XRADIO_DBG_MSG, "write reg_addr:0x%08x, val:0x%08x\n", reg_addr, reg_wr.arg[0].reg_val);
	ret = wsm_write_mib(hw_priv, WSM_MIB_ID_RW_FW_REG, (void *)&reg_wr, reg_wr.data_size, 0);
	if (ret) {
		sta_printk(XRADIO_DBG_ERROR, "write reg is failed! addr =0x%08x, val = 0x%x\n", reg_addr, reg_wr.arg[0].reg_val);
	...
	sta_printk(XRADIO_DBG_MSG, "%s restart rx success!\n", __func__);
	return 0;
}

void xradio_monitor_rx_restart_work(struct work_struct *work)
{
	struct xradio_common *hw_priv = container_of(work, struct xradio_common, monitor_rx_restart_work);

	if (!hw_priv) {
		sta_printk(XRADIO_DBG_ERROR, "%s hw_priv is NULL!\n", __func__);
		return;
	}

	xraido_rx_restart(hw_priv);
}
#endif
