...
int fb_g2d_rot_exit(void *rot)
{
    ...
    IS_ENABLED(CONFIG_AW_DMABUF_HEAPS_CARVEOUT)
		disp_ion_free((void *__force)inst->dst_vir_addr,
    ...
    IS_ENABLED(CONFIG_AW_DMABUF_HEAPS_CARVEOUT)
{
	...
	p_ion_mem = disp_ion_malloc(fb_rot->dst_mem_len, (&fb_rot->dst_phy_addr));
	fb_rot->dst_vir_addr = p_ion_mem ? (char __iomem *)p_ion_mem->vaddr : NULL;
}
#else
	fb_rot->dst_vir_addr =
