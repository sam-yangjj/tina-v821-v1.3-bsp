...
extern u8 dbg_itp    ;
extern u8 dbg_etf    ;
extern u8 dbg_bypass ;
extern u8 dbg_xradio_rpmsg ;
...
extern struct timeval mac_start_time;
extern struct timeval ind_rx_time;
extern struct timeval upper_rx_time;
extern struct timeval proc_start_time;
extern struct timeval bh_put_time;
extern struct timeval tx_start_time1;
extern struct timeval tx_start_time2;
extern struct timeval rx_start_time1;
extern struct timeval rx_start_time2;
extern struct timeval bh_start_time;
extern struct timeval sdio_reg_time;
extern struct timeval wakeup_time;
extern struct timeval last_showtime;
extern size_t upper_rx_size;
#if TEST_GETTIME
...
#define hifbypass_printk(level, ...)     \
	do {							  \
		if ((level) & dbg_bypass & XRADIO_DBG_ERROR)		 \
			printk(KERN_ERR "[BYPASS_ERR] " __VA_ARGS__);  \
		else if ((level) & dbg_bypass & XRADIO_DBG_WARN)	 \
			printk(KERN_ERR "[BYPASS_WRN] " __VA_ARGS__);  \
		else if ((level) & dbg_bypass)					 \
			printk(KERN_ERR "[BYPASS] " __VA_ARGS__); 	 \
		if ((level) & dbg_logfile)		   \
			LOG_FILE_VARS(((level)&XRADIO_DBG_ERROR),	 \
					  "[BYPASS] " __VA_ARGS__);	 \
	} while (0)

#define xradio_rpmsg_printk(level, ...)     \
	do {							  \
		if ((level) & dbg_xradio_rpmsg & XRADIO_DBG_ERROR)		 \
			printk(KERN_ERR "[XRADIO_RPMSG_ERR] " __VA_ARGS__);  \
		else if ((level) & dbg_xradio_rpmsg & XRADIO_DBG_WARN)	 \
			printk(KERN_ERR "[XRADIO_RPMSG_WRN] " __VA_ARGS__);  \
		else if ((level) & dbg_xradio_rpmsg)					 \
			printk(KERN_ERR "[XRADIO_RPMSG] " __VA_ARGS__);	 \
		if ((level) & dbg_logfile)		   \
			LOG_FILE_VARS(((level)&XRADIO_DBG_ERROR),	 \
					  "[BYPASS] " __VA_ARGS__);  \
	} while (0)
...
#if (defined(CONFIG_XRADIO_DEBUG_MIN))

/*XRADIO_DBG_ALWY   0x01
 *XRADIO_DBG_ERROR  0x02
 *XRADIO_DBG_WARN   0x04
 */
#define DEBU_MIN_LEVEL 0x07

#define xradio_dbg(level, ...)           \
	do {                                 \
		if ((level) & DEBU_MIN_LEVEL)                    \
			printk(KERN_ERR "XR:" __VA_ARGS__);     \
	} while (0)
...
extern u32 fw_debug_rx;
extern int etf_rx_data;
void fw_debug_print(struct xradio_common *hw_priv);
int fw_debug_ctrl_init(const DBG_CTRL *FW_DBG_CTRL);
#endif
