			       ...
			       struct xradio_queue *queue)
{
	...
	case NL80211_IFTYPE_P2P_DEVICE:
		if (unlikely(priv->bss_loss_status == XRADIO_BSS_LOSS_CHECKING &&
			...
			if (wsm->reserved & WSM_TX_SEND_RAW_FRAME) {
				action = doTx;	 // send raw frame
				hw_priv->raw_packetID = __le32_to_cpu(wsm->packetID);
				wsm_printk(XRADIO_DBG_NIY, "%s__%d send raw frame,PacketID = %u\n",
					   __func__, __LINE__, hw_priv->raw_packetID);
				break;
			}
#endif
