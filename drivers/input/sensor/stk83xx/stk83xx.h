...
#define STK83XX_REG_TILTCFG 0x25
...
static const u8 STK_ID[] = { STK8BA50_R_ID, STK8BA53_ID, STK8323_ID, STK8327_ID,
			     STK8329_ID };
...
/* STK83XX_REG_TILTTHD */
#define STK83XX_TILTTHD_25 0x0B
#define STK83XX_TILTTHD_30 0x11
#define STK83XX_TILTTHD_35 0x17
#define STK83XX_TILTTHD_40 0x1D
#define STK83XX_TILTTHD_45 0x25
#define STK83XX_TILTTHD_50 0x2D
#define STK83XX_TILTTHD_55 0x36
#define STK83XX_TILTTHD_60 0x3F
#define STK83XX_TILTTHD_65 0x49
#define STK83XX_TILTTHD_70 0x54
#define STK83XX_TILTTHD_75 0x5E

/* STK83XX_REG_TILTCFG */
#define STK83XX_TILTCFG_EN 0x10
#define STK83XX_TILTCFG_STS_MASK 0x80
#define STK83XX_TILTCFG_DEB_0S 0x00
#define STK83XX_TILTCFG_DEB_0_5S 0x20
#define STK83XX_TILTCFG_DEB_1S 0x40
#define STK83XX_TILTCFG_DEB_6S 0x60
...
#define STK_ACC_LOG(fmt, args...)                                              \
	printk(KERN_INFO STK_ACC_TAG " %s/%d: " fmt "\n", __FUNCTION__,        \
	       __LINE__, ##args)
#define STK_ACC_ERR(fmt, args...)                                              \
	printk(KERN_ERR STK_ACC_TAG " %s/%d: " fmt "\n", __FUNCTION__,         \
	       __LINE__, ##args)

#define STK_REG_READ(stk_data, reg, val)                                       \
	((stk_data)->bops->read((stk_data)->bus_idx, reg, val))
#define STK_REG_READ_BLOCK(stk_data, reg, count, buf)                          \
	((stk_data)->bops->read_block((stk_data)->bus_idx, reg, count, buf))
#define STK_REG_WRITE(stk_data, reg, val)                                      \
	((stk_data)->bops->write((stk_data)->bus_idx, reg, val))
#define STK_REG_WRITE_BLOCK(stk_data, reg, val, len)                           \
	((stk_data)->bops->write_block((stk_data)->bus_idx, reg, val, len))
#define STK_REG_READ_MODIFY_WRITE(stk_data, reg, val, mask)                    \
	((stk_data)->bops->read_modify_write((stk_data)->bus_idx, reg, val,    \
					     mask))

#define STK_TIMER_REGISTER(stk_data, t_info)                                   \
	((stk_data)->tops->register_timer(t_info))
#define STK_TIMER_START(stk_data, t_info)                                      \
	((stk_data)->tops->start_timer(t_info))
...
#define STK_TIMER_BUSY_WAIT(stk_data, min, max, mode)                          \
	((stk_data)->tops->busy_wait(min, max, mode))

#define STK_GPIO_IRQ_REGISTER(stk_data, g_info)                                \
	((stk_data)->gops->register_gpio_irq(g_info))
#define STK_GPIO_IRQ_START(stk_data, g_info)                                   \
	((stk_data)->gops->start_gpio_irq(g_info))
#define STK_GPIO_IRQ_STOP(stk_data, g_info)                                    \
	((stk_data)->gops->stop_gpio_irq(g_info))
...
#define STK_W_TO_STORAGE(stk_data, name, buf, size)                            \
	((stk_data)->sops->write_to_storage(name, buf, size))
#define STK_R_FROM_STORAGE(stk_data, name, buf, size)                          \
	((stk_data)->sops->read_from_storage(name, buf, size))
#define STK_STORAGE_REMOVE(stk_data) ((stk_data)->sops->remove())

#define STK83XX_READ_DATA_CB(stk_data)                                         \
	if ((stk_data)->read_data_cb)                                          \
	((stk_data)->read_data_cb(stk_data))
#ifdef STK_AMD
#define STK83XX_AMD_REPORT(stk_data, flag)                                     \
	if ((stk_data)->reportamd_cb)                                          \
	((stk_data)->reportamd_cb(stk_data, flag))
#endif
#define STK83XX_ACCEL_REPORT(stk_data)                                         \
	if ((stk_data)->accel_report_cb)                                       \
	((stk_data)->accel_report_cb(stk_data))
#define STK83XX_READ_FIFO_CB(stk_data)                                         \
	if ((stk_data)->read_fifo_cb)                                          \
	((stk_data)->read_fifo_cb(stk_data))
#define STK83XX_FIFO_REPORT(stk_data, xyz, len)                                \
	if ((stk_data)->fifo_report_cb)                                        \
	((stk_data)->fifo_report_cb(stk_data, xyz, len))
#define STK83XX_REQUEST_REGISTRY(stk_data)                                     \
	if ((stk_data)->request_registry_cb)                                   \
	((stk_data)->request_registry_cb(stk_data))
