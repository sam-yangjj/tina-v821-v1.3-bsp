...
#define SUNXI_GMAC_MODULE_VERSION	"2.5.10"
...
/* in some platforms, tx delay bits extend from 3 to 5 */
/* tx delay higher 2 bit 3:4 offset and mask in reg */
#define SUNXI_GMAC_TX_DELAY_HIGH_OFFSET	16
#define SUNXI_GMAC_TX_DELAY_HIGH_MASK	0x3
/* tx delay lower 3 bit 0:2 offset and mask in reg */
#define SUNXI_GMAC_TX_DELAY_MASK	0x07
#define SUNXI_GMAC_TX_DELAY_OFFSET	10
/* rx delay offset and mask in reg */
...
#define SUNXI_PHY_USE_EXT_OSC		1	/* External phy use extern osc 25m */
	...
	/* in some platforms, tx delay bits extend from 3 to 5 */
	bool tx_delay_extend;
	/* adjust transmit clock delay, value: 0~7 or 0~31 */
	u32 tx_delay;
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (chip->tx_delay_extend) {
		u16 tx_delay_high;
		tx_delay_high = (reg_val >> SUNXI_GMAC_TX_DELAY_HIGH_OFFSET) & SUNXI_GMAC_TX_DELAY_HIGH_MASK;
		tx_delay |= tx_delay_high << 3; /* higher 2 bit 3:4 */
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (chip->tx_delay_extend) {
		reg_val &= ~(SUNXI_GMAC_TX_DELAY_HIGH_MASK << SUNXI_GMAC_TX_DELAY_HIGH_OFFSET);
		/* get higher 2 bit 3:4 in clk_value */
		reg_val |= ((tx_delay & 0x18) << SUNXI_GMAC_TX_DELAY_HIGH_OFFSET);
	}
...
static int sunxi_gmac_clk_enable(struct sunxi_gmac *chip)
{
	...
	if (chip->tx_delay_extend) {
		clk_value &= ~(SUNXI_GMAC_TX_DELAY_HIGH_MASK << SUNXI_GMAC_TX_DELAY_HIGH_OFFSET);
		/* get higher 2 bit in clk_value */
		clk_value |= ((chip->tx_delay & 0x18) << SUNXI_GMAC_TX_DELAY_HIGH_OFFSET);
	}
...
#if IS_ENABLED(CONFIG_AW_CHIPS) && IS_ENABLED(CONFIG_ARCH_SUN300IW1)
	if (sunxi_chip_alter_version() == SUNXI_CHIP_ALTER_VERSION_V821B)
		chip->tx_delay_extend = true;
#endif
