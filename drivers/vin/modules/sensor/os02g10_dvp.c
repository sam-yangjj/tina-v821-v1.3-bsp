...
#define SENSOR_NAME "os02g10_dvp"
#define SENSOR_NAME_2 "os02g10_dvp_2"
...
static int sensor_g_fps(struct v4l2_subdev *sd, struct sensor_fps *fps)
{
	...
	sensor_read(sd, 0x4e, &frame_length);
	act_vts = frame_length << 8;
	sensor_read(sd, 0x4f, &frame_length);
...
{
	struct sensor_info *info = to_state(sd);
	*code = info->fmt->mbus_code;// os02g10 support change the rgb format by itself
	return 0;
}
