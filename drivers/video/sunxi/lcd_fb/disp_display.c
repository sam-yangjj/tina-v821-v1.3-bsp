...
struct sunxi_lcd_fb_disp_dev_t g_lcd_fb_disp;
					     ...
					     struct sunxi_lcd_fb_disp_lcd_panel_fun *lcd_cfg)
{
	...
	for (screen_id = 0; screen_id < num_screens; screen_id++) {
		lcd = sunxi_lcd_fb_disp_get_lcd(screen_id);
		if (lcd && (lcd->set_panel_func)) {
			if (!lcd->set_panel_func(lcd, name, lcd_cfg)) {
				g_lcd_fb_disp.lcd_registered[screen_id] = 1;
				registered_cnt++;
				LCDFB_INF("panel driver %s register\n", name);
			}
		}
	}
...
void LCD_FB_OPEN_FUNC(u32 disp, LCD_FUNC func, u32 delay)
{
	struct sunxi_lcd_fb_device *lcd;
...
void LCD_FB_CLOSE_FUNC(u32 disp, LCD_FUNC func, u32 delay)
{
	struct sunxi_lcd_fb_device *lcd;
...
s32 sunxi_lcd_fb_bsp_disp_get_lcd_registered(u32 disp)
{
	return g_lcd_fb_disp.lcd_registered[disp];
}
...
void *sunxi_lcd_fb_dma_malloc(u32 num_bytes, void *phys_addr)
{
	...
	if (num_bytes != 0) {
		...
		address = dma_alloc_coherent(g_drv_info.device, actual_bytes,
					     (dma_addr_t *)phys_addr, GFP_KERNEL);
		if (address) {
			LCDFB_INF("dma_alloc_coherent ok, address=0x%p, size=0x%x\n",
				  (void *)(*(unsigned long *)phys_addr), num_bytes);
			return address;
		} else {
			LCDFB_WRN("dma_alloc_coherent fail, size=0x%x\n", num_bytes);
			return NULL;
		}
	} else {
		LCDFB_WRN("%s size is zero\n", __func__);
	}
...
void sunxi_lcd_fb_dma_free(void *virt_addr, void *phys_addr, u32 num_bytes)
{
	...
	if (phys_addr && virt_addr)
		dma_free_coherent(g_drv_info.device, actual_bytes, virt_addr,
				  (dma_addr_t)(uintptr_t)phys_addr);
}
...
s32 sunxi_lcd_fb_bsp_disp_qspi_lcd_cmd_write(u32 screen_id, u8 cmd)
{
	...
	lcd = sunxi_lcd_fb_disp_get_lcd(screen_id);
	if (lcd && lcd->qspi_cmd_write)
		ret = lcd->qspi_cmd_write(lcd, cmd);
...
s32 sunxi_lcd_fb_bsp_disp_qspi_lcd_para_write(u32 screen_id, u8 cmd, u8 para)
{
	...
	lcd = sunxi_lcd_fb_disp_get_lcd(screen_id);
	if (lcd && lcd->qspi_para_write)
		ret = lcd->qspi_para_write(lcd, cmd, para);
...
s32 sunxi_lcd_fb_bsp_disp_qspi_lcd_multi_para_write(u32 screen_id, u8 cmd, u8 *tx_buf, u32 len)
{
	...
	lcd = sunxi_lcd_fb_disp_get_lcd(screen_id);
	if (lcd && lcd->qspi_multi_para_write)
		ret = lcd->qspi_multi_para_write(lcd, cmd, tx_buf, len);
...
s32 sunxi_lcd_fb_bsp_disp_qspi_lcd_cmd_read(u32 screen_id, u8 cmd, u8 *rx_buf, u8 len)
{
	...
	lcd = sunxi_lcd_fb_disp_get_lcd(screen_id);
	if (lcd && lcd->qspi_cmd_read)
		ret = lcd->qspi_cmd_read(lcd, cmd, rx_buf, len);
