...
/*
 * Copyright (C) 2024 Allwinner Technology Limited. All rights reserved.
 *
		...
		struct device_attribute *attr, char *buf)
{
	...
	total += scnprintf(buf + total, max_size - total, "GPU_IRQ_RAWSTAT=0x%08x"
		"	GPU_STATUS=0x%08x\n",
		sunxi_kbase_reg_read(s_kbdev, GPU_CONTROL_ENUM(GPU_IRQ_RAWSTAT)),
		sunxi_kbase_reg_read(s_kbdev, GPU_CONTROL_ENUM(GPU_STATUS)));
	total += scnprintf(buf + total, max_size - total, "JOB_IRQ_RAWSTAT=0x%08x"
		"	JOB_IRQ_JS_STATE=0x%08x\n",
		sunxi_kbase_reg_read(s_kbdev, JOB_CONTROL_ENUM(JOB_IRQ_RAWSTAT)),
		sunxi_kbase_reg_read(s_kbdev, JOB_CONTROL_ENUM(JOB_IRQ_JS_STATE)));

	for (i = 0; i < 3; i++) {
		total += scnprintf(buf + total, max_size - total, "JS%d_STATUS=0x%08x"
				"	JS%d_HEAD_LO=0x%08x\n",
				i, sunxi_kbase_reg_read(s_kbdev, JOB_SLOT_OFFSET(i, STATUS)),
				i, sunxi_kbase_reg_read(s_kbdev, JOB_SLOT_OFFSET(i, HEAD)));
	}

	total += scnprintf(buf + total, max_size - total, "MMU_IRQ_RAWSTAT=0x%08x"
		"	GPU_FAULTSTATUS=0x%08x\n",
		sunxi_kbase_reg_read(s_kbdev, MMU_CONTROL_ENUM(IRQ_RAWSTAT)),
		sunxi_kbase_reg_read(s_kbdev, GPU_CONTROL_ENUM(GPU_FAULTSTATUS)));
	total += scnprintf(buf + total, max_size - total, "GPU_IRQ_MASK=0x%08x"
		"	JOB_IRQ_MASK=0x%08x	MMU_IRQ_MASK=0x%08x\n",
		sunxi_kbase_reg_read(s_kbdev, GPU_CONTROL_ENUM(GPU_IRQ_MASK)),
		sunxi_kbase_reg_read(s_kbdev, JOB_CONTROL_ENUM(JOB_IRQ_MASK)),
		sunxi_kbase_reg_read(s_kbdev, MMU_CONTROL_ENUM(IRQ_MASK)));
	total += scnprintf(buf + total, max_size - total, "PWR_OVERRIDE0=0x%08x"
		"	PWR_OVERRIDE1=0x%08x\n",
		sunxi_kbase_reg_read(s_kbdev, GPU_CONTROL_ENUM(PWR_OVERRIDE0)),
		sunxi_kbase_reg_read(s_kbdev, GPU_CONTROL_ENUM(PWR_OVERRIDE1)));
	total += scnprintf(buf + total, max_size - total, "SHADER_CONFIG=0x%08x"
		"	L2_MMU_CONFIG=0x%08x\n",
		sunxi_kbase_reg_read(s_kbdev, GPU_CONTROL_ENUM(SHADER_CONFIG)),
		sunxi_kbase_reg_read(s_kbdev, GPU_CONTROL_ENUM(L2_MMU_CONFIG)));
	total += scnprintf(buf + total, max_size - total, "TILER_CONFIG=0x%08x"
		"	JM_CONFIG=0x%08x\n",
		sunxi_kbase_reg_read(s_kbdev, GPU_CONTROL_ENUM(TILER_CONFIG)),
		sunxi_kbase_reg_read(s_kbdev, GPU_CONTROL_ENUM(JM_CONFIG)));
