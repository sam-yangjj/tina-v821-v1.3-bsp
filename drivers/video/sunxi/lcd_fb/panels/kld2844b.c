...
static void LCD_panel_init(unsigned int sel)
{
	...
	/* sunxi_lcd_cmd_read(sel, 0x04, result, 4);
	printk("[LCD_FB] panel cmd read ID: %x, %x, %x, %x\n", result[0],
	...
	if (info.lcd_pixel_fmt == LCDFB_FORMAT_RGB_565 ||
		...
		if (info.lcd_pixel_fmt == LCDFB_FORMAT_BGRA_8888 ||
		    info.lcd_pixel_fmt == LCDFB_FORMAT_BGRX_8888 ||
		    info.lcd_pixel_fmt == LCDFB_FORMAT_ABGR_8888 ||
		    info.lcd_pixel_fmt == LCDFB_FORMAT_XBGR_8888) {
			rotate |= 0x08;
		}
	} else {
		sunxi_lcd_para_write(sel, 0x66);
	...
	sunxi_lcd_cmd_write(
		sel, 0x36); /* Memory sunxi_lcd_para_write Access Control */
