...
#define WSM_TX_PATCH_POLICY_MAX		(BIT(1))
...
#define WSM_MIB_ID_SET_PRE_RX_BEACON	0x1052
#define WSM_MIB_ID_BEACON_WIN_CFG	0x1073

/*use for changing mac address of interface*/
#ifdef CONFIG_DRIVER_V821
#define WSM_MIB_ID_CHANGE_MAC		0x1048
#else
...
#define WSM_MIB_ID_SET_HW_INFO		0x1070

#define WSM_MIB_ID_CE_TEST_CONFIG               0x1080
#define WSM_MIB_ID_SET_POWER_LEVEL_TAB          0x1091  /* set dot11CurrentTxPowerLevelTable */
#define WSM_MIB_ID_SET_DOT11_CHANNEL_FEC        0x1093
#define WSM_MIB_ID_BEACON_OFFSET_TIME           0x1096
#define WSM_MIB_ID_SET_LFCLOCK_PARAM            0x1097
#define WSM_MIB_ID_SET_TX_CONFIRM_STATUS        0x109F

#define WSM_MIB_ID_AIF_ENABLE                   0x10B5

#define WSM_MIB_ID_CONFIG_FREQ_OFFSET           0x10C0
#define WSM_MIB_ID_GET_TX_POLICY_CNT            0x10C3
#ifdef CONFIG_FW_TXRXBUF_USE_COEXIST_RAM
#define WSM_MIB_ID_CONFIG_EPTA                  0x10C6
#define WSM_MIB_ID_SWITCH_FW_TXRX_BUFFER        0x10C7
#endif
#define WSM_MIB_ID_FW_SOFT_AGC_CCA_POLICY       0x10CA
...
#define WSM_STATUS_FLUSH                (15)
#define WSM_STATUS_FLUSH_END_OF_LIFE    (16)
...
#ifdef CONFIG_FW_TXRXBUF_USE_COEXIST_RAM
#define WSM_CMD_LOCK_TIMEOUT            (WSM_CMD_LAST_CHANCE_TIMEOUT)
#endif

#define WSM_RI_GET_PEER_ID_FROM_FLAGS(_f)         (((_f)&(0xF<<25)>>25))

/* ******************************************************************** */
/* TESAL_CHIP_SPECIAL_MARKER                                            */
#define TESAL_CHIP_USE_GUBEI_PROJ                 (BIT(0))
#define TESAL_CHIP_USE_IPC_PROJ                   (BIT(1))
#define RTS_POLICY_OPT_KEEP_CODE                  (BIT(2))
#define ENABLE_P2P_KEEP_CODE                      (BIT(3))
...
#if CONFIG_DRIVER_V821
				u32 pktdebug;          /*Records the flow of frames in the firmware*/
				u32 rsv[3]; 		   /* reserve for patch */
#endif
	/* [out] */ u32 link_id;
...
static inline int wsm_set_beacon_win_cfg(struct xradio_common *hw_priv,
					      u32 beacon_win,
					      int if_id)
{
	__le32 val = __cpu_to_le32(beacon_win);

	return wsm_write_mib(hw_priv, WSM_MIB_ID_BEACON_WIN_CFG,
		    &val, sizeof(val), if_id);
}
...
static inline int wsm_get_tx_policy_cnt(struct xradio_common *hw_priv, u32 *mac)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_GET_TX_POLICY_CNT, mac, sizeof(*mac), 0);
}

#define TX_POLICY_CACHE_SIZE           (15)
...
static inline int wsm_beacon_offset_time(struct xradio_common *hw_priv,
					int beacon_offset, int if_id)
{
	u32 val = __cpu_to_le16(beacon_offset);

	return wsm_write_mib(hw_priv, WSM_MIB_ID_BEACON_OFFSET_TIME,
			&val, sizeof(val), if_id);
};

static inline int wsm_11b_beacon_only(struct xradio_common *hw_priv,
					int enable, int if_id)
{
	u32 val = __cpu_to_le16(enable);
...
static inline int
wsm_set_freq_offset(struct xradio_common *hw_priv, struct wsm_config_freq_offset *arg, int if_id)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_CONFIG_FREQ_OFFSET, arg, sizeof(struct wsm_config_freq_offset), if_id);
}

static inline int
wsm_get_freq_offset(struct xradio_common *hw_priv, struct wsm_config_freq_offset *arg)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_CONFIG_FREQ_OFFSET, arg, sizeof(*arg), 0);
}

static inline int
wsm_use_new_tx_confirm_status(struct xradio_common *hw_priv, bool enabled, int if_id)
{
	__le32 arg = enabled ? __cpu_to_le32(1) : 0;

	return !(wsm_write_mib(hw_priv, WSM_MIB_ID_SET_TX_CONFIRM_STATUS, &arg, sizeof(arg), if_id));
}
...
#ifdef CONFIG_FW_TXRXBUF_USE_COEXIST_RAM
static inline int
wsm_set_epta_config(struct xradio_common *hw_priv, u32 *arg, int if_id)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_CONFIG_EPTA, arg, sizeof(u32), if_id);
}

static inline int
wsm_set_switch_fw_txrx_buf(struct xradio_common *hw_priv, u32 *arg, int if_id)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SWITCH_FW_TXRX_BUFFER, arg, sizeof(u32), if_id);
}

static inline int
wsm_get_switch_fw_txrx_buf(struct xradio_common *hw_priv, u32 *big)
{
	return wsm_read_mib(hw_priv, WSM_MIB_ID_SWITCH_FW_TXRX_BUFFER, big, sizeof(*big), 0);
}
#endif

#ifdef CONFIG_DRIVER_V821
/* WSM_MIB_ID_SET_HW_INFO */
struct wsm_sethardware_info {
	u16 bg_settling_time;
	u16 pfmtopwm_settling_time;
	u32 osc_freq;		/* based on Hz */
	u16 osc_type;		/* 0:LCOSC, 1:RCOSC */
	u16 osc_ppm;		/* ppm/deg */
#if (defined(CONFIG_CHIP_XRADIO) || defined(CONFIG_DRIVER_R128) || defined(CONFIG_DRIVER_V821))
#ifdef SUPPORT_HT40
	u8 efuse[256];
#else
#if (defined(CONFIG_CHIP_XRADIO) && (CONFIG_CHIP_ARCH_VER == 1))
	u8 efuse[256];
#elif (defined(CONFIG_CHIP_XRADIO) && (CONFIG_CHIP_ARCH_VER == 2))
	u8 efuse[128];
#elif ((defined(CONFIG_CHIP_XRADIO) && (CONFIG_CHIP_ARCH_VER == 3)) || \
		defined(CONFIG_DRIVER_R128) || defined(CONFIG_DRIVER_V821))
	u8 efuse[256];
#endif
#endif /* SUPPORT_HT40 */
#endif /* defined(CONFIG_CHIP_XRADIO) || defined(CONFIG_DRIVER_R128) */
	u16 use_host_pwr_up_cfg;
	u16 ldo1_sel_set_time;
	u16 ldo2_sel_set_time;
	u16 ldo_dcxo_set_time;
	u16 dcxo_set_time;
	u16 ldo2_set_time;
	u16 dpll_set_time;
	u16 rfdig_set_time;
	u16 ldo1_set_time;
	u16 vdd_to_sleep_time;
} __packed;

static inline int wsm_set_hardware_info(struct xradio_common *hw_priv, struct wsm_sethardware_info *arg)
{
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_HW_INFO, arg, sizeof(*arg), -1);
}
...
static inline int wsm_set_channel_fec(struct xradio_common *hw_priv, int16_t ch_1, int16_t ch_7, int16_t ch_13, int if_id)
{
	int ch;
	int16_t channel_fec_tab[14];

	for (ch = 0; ch < 14; ch++) {
		if (ch < 5)
			channel_fec_tab[ch] = ch_1;
		else if (ch >= 5 && ch < 10)
			channel_fec_tab[ch] = ch_7;
		else
			channel_fec_tab[ch] = ch_13;
	}
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_DOT11_CHANNEL_FEC, (void *)channel_fec_tab,
			sizeof(channel_fec_tab), if_id);
}

#endif

/**
 * @brief Parameter for WSM_MIB_ID_SET_LFCLOCK_PARAM
 */
