...
#include <cqhci.h>
#endif
#if IS_ENABLED(CONFIG_AW_MMC_DELAYINIT)
...
#if IS_ENABLED(CONFIG_AW_MMC_DELAYINIT)
static int sunxi_mmc_rpmsg_callback(void *dev, void *data, int len)
{
	const char *p = data;

	pr_info(" notify: %s\n", p);
	deferred_dev_add_by_name(p);
...
{
	rpmsg_notify_add(CONFIG_AW_MMC_DELAYINIT_RPROC_NAME, "mmc",
					sunxi_mmc_rpmsg_callback, NULL);
	return platform_driver_register(&sunxi_mmc_driver);
}
module_init(sunxi_mmc_driver_init);

static void __exit sunxi_mmc_driver_exit(void)
{
	platform_driver_unregister(&sunxi_mmc_driver);
}
module_exit(sunxi_mmc_driver_exit);
#else
static int __init sunxi_mmc_driver_init(void)
{
	return platform_driver_register(&sunxi_mmc_driver);
...
core_initcall(sunxi_mmc_driver_init);
#else
module_init(sunxi_mmc_driver_init);
#endif
