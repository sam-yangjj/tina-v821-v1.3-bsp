...
#if defined(USE_CSC2_COLOR_MATRIX)
/* v211 use */
...
int de_csc2_apply(struct disp_csc_config *config)
{
	...
	if (config->in_fmt == config->out_fmt && config->in_mode == config->out_mode
	    ...
	    && config->contrast == 50 && config->saturation == 50 && config->hue == 0
...
{
	struct de_csc2_private *priv;
	priv = get_csc2_priv(sel, 0, 0, CSC_TYPE_DISP);
	pq_enh[0] = _csc_enhance_setting[priv->csc_config_backup.enhance_mode][0];
	pq_enh[1] = _csc_enhance_setting[priv->csc_config_backup.enhance_mode][1];
	pq_enh[2] = _csc_enhance_setting[priv->csc_config_backup.enhance_mode][2];
	pq_enh[3] = _csc_enhance_setting[priv->csc_config_backup.enhance_mode][3];
}
...
void de_csc2_pq_set_enhance(u32 sel, int *pq_enh)
{
	struct de_csc2_private *priv;
	priv = get_csc2_priv(sel, 0, 0, CSC_TYPE_DISP);
	_csc_enhance_setting[priv->csc_config_backup.enhance_mode][0] = pq_enh[0];
	_csc_enhance_setting[priv->csc_config_backup.enhance_mode][1] = pq_enh[1];
	_csc_enhance_setting[priv->csc_config_backup.enhance_mode][2] = pq_enh[2];
	_csc_enhance_setting[priv->csc_config_backup.enhance_mode][3] = pq_enh[3];
}

/* void pq_set_matrix(struct __scal_matrix4x4 *conig, int choice, int out, int write)
{
	...
	if (ctx->output.data_bits == (enum de_data_bits)DISP_DATA_10BITS) {
		if (out) {
			if (write) {
				memcpy((y2r10bit + 0x20 * choice), conig, sizeof(struct __scal_matrix4x4));
			} else {
				memcpy(conig, (y2r10bit + 0x20 * choice), sizeof(struct __scal_matrix4x4));
			}
		} else {
			if (write) {
				memcpy((ir2y10bit + 0x20 * choice), conig, sizeof(struct __scal_matrix4x4));
			} else {
				memcpy(conig, (ir2y10bit + 0x20 * choice), sizeof(struct __scal_matrix4x4));
			}
		}
	} else {
		if (out) {
			if (write) {
				memcpy((y2r8bit + 0x20 * choice), conig, sizeof(struct __scal_matrix4x4));
			} else {
				memcpy(conig, (y2r8bit + 0x20 * choice), sizeof(struct __scal_matrix4x4));
			}
		} else {
			if (write) {
				memcpy((ir2y8bit + 0x20 * choice), conig, sizeof(struct __scal_matrix4x4));
			} else {
				memcpy(conig, (ir2y8bit + 0x20 * choice), sizeof(struct __scal_matrix4x4));
			}
		}
	}
} */
