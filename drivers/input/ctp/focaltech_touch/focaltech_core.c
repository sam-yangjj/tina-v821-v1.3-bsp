...
#include "../../sunxi_input_notify_callback.h"
...
static int fts_connect_test(struct fts_ts_data *ts_data)
{
	int ret = 0;
	int cnt = 0;
	u8 chip_id[2] = { 0 };
	...
	while (cnt < 2) {
		...
		if (ret < 0) {
			cnt++;
			msleep(50);
			FTS_DEBUG("fts_connect_test fail, retry");
		} else {
			break;
		}
	}
...
int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	ret = fts_connect_test(ts_data);
	if (ret < 0) {
		FTS_ERROR("fail fts_connect_test");
		goto err_irq_req;
	}
