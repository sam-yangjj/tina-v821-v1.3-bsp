...
static int __csi_pin_config(struct csi_dev *dev, int enable)
{
	...
	if (enable == 1) {
		//printk("pinctrl.smvs......\n");
		struct pinctrl *pctrl_smvs;
		sprintf(pinctrl_names, "%s", "csi_sm-default");
		pctrl_smvs = devm_pinctrl_get_select(&dev->pdev->dev, pinctrl_names);
		if (!IS_ERR_OR_NULL(pctrl_smvs)) {
			devm_pinctrl_put(pctrl_smvs);
		}
	} else {
		struct pinctrl *pctrl_smvs;
		sprintf(pinctrl_names, "%s", "csi_sm-sleep");
		pctrl_smvs = devm_pinctrl_get_select(&dev->pdev->dev, pinctrl_names);
		if (!IS_ERR_OR_NULL(pctrl_smvs)) {
			devm_pinctrl_put(pctrl_smvs);
		}
