...
#define TDM_INTERVAL_TIME (10*1000*1000) //10ms
...
static int tdm_rx_bufs_alloc(struct tdm_rx_dev *tdm_rx, u32 size, u32 count)
{
	...
	if (tdm_rx->large_image)
		return 0;
#elif defined BK_ONE_BUFFER_MODE
	if (tdm->use_bk_one_buffer == 2)
		return 0;
#endif
	if (tdm_rx->runtime_flag) {
		tdm_rx->runtime_flag = 0;
		return 0;
	}
	...
	/*	buf->dmabuf = ion_share_dma_buf(mm->client, mm->handle);
		...
		buf->id = i;
#elif IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
		list_add_tail(&buf->list, &tdm->rx_active[tdm_rx->id]);
		buf->id = i;
#endif
...
{
	struct tdm_dev *tdm = container_of(tdm_rx, struct tdm_dev, tdm_rx[tdm_rx->id]);
	__maybe_unused struct vin_md *vind = dev_get_drvdata(tdm->tdm_rx[0].subdev.v4l2_dev->dev);
	__maybe_unused struct vin_core *vinc = NULL;
	__maybe_unused int offset_width;
	__maybe_unused struct vin_buffer *buf;
	__maybe_unused unsigned int d3d_line_stride = 0;
	__maybe_unused unsigned int tdm_rx_size = 0;
	u32 size = 0, tatol;
	...
	if (tdm_rx->large_image) {
		for (i = 0; i < VIN_MAX_DEV; i++) {
			...
			if (vind->vinc[i]->tdm_rx_sel == tdm_rx->id && (vind->vinc[i]->id / 4 == 0)) {
				vinc = vind->vinc[i];
				if (vinc->dma_merge_mode && tdm_rx->id == 0)
					vinc = vind->vinc[i + 1];
				break;
			}
		}
		tdm_buf_num = vinc->vin_status.buf_cnt;
		if (tdm_buf_num != 1)
			vin_err("only support one buf mode\n");
	}
#elif defined BK_ONE_BUFFER_MODE
		if (tdm->use_bk_one_buffer == 2) {
			for (i = 0; i < VIN_MAX_DEV; i++) {
				...
				if (vind->vinc[i]->tdm_rx_sel == tdm_rx->id && (vind->vinc[i]->id / 4 == 0)) {
					vinc = vind->vinc[i];
					if (vinc->dma_merge_mode && tdm_rx->id % 2 == 0)
						vinc = vind->vinc[i + 1];
					break;
				}
			}
	...
	if (tdm_buf_num) {
...
#else /* not CONFIG_TDM_ONE_BUFFER_WITH_TWORX */
			ret = tdm_rx_bufs_alloc(tdm_rx, size, tdm_buf_num);
			if (ret)
				return ret;
			/* tdm need two buffer */
...
#if IS_ENABLED(CONFIG_TDM_USE_BK_BUFFER)
			for (i = 0; i < tdm_buf_num; i++) {
				buf = list_entry(vinc->vid_cap.vidq_active.next, struct vin_buffer, list);
#if IS_ENABLED(CONFIG_VIN_INIT_MELIS)
				tdm_rx->buf[i].dma_addr = buf->paddr;
#else
				tdm_rx->buf[i].dma_addr = (void *)(unsigned int)vb2_dma_contig_plane_dma_addr(&buf->vb.vb2_buf, 0);
#endif
				if (tdm_rx->large_image == 5) {
					if (tdm_rx->id == 0) {
						csic_tdm_rx_set_address(tdm->id, tdm_rx->id, (vin_dma_addr_t)tdm_rx->buf[i].dma_addr);
						csic_tdm_rx_set_address(tdm->id, tdm_rx->id, (vin_dma_addr_t)tdm_rx->buf[i].dma_addr);
					} else if (tdm_rx->id == 1) {
						offset_width = ALIGN(tdm_rx->width * tdm_rx->tdm_fmt->input_bit_width, 512) / 8 * tdm_rx->height + ALIGN(tdm_rx->height, 64);
						csic_tdm_rx_set_address(tdm->id, tdm_rx->id, (vin_dma_addr_t)tdm_rx->buf[i].dma_addr + offset_width);
						csic_tdm_rx_set_address(tdm->id, tdm_rx->id, (vin_dma_addr_t)tdm_rx->buf[i].dma_addr + offset_width);
					}
				} else if (tdm_rx->large_image == 3) {
					if (tdm_rx->id == 0) {
						offset_width = ALIGN(tdm_rx->format.width, VIN_ALIGN_WIDTH) * ALIGN(tdm_rx->format.height, VIN_ALIGN_HEIGHT) -
							ALIGN((ALIGN(tdm_rx->width * tdm_rx->tdm_fmt->input_bit_width, 512) / 8 * tdm_rx->height + ALIGN(tdm_rx->height, 64)), 4096) - 4096;
						csic_tdm_rx_set_address(tdm->id, tdm_rx->id, (vin_dma_addr_t)tdm_rx->buf[i].dma_addr + offset_width);
						if (tdm_buf_num == 1)
							csic_tdm_rx_set_address(tdm->id, tdm_rx->id, (vin_dma_addr_t)tdm_rx->buf[i].dma_addr + offset_width);
					} else if (tdm_rx->id == 1) {
						offset_width = ALIGN(tdm_rx->format.width, VIN_ALIGN_WIDTH) * ALIGN(tdm_rx->format.height, VIN_ALIGN_HEIGHT) * 12 / 8;
						csic_tdm_rx_set_address(tdm->id, tdm_rx->id, (vin_dma_addr_t)tdm_rx->buf[i].dma_addr + offset_width);
						if (tdm_buf_num == 1)
							csic_tdm_rx_set_address(tdm->id, tdm_rx->id, (vin_dma_addr_t)tdm_rx->buf[i].dma_addr + offset_width);
					}
				...
				list_move_tail(&buf->list, &vinc->vid_cap.vidq_active);
			}
#elif defined BK_ONE_BUFFER_MODE && IS_ENABLED(CONFIG_VIDEO_SUNXI_VIN_SPECIAL)
			for (i = 0; i < tdm_buf_num; i++) {
				if (tdm->use_bk_one_buffer == 2 && vinc) {
					buf = list_entry(vinc->vid_cap.vidq_active.next, struct vin_buffer, list);
					d3d_line_stride = roundup(tdm_rx->width * 12, 512) / 32;
					tdm_rx_size = tdm_rx->height / 2 * (((roundup(tdm_rx->width * 12, 512) / 32) << 2) * 2);
					if (vinc->dma_merge_mode) {
						tdm_rx_size *= 2;
					}
					if (vinc->vid_cap.special_active == 1) {
						if (vinc->dma_merge_mode && tdm_rx->id % 2)
							tdm_rx->buf[i].dma_addr = buf->paddr_uv + tdm_rx_size / 2 + (224 - 196) * ((d3d_line_stride << 2) * 2);
						else
							tdm_rx->buf[i].dma_addr = buf->paddr_uv + 224 * ((d3d_line_stride << 2) * 2);
					} else
		...
		tdm_rx->bk_cap_once_ready = false;
		tdm_rx->set_buf_cnt = 0;
#if IS_ENABLED(CONFIG_TDM_OFFLINE_HANDLE_RAW) || IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
		INIT_LIST_HEAD(&tdm->rx_done[tdm_rx->id]);
#endif
	}
...
#elif IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
		ini_en |= RX0_FRM_DONE_INT_EN | RX1_FRM_DONE_INT_EN;
#endif
...
static void tdm_set_rx_data_rate(struct tdm_rx_dev *tdm_rx, unsigned int en)
{
	...
	if (tdm->user_valid_set) {
		vin_print("It will use user tdm tx cfg, valid_num = %d, invalid_num = %d\n", tdm->tx_cfg.valid_num, tdm->tx_cfg.invalid_num);
		return;
	}
	...
	if (en) {
		...
		if (tdm_rx->large_image == 3)
			width[tdm_rx->id] = tdm_rx->format.width / 2 + vin_set_large_overlayer(tdm_rx->format.width);
		else
	...
	vin_print("tdm_rx%d %s, size is %dfps@%dx%d, isp_clk is %d\n",
		...
		fps[tdm_rx->id], width[tdm_rx->id], height[tdm_rx->id],
		tdm_rx->isp_clk);
	list_for_each_entry(chn_fmt, &tdm->working_chn_fmt, list) {
		if (chn_fmt->rx_dev->id == tdm_rx->id)
			continue;
		rx_stream[chn_fmt->rx_dev->id] = true;
		if (chn_fmt->rx_dev->large_image == 3)
			width[chn_fmt->rx_dev->id] = chn_fmt->rx_dev->format.width / 2 + vin_set_large_overlayer(chn_fmt->rx_dev->format.width);
		else
		...
		vin_print("at the same time tdm_rx%d %s, size is %dfps@%dx%d, vts is %d\n",
				...
				fps[chn_fmt->rx_dev->id], width[chn_fmt->rx_dev->id], height[chn_fmt->rx_dev->id],
				vts[chn_fmt->rx_dev->id]);
	}
	for (i = 0; i < TDM_RX_NUM; i++) {
...
#endif
	tdm->tx_cfg.valid_num += tdm->valid_num_offset;
...
{
#if IS_ENABLED(CONFIG_TDM_OFFLINE_HANDLE_RAW) && !defined CONFIG_ISP_SERVER_MELIS /* send buf to linux user space */
...
#ifdef CONFIG_SOFTWARE_TDM_FOR_ISP
static int tdm_check_sesnor_embed_data(struct tdm_rx_dev *tdm_rx, void *embed_data)
{
	struct vin_md *vind = dev_get_drvdata(tdm_rx->subdev.v4l2_dev->dev);
	struct vin_core *vinc = NULL;
	int i;
	int ret = 0;
	...
	for (i = 0; i < VIN_MAX_DEV; i++) {
		...
		vinc = vind->vinc[i];
		if (vinc->tdm_rx_sel == tdm_rx->id)
			break;
	}
	if (vinc != NULL)
		ret = v4l2_subdev_call(vinc->vid_cap.pipe.sd[VIN_IND_SENSOR], core, ioctl,
					CHECK_SENSOR_EMBED_DATA, embed_data);

	return ret;
}

static void tdm_set_isp_handle_embed(struct tdm_rx_dev *tdm_rx, bool embed_mark)
{
	struct vin_md *vind = dev_get_drvdata(tdm_rx->subdev.v4l2_dev->dev);
	struct vin_core *vinc = NULL;
	int i;
	...
	if (vinc != NULL)
		isp_update_load_for_embed(vinc->vid_cap.pipe.sd[VIN_IND_ISP], embed_mark);
}
#endif
...
static irqreturn_t tdm_isr(int irq, void *priv)
{
	...
	static int reset_task_num;
	__maybe_unused unsigned int i;
	__maybe_unused unsigned int delay_cnt = 10;
	...
	if (status.rx0_frm_done) {
	 ...
	 IS_ENABLED(CONFIG_ARCH_SUN65IW1)
...
#if IS_ENABLED(CONFIG_TDM_USE_BK_BUFFER)
		if (tdm->tdm_rx[0].large_image == 5) {
			tdm->tdm_rx[0].timestamp = ktime_get_boottime();
			if ((csic_tdm_rx_get_ctrl_st(tdm->id, 1) == 0x4) && !csic_tdm_int_sel_get_status(tdm->id, RX1_FRM_DONE_INT_EN) &&
				(abs(tdm->tdm_rx[0].timestamp - tdm->tdm_rx[1].timestamp) > TDM_INTERVAL_TIME)) { //10ms
					tdm->tdm_rx[0].pic_err = 1;
					csic_tdm_rx_cap_enable(tdm->id, 0);
					csic_tdm_rx_cap_enable(tdm->id, 1);
					vin_print("tdm_rx1 frame loss\n");
			}
		}
#endif
...
#if defined BK_ONE_BUFFER_MODE
		if (tdm->use_bk_one_buffer == 2) {
			if (tdm->tdm_rx[0].cap_once_ready) {
				csic_tdm_rx_cap_disable(tdm->id, 0);
				//vin_print("close rx0 cap\n");
			} else if (tdm->tdm_rx[0].get_cap_once) {
				tdm->tdm_rx[0].cap_once_ready = true;
				tdm->tdm_rx[0].bk_cap_once_ready = true;
			}
		}
#endif

#else /* IS_ENABLED(CONFIG_TDM_OFFLINE_HANDLE_RAW) or IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP) */
#if IS_ENABLED(CONFIG_TDM_OFFLINE_HANDLE_RAW)
		if (tdm->tx_next_id == 0xff) {
			tdm->tx_next_id = 0;
		}
...
#elif IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
		if (list_empty(&tdm->rx_active[0])) {
			vin_warn("no buffer left for tdm_rx0\n");
			spin_unlock_irqrestore(&tdm->slock, flags);
			return IRQ_HANDLED;
		}
		vin_log(VIN_LOG_TDM, "tdm%d rx0 frame done!\n", tdm->id);
		...
		if (!tdm->tx_busy || ((&tdm->rx_active[0]) == tdm->rx_active[0].next->next)) {
			buf = list_entry(tdm->rx_done[0].next, struct tdm_buffer, list);
			vin_log(VIN_LOG_TDM, "tdm rx0 send buf%d\n", buf->id);

			if (tdm->tdm_rx[1].subdev.entity.stream_count && !list_empty(&tdm->rx_done[1])) {
				buf = list_entry(tdm->rx_done[1].next, struct tdm_buffer, list);
				tdm_set_isp_handle_embed(&tdm->tdm_rx[1], buf->embed_mask);
			}
	...
	if (status.rx1_frm_done) {
	 ...
	 IS_ENABLED(CONFIG_ARCH_SUN65IW1)
...
#if IS_ENABLED(CONFIG_TDM_USE_BK_BUFFER)
		if (tdm->tdm_rx[1].large_image == 5) {
			tdm->tdm_rx[1].timestamp = ktime_get_boottime();
			if ((csic_tdm_rx_get_ctrl_st(tdm->id, 0) == 0x4) && !csic_tdm_int_sel_get_status(tdm->id, RX0_FRM_DONE_INT_EN) &&
				(abs(tdm->tdm_rx[0].timestamp - tdm->tdm_rx[1].timestamp) > TDM_INTERVAL_TIME)) { //10ms
					csic_tdm_rx_cap_disable(tdm->id, 1);
					csic_tdm_rx_disable(tdm->id, 1);
					csic_tdm_rx_enable(tdm->id, 1);
					csic_tdm_rx_cap_enable(tdm->id, 1);
					vin_print("tdm_rx0 frame loss\n");
			}
		}
#endif
...
#if defined BK_ONE_BUFFER_MODE
		if (tdm->use_bk_one_buffer == 2) {
			if (tdm->tdm_rx[1].cap_once_ready) {
				csic_tdm_rx_cap_disable(tdm->id, 1);
				//vin_print("close rx1 cap\n");
			} else if (tdm->tdm_rx[1].get_cap_once && tdm->tdm_rx[0].cap_once_ready) {
				tdm->tdm_rx[1].cap_once_ready = true;
				tdm->tdm_rx[1].bk_cap_once_ready = true;
			}
		}
#endif
		...
		vin_log(VIN_LOG_TDM, "tdm%d rx1 frame done! tx_next_id is %d\n", tdm->id, tdm->tx_next_id);
...
#elif IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
		if (list_empty(&tdm->rx_active[1])) {
			vin_warn("no buffer left for tdm_rx1\n");
			spin_unlock_irqrestore(&tdm->slock, flags);
			return IRQ_HANDLED;
		}
		buf = list_entry(tdm->rx_active[1].next, struct tdm_buffer, list);
		if (tdm_check_sesnor_embed_data(&tdm->tdm_rx[1], buf->virt_addr) == 1) {
			buf->embed_mask = true;
			vin_log(VIN_LOG_TDM, "tdm rx1 buf%d for embed\n", buf->id);
		} else {
			buf->embed_mask = false;
			vin_log(VIN_LOG_TDM, "tdm rx1 buf%d for not embed\n", buf->id);
		}
		list_move_tail(&buf->list, &tdm->rx_done[1]);

		if (!tdm->tx_busy || ((&tdm->rx_active[1]) == tdm->rx_active[1].next->next)) {
			buf = list_entry(tdm->rx_done[1].next, struct tdm_buffer, list);
			vin_log(VIN_LOG_TDM, "tdm rx1 send buf%d data%d\n", buf->id, buf->embed_mask);
			tdm_set_isp_handle_embed(&tdm->tdm_rx[1], buf->embed_mask);
			csic_tdm_tx_cap_enable(tdm->id);
			for (i = 0; i < delay_cnt; i++)
				;
