...
{
#if defined VIPP_200 && !IS_ENABLED(CONFIG_ARCH_SUN55IW3) && !IS_ENABLED(CONFIG_ARCH_SUN60Iw1)
...
#if IS_ENABLED(CONFIG_MULTI_FRAME) || IS_ENABLED(CONFIG_MULTI_FRM_MERGE_INT)
...
static int __vin_pipeline_s_stream(struct vin_pipeline *p, int on_idx)
{
	...
	for (i = 0; i < VIN_IND_ACTUATOR; i++) {
		...
		if (vinc->ptn_cfg.ptn_en && (idx <= VIN_IND_MIPI))
			continue;
		if (vinc->large_image == 3 && vinc->id % 4 == 0 && (idx == VIN_IND_SENSOR || idx == VIN_IND_CSI))
			continue;
