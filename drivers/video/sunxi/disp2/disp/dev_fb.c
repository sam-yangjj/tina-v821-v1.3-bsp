...
#define dma_mmap_writecombine dma_mmap_wc
	...
	IS_ENABLED(CONFIG_AW_DISP2_FB_HW_ROTATION_SUPPORT)
...
#endif
	if (info->screen_base) {
		DE_INFO("(reserve),va=0x%p, pa=0x%p size:0x%x\n",
...
{
	if (!info->screen_base) {
		DE_WARN("screen_base is null\n");
		return;
	}
	DE_INFO("screen_base=0x%p, smem=0x%p, len=0x%x\n",
	      (void *)info->screen_base,
			  ...
			  struct fb_fix_screeninfo *fix)
{
	if (var->nonstd == 0) {
		...
		default:
			DE_WARN("invalid bits_per_pixel :%d\n",
			      var->bits_per_pixel);
			return -EINVAL;
		}
	}
			      ...
			      struct fb_info *info)
{
	...
	case FB_VISUAL_TRUECOLOR:
		if (regno < 16) {
			...
			DE_INFO("regno=%2d,a=%2X,r=%2X,g=%2X,b=%2X,result=%08X\n",
...
static int sunxi_fb_setcmap(struct fb_cmap *cmap, struct fb_info *info)
{
	...
	DE_INFO("Fb_setcmap, cmap start:%d len:%d, %dbpp\n", cmap->start,
	...
	case FBIO_FREE:
		{
			...
			if ((!g_fbi.fb_enable[fb_id])
			    || (fbinfo != info)) {
				DE_WARN("fb%d already release ? or fb_info mismatch: fbinfo=0x%p, info=0x%p\n",
				      fb_id, fbinfo, info);
				return -1;
			}
	...
	case FBIO_ALLOC:
		{
			...
			if (g_fbi.fb_enable[fb_id]
			    || (fbinfo != info)) {
				DE_WARN("fb%d already enable ? or fb_info mismatch: fbinfo=0x%p, info=0x%p\n",
						 fb_id, fbinfo, info);
				return -1;
			}
...
static int Fb_copy_boot_fb(u32 sel, struct fb_info *info)
{
	...
	if (boot_fb_str != NULL) {
		...
		if (sizeof(boot_fb) - 1 < len) {
			DE_WARN("need bigger array size[%d] for boot_fb\n", len);
			return -1;
		}
	...
	if (src_height < dst_height) {
		...
		DE_WARN("src_height(%d) > dst_height(%d),please cut the height\n",
		      src_height,
		      dst_height);
...
int lzma_decode(uintptr_t paddr, struct fb_info *info)
{
	...
	if (!out) {
		DE_WARN("kmalloc outbuffer fail\n");
		goto OUT;
	}
	...
	if ((bmp_bpix != 3) && (bmp_bpix != 4)) {
		DE_WARN("not support bmp format:%d\n", bmp_bpix);
		goto UMAPKERNEL;
	}
...
static int Fb_map_kernel_logo(u32 sel, struct fb_info *info)
{
	...
	paddr = bootlogo_addr;
	if (paddr == 0) {
		DE_INFO("Fb_map_kernel_logo failed!");
	...
	fb_height = info->var.yres;
	if ((paddr <= 0) || x <= 1 || y <= 1) {
		DE_WARN("kernel logo para error\n");
		return -EINVAL;
	}
...
static s32 display_fb_request(u32 fb_id, struct disp_fb_create_info *fb_para)
{
...
#if IS_ENABLED(CONFIG_ARCH_SUN8IW12) || IS_ENABLED(CONFIG_ARCH_SUN8IW16)\
	...
	if (g_fbi.fb_enable[fb_id]) {
		DE_WARN("fb%d is already requested\n", fb_id);
		return -1;
	}
	...
	if ((xres == 0) || (yres == 0) || (info->var.bits_per_pixel == 0)) {
		DE_WARN("invalid paras xres(%d), yres(%d) bpp(%d)\n", xres, yres,
		      info->var.bits_per_pixel);
		return -1;
	}
...
s32 fb_init(struct platform_device *pdev)
{
	...
	if (g_disp_drv.disp_init.b_init) {
		...
		for (i = 0; i < SUNXI_FB_MAX; i++) {
			...
			if (ret)
				break;
#if IS_ENABLED(CONFIG_AW_DISP2_BOOT_COLORBAR)
