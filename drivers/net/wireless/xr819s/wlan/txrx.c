...
#ifdef XRADIO_RAW_FRM_SUP
			mgmt->u.action.category == WLAN_CATEGORY_BACK && t->raw_frm_flag == 0)
#else
...
void xradio_tx(struct ieee80211_hw *dev, struct sk_buff *skb)
{
...
#endif
#ifdef XRADIO_RAW_FRM_SUP
		.raw_frm_flag = skb->cb[sizeof(skb->cb) -1], // set in xrmac_send_raw_frm();
#endif
	};
	...
	if (ieee80211_is_action(frame->frame_control) &&
		...
		/* category & 0x80 is true, indicating that there is
		 * an exception in this action framie
		 */
		ret = __LINE__;
		goto drop;
	}
...
#ifdef XRADIO_RAW_FRM_SUP
	if (t.raw_frm_flag == 1) {
		wsm->reserved |= WSM_TX_SEND_RAW_FRAME;
		txrx_printk(XRADIO_DBG_NIY, "%s__%d: tx raw frame\n", __func__, __LINE__);
	}
#endif
#ifdef CONFIG_XRADIO_TESTMODE
...
#ifdef XRADIO_RAW_FRM_SUP
	if (arg->flags & WSM_TXC_F_IS_RAW_FRM) {
		txrx_printk(XRADIO_DBG_NIY, "%s: txed raw frame, status = 0x%x, flag = 0x%x, pkID = %u, hw_pkId = %u\n",
			    __func__, arg->status, arg->flags, arg->packetID, hw_priv->raw_packetID);
		if ((arg->if_id >= 0 && arg->if_id < XRWL_MAX_VIFS) && hw_priv->raw_packetID == arg->packetID) {
			txrx_printk(XRADIO_DBG_NIY, "updata raw tx status.\n");
			xrmac_set_raw_frm_tx_status(hw_priv->vif_list[arg->if_id], arg->status);
		}
	}
#endif
...
#endif
#ifdef XRADIO_RAW_FRM_SUP
extern u32 rx_spex_enable;
int xradio_rx_spec_frame(struct xradio_common *hw_priv, u8 *skb_data, u32 skb_len);
#endif
		  ...
		  struct sk_buff **skb_p)
{
	...
	if (rx_spex_enable) {
		...
		if (arg->flags & WSM_RX_STATUS_RECV_FILTER_FRAME) {
			skb_push(skb, sizeof(u8));
			tmp_data = *(u8 *)skb->data;
			memcpy(skb->data, &(arg->rcpiRssi), sizeof(u8));
			xradio_rx_spec_frame(hw_priv, skb->data, skb->len);
			memcpy(skb->data, &tmp_data, sizeof(u8));
			skb_pull(skb, sizeof(u8));
			txrx_printk(XRADIO_DBG_NIY, "%s:data = 0x%x rssi = %d\n",
				    __func__, *(u8 *)skb->data, (s8)arg->rcpiRssi);
		}
	}
#endif
