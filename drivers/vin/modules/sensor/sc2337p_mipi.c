...
/*
 * A V4L2 driver for sc2337p Raw cameras.
 *
...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for sc2337p sensors");
MODULE_LICENSE("GPL");
...
#if IS_ENABLED(CONFIG_SENSOR_SC2337P_STREAMON_TOGETHER)
struct v4l2_subdev *sd_master;
#endif

#if IS_ENABLED(CONFIG_SENSOR_SC2337P_SYNCHR_OUTPUT)
...
static struct regval_list sensor_1080p_15fps_regs[] = {
	...
	{0x37f9, 0x27},
	{0x3021, 0x86},
#ifndef CONFIG_SENSOR_SC2337P_SYNCHR_OUTPUT
	{0x0100, 0x01},
#endif
};
...
static int sc2337p_fps_change_flag;
...
//	data_type read_high = 0, read_low = 0;
	int sc2337p_sensor_target_vts = 0;

#ifndef CONFIG_SENSOR_SC2337P_SYNCHR_OUTPUT
	sc2337p_fps_change_flag = 1;
	sc2337p_sensor_target_vts = wsize->pclk/fps->fps/wsize->hts;

	if (sc2337p_sensor_target_vts <= wsize->vts) {// the max fps = 20fps
		sc2337p_sensor_target_vts = wsize->vts;
	} else if (sc2337p_sensor_target_vts >= (wsize->pclk/wsize->hts)) {// the min fps = 1fps
		sc2337p_sensor_target_vts = (wsize->pclk/wsize->hts) - 8;
	}

	sc2337p_sensor_vts = sc2337p_sensor_target_vts;
	sensor_dbg("target_fps = %d, sc2337p_sensor_target_vts = %d, 0x320e = 0x%x, 0x320f = 0x%x\n", fps->fps,
		sc2337p_sensor_target_vts, sc2337p_sensor_target_vts >> 8, sc2337p_sensor_target_vts & 0xff);
	sensor_write(sd, 0x320f, (sc2337p_sensor_target_vts & 0xff));
	sensor_write(sd, 0x320e, (sc2337p_sensor_target_vts >> 8));
//	sensor_read(sd, 0x320e, &read_high);
//	sensor_read(sd, 0x320f, &read_low);
//	sensor_print("[get write_times] 0x41 = 0x%x, 0x42 = 0x%x\n", read_high, read_low);
	sc2337p_fps_change_flag = 0;
#endif
	return 0;
}
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	SENSOR_ID |= (rdval);
	sensor_print("sc2337p V4L2_IDENT_SENSOR = 0x%x\n", SENSOR_ID);
	...
	sensor_read(sd, 0x801e, &rdval);
	sensor_print("0x801e = %x\n", rdval);
...
static int sensor_reg_init(struct sensor_info *info)
{
...
#if IS_ENABLED(CONFIG_SENSOR_SC2337P_STREAMON_IN_KERNEL)
#if IS_ENABLED(CONFIG_SENSOR_SC2337P_STREAMON_TOGETHER)
		if (!strcmp(sd->name, SENSOR_NAME)) {
			sd_master = sd;
			sensor_print("get %s sd\n", sd->name);
		}
		if (!strcmp(sd->name, SENSOR_NAME_2)) {
			/* stream on */
			sensor_write(sd, 0x0100, 0x01);
			sensor_write(sd_master, 0x0100, 0x01);
			sensor_print("%s %s do_streamon_together\n", sd_master->name, sd->name);
		}
#else
		/* stream on */
		sensor_write(sd, 0x0100, 0x01);
	...
	sc2337p_sensor_vts = wsize->vts;
	sc2337p_fps_change_flag = 0;
#if IS_ENABLED(CONFIG_SENSOR_SC2337P_SYNCHR_OUTPUT)
	sc2337p_synchr_max_exp_line = wsize->intg_max;
	sensor_print("%s  sc2337p_synchr_max_exp_line = %d\n", sd->name, sc2337p_synchr_max_exp_line);
#endif
