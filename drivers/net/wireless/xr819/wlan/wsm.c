...
int wsm_etf_cmd(struct xradio_common *hw_priv, struct wsm_hdr *arg)
{
	...
	if (unlikely(hw_priv->bh_error)) {
		...
		} while (!ret && time_before_eq(jiffies, hw_priv->rx_timestamp + tmo) &&
			  ...
			  int interface_link_id)
{
	...
	if (is_hardware_xradio(hw_priv)) {
		...
		tx_confirm.link_id = xradio_queue_get_link_id(tx_confirm.packetID);
	} else {
...
int wsm_add_key(struct xradio_common *hw_priv, const struct wsm_add_key *arg, int if_id)
{
	int ret;
	struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;
				  ...
				  struct wsm_buf *buf)
{
	...
	if (hw_priv->hw_revision == XR819_HW_REV1) {
		WSM_GET(buf, hw_priv->wsm_caps.efuse_mac, sizeof(hw_priv->wsm_caps.efuse_mac));
...
void wsm_send_deauth_to_self(struct xradio_common *hw_priv, struct xradio_vif *priv)
{
...
void wsm_send_disassoc_to_self(struct xradio_common *hw_priv, struct xradio_vif *priv)
{
	struct sk_buff *skb = NULL;
		...
		* convert RCPI to RSSI here */
		...
		if (!rx.status && unlikely(ieee80211_is_deauth(hdr->frame_control))) {
			if (priv->join_status == XRADIO_JOIN_STATUS_STA) {
				/* Shedule unjoin work */
				wsm_printk(XRADIO_DBG_WARN, "Issue unjoin command (RX).\n");
...
static int wsm_channel_switch_indication(struct xradio_common *hw_priv, struct wsm_buf *buf)
{
...
static int wsm_set_pm_indication(struct xradio_common *hw_priv, struct wsm_buf *buf)
{
	wsm_oper_unlock(hw_priv);
	return 0;
}

static int wsm_scan_complete_indication(struct xradio_common *hw_priv, struct wsm_buf *buf)
{
#ifdef ROAM_OFFLOAD
...
static int wsm_find_complete_indication(struct xradio_common *hw_priv, struct wsm_buf *buf)
{
	/* TODO: Implement me. */
	/*STUB();*/
	return 0;
...
void wsm_vif_lock_tx(struct xradio_vif *priv)
{
	...
	if (atomic_add_return(1, &hw_priv->tx_lock) == 1) {
		if (wsm_vif_flush_tx(priv))
			wsm_printk(XRADIO_DBG_MSG, "TX is locked for if_id %d.\n", priv->if_id);
	}
	up(&hw_priv->tx_lock_sem);
}
...
bool wsm_flush_tx(struct xradio_common *hw_priv)
{
	...
	if (hw_priv->bh_error) {
		...
		wsm_printk(XRADIO_DBG_ERROR, "Fatal error occured, will not flush TX.\n");
		return false;
	} else {
...
bool wsm_vif_flush_tx(struct xradio_vif *priv)
{
	...
	if (hw_priv->bh_error) {
		...
		while (xradio_query_txpkt_timeout(hw_priv, if_id,
			...
			ret = wait_event_timeout(hw_priv->bh_evt_wq,
						 !hw_priv->hw_bufs_used_vif[if_id], timeout);
			if (ret > 0) {
				return true;
			}
		}
...
static int wsm_debug_indication(struct xradio_common *hw_priv, struct wsm_buf *buf)
{
...
/***************************for HWT ********************************/
	...
	if (id == 0x404) {
		...
		case 0x041C: /* map_link */
			...
			if (ret)
				wsm_printk(XRADIO_DBG_ERROR,
					   "wsm_generic_confirm failed for request 0x%.4X.\n",
					   id & ~0x0400);
			break;
		default:
			SYS_BUG(1);
			       ...
			       struct xradio_queue *queue)
{
		...
		if (unlikely(priv->bss_loss_status == XRADIO_BSS_LOSS_CHECKING &&
			...
			if (ieee80211_is_auth(fctl)) {
				...
				action = doTx;
				   ...
				   (priv->join_status ==
				   XRADIO_JOIN_STATUS_STA) && (ieee80211_is_data(fctl))) {
				action = doDrop;
	...
	case NL80211_IFTYPE_AP:
		...
		if (xradio_queue_get_generation(wsm->packetID) > XRADIO_MAX_REQUEUE_ATTEMPTS) {
			...
			wsm_printk(XRADIO_DBG_WARN,
				   "Too many attempts to requeue a frame. Frame is dropped, fctl=0x%04x.\n", fctl);
			action = doDrop;
		}
		break;
...
static int xradio_get_prio_queue(struct xradio_vif *priv, u32 link_id_map, int *total)
{
	struct xradio_common *hw_priv = xrwl_vifpriv_to_hwpriv(priv);
	static u32 urgent;
	...
	for (i = 0; i < 4; ++i) {
		queued = xradio_queue_get_num_queued(priv, &hw_priv->tx_queue[i], link_id_map);
		if (!queued)
			continue;
		*total += queued;
		...
		*vif_selected = -1;
		...
		for (;;) {
			...
			* packet needs to be found */
			...
			if (xradio_queue_get(queue, priv->if_id, tx_allowed_mask,
					     &wsm, &tx_info, &txpriv)) {
				wsm_printk(XRADIO_DBG_WARN,
					   "%s, if_id=%d(enable=%d), tx_allowed_mask=%08x, queue_num=%d, queued_item=%zd, pending_item=%zd link_id_after_dtim=%d, link_id_uapsd=%d, tx_multicast=%d, pspoll_mask=%d, sta_asleep_mask=%d\n",
					   __func__, priv->if_id, atomic_read(&priv->enabled),
					   tx_allowed_mask, queue_num, queue->num_queued,
					   queue->num_pending, priv->link_id_after_dtim,
					   priv->link_id_uapsd, priv->tx_multicast,
					   priv->pspoll_mask, priv->sta_asleep_mask);
				spin_unlock(&priv->vif_lock);
				if_pending = 0;
				continue;
			}
...
#ifndef P2P_MULTIVIF
			{
				...
				wsm_printk(XRADIO_DBG_ERROR, "QGET-1 %x, off_id %d, if_id %d\n",
					   hdr->frame_control,
...
#ifdef P2P_MULTIVIF
			if (txpriv->raw_if_id)
				wsm->hdr.id |= cpu_to_le16(WSM_TX_IF_ID(txpriv->raw_if_id));
#else
			if (txpriv->offchannel_if_id)
				wsm->hdr.id |= cpu_to_le16(WSM_TX_IF_ID(txpriv->offchannel_if_id));
#endif
			else
			...
			*vif_selected = priv->if_id;
...

			{
				...
				wsm_printk(XRADIO_DBG_ERROR, "QGET-2 %x, off_id %d, if_id %d\n",
					   hdr->frame_control,
					   txpriv->offchannel_if_id,
					   priv->if_id);
			}
			...
			*tx_len = __le16_to_cpu(wsm->hdr.len);
			...
			if (priv->edca.params[queue_num].txOpLimit)
				*burst = min(*burst, (int)xradio_queue_get_num_queued(priv,
