...
unsigned long de_peak_get_reg_base(int sel)
{

	struct de_peak_private *priv = &peak_priv[sel][0];
	struct de_reg_block *reg_blk = &(priv->reg_blks[PEAK_PEAK_REG_BLK]);
	return (unsigned long)reg_blk->reg_addr;
}
