...
static int  etf_check_version(u32 ver)
{
	...
	if (main_ver > REF_ETF_MAIN_VER) {
		...
		return 1;
	} else if (main_ver == REF_ETF_MAIN_VER &&
		   sub_ver >= REF_ETF_SUB_VER &&
	...
	case ETF_GET_SDD_POWER_DEFT:  /* get default tx power */
		{
			...
			if (likely(sdd) && likely(sdd->data)) {
				...
				void *ie_data = NULL;
				int ie_len = etf_get_sdd_param((u8 *)sdd->data,
							       sdd->size,
							       SDD_MAX_OUTPUT_POWER_2G4_ELT_ID,
							       &ie_data);
				if (ie_len > 0 && ie_data &&
	...
	case ETF_GET_SDD_PARAM_ID:
		{
			...
			if (likely(sdd) && likely(sdd->data)) {
				if (sddreq->flags & FLAG_GET_SDD_ALL) {
					...
					xradio_adapter_send_pkg(sddret, sizeof(*sddret),
								(u8 *)sdd->data,
								sddret->length);
				} else {
	...
	case ETF_SET_CLI_PAR_DEFT:
		{
			...
			if (etf_check_version(hdr_data->version)) {
				if (etf_priv.cli_data && hdr_data->data_len ==
				    etf_priv.cli_data_len) {
					...
					etf_printk(XRADIO_DBG_ALWY,
						...
						etf_priv.cli_data_len, hdr_data->data_len);
