...
#if IS_ENABLED(CONFIG_AW_SPIF_DELAYINIT)
...
static void sunxi_spif_set_clk(u32 spif_clk, struct sunxi_spif *sspi)
{
	clk_set_rate(sspi->mclk, spif_clk);
	if (clk_get_rate(sspi->mclk) != spif_clk) {
		sunxi_debug(&sspi->pdev->dev,
				"set spif clock %d failed, use clk:%ld\n",
				spif_clk, clk_get_rate(sspi->mclk));
	} else {
...
static void sunxi_spif_soft_reset(struct sunxi_spif *sspi)
{
	...
	while (timeout) {
		if (!(readl(sspi->base_addr + SPIF_GCA_REG) & SPIF_RESET))
			break;
		timeout--;
	}

	sunxi_err(&sspi->pdev->dev, "%s timeout\n", __func__);
}
...
static void sunxi_spif_config_tc(struct sunxi_spif *sspi)
{
	if (sspi->data->sample_mode != SAMP_MODE_DL_DEFAULT) {
		sunxi_spif_set_clk(sspi->data->max_speed_hz, sspi);
		double_clk_flag = 0;
				...
				boot_spinor_info_t *boot_info)
{
	...
	if (!len_ok) {
		...
		/* default clock */
		sunxi_spif_set_clk(25000000, sspi);
		double_clk_flag = 0;
...
static irqreturn_t sunxi_spif_handler(int irq, void *dev_id)
{
	...
	enable = readl(sspi->base_addr + SPIF_INT_EN_REG);
...
static void sunxi_spif_dtr_enable(struct sunxi_spif *sspi, struct spi_mem_op *op)
{
	...
	if (CHECK_DTR_OP(op->cmd.opcode)) {
		if (!double_clk_flag) {
			...
			sunxi_spif_set_clk(dtr_double_clk, sspi);
			double_clk_flag = 1;
		}
	} else {
		if (double_clk_flag) {
...
*/
	...
	ret = of_hwspin_lock_get_id_byname(np, "hwlock");
	if (ret >= 0) {
		sunxi_info(&sspi->pdev->dev, "hwspinlock%d enabled\n", ret);
		sspi->hwlock = devm_hwspin_lock_request_specific(&sspi->pdev->dev, ret);
		ret = of_property_read_u32(np, "hwlock-timeout_ms", &sspi->hwlock_timeout_ms);
		if (ret) {
			sspi->hwlock_timeout_ms = HWLOCK_TIMEOUT_MS_DEFAULT;
		}
	} else
		sspi->hwlock = NULL;
...
static void sunxi_spif_resource_put(struct sunxi_spif *sspi)
{
	...
	if (sspi->hwlock) {
		devm_hwspin_lock_free(&sspi->pdev->dev, sspi->hwlock);
		sspi->hwlock = NULL;
	}
}
...
static void sunxi_spif_init_hwcaps(struct sunxi_spif *sspi, struct spi_nor_hwcaps *hwcaps)
{
	struct device_node *np = sspi->pdev->dev.of_node;
	u32 bus_width;
	u32 io_mode = 0, dtr_mode = 0;

	if (of_property_read_bool(np, "io_mode_enabled")) {
		sunxi_debug(&sspi->pdev->dev, "IO mode enabled");
		io_mode = 1;
	}

	if (of_property_read_bool(np, "dtr_mode_enabled")) {
		sunxi_debug(&sspi->pdev->dev, "DTR mode enabled");
		dtr_mode = 1;
	}
...
static int sunxi_spif_nor_read_reg(struct spi_nor *nor, u8 opcode, u8 *buf, int len)
{
	struct sunxi_spif *sspi = nor->priv;
...
static int sunxi_spif_nor_write_reg(struct spi_nor *nor, u8 opcode, u8 *buf, int len)
{
	struct sunxi_spif *sspi = nor->priv;
...
ssize_t sunxi_spif_nor_io_read(struct spi_nor *nor, loff_t from, size_t len,
				 u_char *buf)
{
	struct sunxi_spif *sspi = nor->priv;
...
static int sunxi_spif_nor_register(struct sunxi_spif *sspi)
{
	...
	nor->read_reg = sunxi_spif_nor_read_reg;
	nor->write_reg = sunxi_spif_nor_write_reg;
	nor->read = sunxi_spif_nor_read;
	nor->write = sunxi_spif_nor_write;
...
static ssize_t sunxi_spif_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE,
		...
		"sspi->irq = %d\n"
		"sspi->data->bus_num   = %d\n"
		"sspi->data->chip_select = %d\n"
		"sspi->data->min_speed_hz  = %d\n"
		"sspi->data->max_speed_hz  = %d\n"
		"sspi->sample_mode  = %#x\n"
		"sspi->sample_delay = %#x\n",
		sunxi_spif_get_version(sspi),
		pdev->id, pdev->name, pdev->num_resources,
		sspi->base_addr, sspi->irq, sspi->data->bus_num, sspi->data->chip_select,
		sspi->data->min_speed_hz, sspi->data->max_speed_hz,
		sspi->data->sample_mode, sspi->data->sample_delay);
}
...
static ssize_t sunxi_spinor_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct sunxi_spif *sspi = platform_get_drvdata(pdev);
	...
	return scnprintf(buf, PAGE_SIZE,
		...
		"hwcaps mask:0x%x\n",
		nor->params.size >> 10,
		nor->page_size, nor->addr_width, nor->flags,
		nor->read_opcode, nor->read_proto, nor->read_dummy,
		nor->program_opcode, nor->write_proto, nor->erase_opcode,
		nor->params.hwcaps.mask);
}

static struct device_attribute sunxi_spif_debug_attr[] = {
	__ATTR(info, 0444, sunxi_spif_info_show, NULL),
	__ATTR(dump, 0444, sunxi_spi_dump_show, NULL),
	__ATTR(spinor-info, 0444, sunxi_spinor_info_show, NULL),
};

static void sunxi_spif_create_sysfs(struct sunxi_spif *sspi)
{
	int i;
	for (i = 0; i < ARRAY_SIZE(sunxi_spif_debug_attr); i++)
		device_create_file(&sspi->pdev->dev, &sunxi_spif_debug_attr[i]);
}

static void sunxi_spif_remove_sysfs(struct sunxi_spif *sspi)
{
	int i;
	for (i = 0; i < ARRAY_SIZE(sunxi_spif_debug_attr); i++)
		device_remove_file(&sspi->pdev->dev, &sunxi_spif_debug_attr[i]);
}
...
static int sunxi_spif_probe(struct platform_device *pdev)
{
	...
	atomic_set(&sspi->hwlock_available, 1);

	if (sspi->hwlock) {
		if (hwspin_lock_timeout_raw(sspi->hwlock, sspi->hwlock_timeout_ms) == -ETIMEDOUT) {
			sunxi_err(&sspi->pdev->dev, "hwspinlock lock timeout\n");
			err = -EAGAIN;
			goto err5;
		}
	}
	...
	init_completion(&sspi->hwlock_unlock_done);
...
static int sunxi_spif_remove(struct platform_device *pdev)
{
	...
	sunxi_spif_remove_sysfs(sspi);
	sunxi_spif_hw_deinit(sspi);
	sunxi_spif_resource_put(sspi);
	for (i = 0; i < DESC_PER_DISTRIBUTION_MAX_NUM; i++)
		dma_pool_free(sspi->pool, sspi->dma_desc[i], sspi->desc_phys[i]);
	dmam_pool_destroy(sspi->pool);
	aw_spinor_remove_cdev();
	platform_set_drvdata(pdev, NULL);

	return 0;
}

static void sunxi_spif_unlock_timeout(struct sunxi_spif *sspi)
{
	unsigned timeout;

	if (sspi && !sspi->in_init && sspi->hwlock && (atomic_read(&sspi->hwlock_locked) == 1)) {
		timeout = wait_for_completion_timeout(&sspi->hwlock_unlock_done, msecs_to_jiffies(XFER_TIMEOUT * 2));
		if (timeout == 0) {
			sunxi_warn(&sspi->pdev->dev, "[spi-flash%d] hwspinlock is locked, unlock when shutdown or suspend\n", sspi->data->bus_num);
			hwspin_unlock_raw(sspi->hwlock);
			atomic_set(&sspi->hwlock_locked, 0);
		}
	}
...
static void sunxi_spif_shutdown(struct platform_device *pdev)
{
	struct sunxi_spif *sspi = platform_get_drvdata(pdev);

	atomic_set(&sspi->hwlock_available, 0);
	sunxi_spif_unlock_timeout(sspi);
...
#if IS_ENABLED(CONFIG_AW_SPIF_DELAYINIT)
static int sunxi_spif_rpmsg_callback(void *dev, void *data, int len)
{
	platform_driver_register(&sunxi_spif_driver);
	return 0;
}

static int __init sunxi_spif_driver_init(void)
{
	return rpmsg_notify_add(CONFIG_AW_SPIF_DELAYINIT_RPROC_NAME, "spif",
					sunxi_spif_rpmsg_callback, NULL);
}
device_initcall(sunxi_spif_driver_init);

static void __exit sunxi_spif_driver_exit(void)
{
	platform_driver_unregister(&sunxi_spif_driver);
}
module_exit(sunxi_spif_driver_exit);
#else
