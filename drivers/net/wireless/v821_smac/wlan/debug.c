...
u8 dbg_bypass  = XRADIO_DBG_DEFAULT;
u8 dbg_xradio_rpmsg = XRADIO_DBG_DEFAULT;
u8 dbg_logfile = XRADIO_DBG_ERROR;

int listen_interval;
int suspend_dtim = 3;

void xradio_dbg_dump(char *pref, int width, unsigned char *buf, int len)
{
	print_hex_dump(KERN_DEBUG, pref, DUMP_PREFIX_OFFSET, width, 4, buf, len, 0);
}
	...
	char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (tpa_info.Status && tpa_info.InfoID != 0x43) {
		...
		pos += sprintf(&buf_show[pos], "th_q=%d,th_tm=%d,th_updt=%d," \
			       "th_def_lstn=%d, th_stbl=%d, pwr_max_margin=%d\n",
			       ...
			       tpa_info.u.ctrl.pwr_max_margin);
	  for (i = 0; i < 4; i++) {
			pos += sprintf(&buf_show[pos],
...
#define PUT_PERF_INFO(tab, name) \
	...
	seq_printf(seq, "\nTime_int=%d, Tx=%d.%02dMbps, Rx=%d.%02dMbps, RealTx=%d.%02dMbps, RealRx=%d.%02dMbps\n",
		time_int*10,
...
static ssize_t xradio_pre_rx_bcn_get(struct file *file,
	char __user *user_buf, size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;*/
	char buf[100];
	...
	sprintf(buf, "enable=%d, flags=%d, stop_num=%d\n",
		pre_rx_bcn.enable, pre_rx_bcn.flags, pre_rx_bcn.stop_num);
	size = strlen(buf);

	return simple_read_from_buffer(user_buf, count, ppos, buf, size);
}

static ssize_t xradio_pre_rx_bcn_set(struct file *file,
				      ...
				      size_t count, loff_t *ppos)
{
	...
	pre_rx_bcn.enable = simple_strtoul(buf, &endptr, 16);
	start = endptr + 1;
	if (start < buf + count)
		pre_rx_bcn.flags = simple_strtoul(start, &endptr, 16);
	start = endptr + 1;
	if (start < buf + count)
		pre_rx_bcn.stop_num = simple_strtoul(start, &endptr, 16);

	xradio_dbg(XRADIO_DBG_ALWY, "enable=%d, flags=%d, stop_num=%d\n",
		pre_rx_bcn.enable, pre_rx_bcn.flags, pre_rx_bcn.stop_num);

	wsm_write_mib(hw_priv, WSM_MIB_ID_SET_PRE_RX_BEACON,
			      &pre_rx_bcn, sizeof(pre_rx_bcn), if_id);

	return count;
}

static const struct file_operations fops_pre_rx_bcn = {
	.open = xradio_generic_open,
	.write = xradio_pre_rx_bcn_set,
	.read = xradio_pre_rx_bcn_get,
	.llseek = default_llseek,
};


static ssize_t xradio_dtim_get(struct file *file,
	char __user *user_buf, size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;
	...
	sprintf(buf, "dtim_period = %d\n", suspend_dtim);
	size = strlen(buf);

	return simple_read_from_buffer(user_buf, count, ppos, buf, size);
}

static ssize_t xradio_dtim_set(struct file *file,
				    const char __user *user_buf,
				    size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;
	...
	suspend_dtim = simple_strtoul(buf, &endptr, 10);
	...
	xradio_dbg(XRADIO_DBG_ALWY, "dtim_period=%d\n", suspend_dtim);

	return count;
}

static const struct file_operations fops_dtim = {
	.open = xradio_generic_open,
	.write = xradio_dtim_set,
	.read = xradio_dtim_get,
	.llseek = default_llseek,
};



static ssize_t xradio_listen_interval_get(struct file *file,
	char __user *user_buf, size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;
	struct xradio_vif *priv = NULL;*/
	...
	sprintf(buf, "listen_interval = %d\n", listen_interval);
	size = strlen(buf);

	return simple_read_from_buffer(user_buf, count, ppos, buf, size);
}

static ssize_t xradio_listen_interval_set(struct file *file,
				    const char __user *user_buf,
				    size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;
	...
	listen_interval = simple_strtoul(buf, &endptr, 10);

	if (listen_interval < 0)
		listen_interval = 0;

	if (listen_interval > 30)
		listen_interval = 30;

	xradio_dbg(XRADIO_DBG_ALWY, "listen_interval = %d\n", listen_interval);

	return count;
}

static const struct file_operations fops_listen_interval = {
	.open = xradio_generic_open,
	.write = xradio_listen_interval_set,
	.read = xradio_listen_interval_get,
	.llseek = default_llseek,
};

int beacon_win;
static ssize_t xradio_beacon_win_get(struct file *file,
	char __user *user_buf, size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;
	struct xradio_vif *priv = NULL;*/
	...
	sprintf(buf, "beacon_win = %d\n", beacon_win);
	size = strlen(buf);

	return simple_read_from_buffer(user_buf, count, ppos, buf, size);
}

static ssize_t xradio_beacon_win_set(struct file *file,
				    const char __user *user_buf,
				    size_t count, loff_t *ppos)
{
	struct xradio_common *hw_priv = file->private_data;
	...
	beacon_win = simple_strtoul(buf, &endptr, 10);

	if (beacon_win < 0)
		beacon_win = 0;

	wsm_set_beacon_win_cfg(hw_priv, beacon_win, 0);
	xradio_dbg(XRADIO_DBG_ALWY, "beacon_win = %d\n", beacon_win);

	return count;
}

static const struct file_operations fops_beacon_win = {
	.open = xradio_generic_open,
	.write = xradio_beacon_win_set,
	.read = xradio_beacon_win_get,
	.llseek = default_llseek,
};

int null_prd;
static ssize_t xradio_null_prd_get(struct file *file,
	char __user *user_buf, size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;
	struct xradio_vif *priv = NULL;*/
	...
	sprintf(buf, "null_prd = %d\n", null_prd);
	size = strlen(buf);

	return simple_read_from_buffer(user_buf, count, ppos, buf, size);
}

static ssize_t xradio_null_prd_set(struct file *file,
				    const char __user *user_buf,
				    size_t count, loff_t *ppos)
{
	struct xradio_common *hw_priv = file->private_data;
	...
	null_prd = simple_strtoul(buf, &endptr, 10);

	if (null_prd < 0)
		null_prd = 0;

	wsm_keep_alive_period(hw_priv, null_prd, 0);
	xradio_dbg(XRADIO_DBG_ALWY, "null_prd = %d\n", null_prd);

	return count;
}

static const struct file_operations fops_null_prd = {
	.open = xradio_generic_open,
	.write = xradio_null_prd_set,
	.read = xradio_null_prd_get,
	.llseek = default_llseek,
};

int beacon_offset;
static ssize_t xradio_beacon_offset_get(struct file *file,
	char __user *user_buf, size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;
	struct xradio_vif *priv = NULL;*/
	...
	sprintf(buf, "beacon_offset = %d\n", beacon_offset);
	size = strlen(buf);

	return simple_read_from_buffer(user_buf, count, ppos, buf, size);
}

static ssize_t xradio_beacon_offset_set(struct file *file,
				    const char __user *user_buf,
				    size_t count, loff_t *ppos)
{
	struct xradio_common *hw_priv = file->private_data;
	...
	beacon_offset = simple_strtoul(buf, &endptr, 10);

	if (beacon_offset < 0)
		beacon_offset = 0;

	wsm_beacon_offset_time(hw_priv, beacon_offset, 0);
	xradio_dbg(XRADIO_DBG_ALWY, "beacon_offset = %d\n", beacon_offset);

	return count;
}

static const struct file_operations fops_beacon_offset = {
	.open = xradio_generic_open,
	.write = xradio_beacon_offset_set,
	.read = xradio_beacon_offset_get,
	.llseek = default_llseek,
};

int beacon_only_11b;
static ssize_t xradio_11b_beacon_only_get(struct file *file,
	char __user *user_buf, size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;
	struct xradio_vif *priv = NULL;*/
	...
	sprintf(buf, "beacon_only_11b = %d\n", beacon_only_11b);
	size = strlen(buf);

	return simple_read_from_buffer(user_buf, count, ppos, buf, size);
}

static ssize_t xradio_11b_beacon_only_set(struct file *file,
				    const char __user *user_buf,
				    size_t count, loff_t *ppos)
{
	struct xradio_common *hw_priv = file->private_data;
	...
	beacon_only_11b = simple_strtoul(buf, &endptr, 10);

	if (beacon_only_11b < 0)
		beacon_only_11b = 0;

	wsm_11b_beacon_only(hw_priv, beacon_only_11b, 0);
	xradio_dbg(XRADIO_DBG_ALWY, "11b_beacon_only = %d\n", beacon_only_11b);

	return count;
}

static const struct file_operations fops_11b_beacon_only = {
	.open = xradio_generic_open,
	.write = xradio_11b_beacon_only_set,
	.read = xradio_11b_beacon_only_get,
	.llseek = default_llseek,
};
...
extern u8 monitor_test_flag;

static ssize_t xradio_monitor_test_flags_get(struct file *file,
	char __user *user_buf, size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;*/
	char buf[100];
	...
	sprintf(buf, "monitor_test_flag=%d\n", monitor_test_flag);
	size = strlen(buf);

	return simple_read_from_buffer(user_buf, count, ppos, buf, size);
}

static ssize_t xradio_monitor_test_flags_set(struct file *file,
				      const char __user *user_buf,
				      size_t count, loff_t *ppos)
{
	/*struct xradio_common *hw_priv = file->private_data;*/
	...
	monitor_test_flag = simple_strtoul(buf, &endptr, 10);

	xradio_dbg(XRADIO_DBG_ALWY, "monitor_test_flag=%d\n",
		   monitor_test_flag);

	return count;
}

static const struct file_operations fops_monitor_test_flags = {
	.open = xradio_generic_open,
	.write = xradio_monitor_test_flags_set,
	.read = xradio_monitor_test_flags_get,
	.llseek = default_llseek,
};
...
static ssize_t xradio_get_fw_soft_agc(struct file *file,
       char __user *user_buf, size_t count, loff_t *ppos)
{
	struct xradio_common *hw_priv = file->private_data;
	int ret;
	...
	ret = wsm_read_mib(hw_priv, WSM_MIB_ID_FW_SOFT_AGC_CCA_POLICY, &fw_soft_agc_en, sizeof(fw_soft_agc_en), 0);
	if (ret) {
		xradio_dbg(XRADIO_DBG_ALWY, "fw soft agc read error!\n");
	} else {
		xradio_dbg(XRADIO_DBG_ALWY, "%sfw soft agc enable = %d\n", __func__, fw_soft_agc_en);
	}
	return 0;
}

static ssize_t xradio_set_fw_soft_agc(struct file *file,
       const char __user *user_buf, size_t count, loff_t *ppos)
{
	struct xradio_common *hw_priv = file->private_data;
	u32 fw_soft_agc_en = 0;
	char buf[12] = { 0 };
	char *endptr = NULL;
	...
	fw_soft_agc_en = simple_strtoul(buf, &endptr, 10);
	if (fw_soft_agc_en >= 8)
		fw_soft_agc_en = 8;

	wsm_write_mib(hw_priv, WSM_MIB_ID_FW_SOFT_AGC_CCA_POLICY, &fw_soft_agc_en, sizeof(fw_soft_agc_en), 0);
	xradio_dbg(XRADIO_DBG_ALWY, "%s fw_soft_agc_en = %d\n", __func__, fw_soft_agc_en);
	return count;
}

static const struct file_operations fops_fw_soft_agc = {
       .open = xradio_generic_open,
       .write = xradio_set_fw_soft_agc,
       .read = xradio_get_fw_soft_agc,
       .llseek = default_llseek,
};
#endif
...
#define ERR_LINE  do { line = __LINE__; goto err; } while (0)
	...
	if (!debugfs_create_file("pre_rx_bcn", S_IRUSR | S_IWUSR,
			  d->debugfs_phy, hw_priv, &fops_pre_rx_bcn))
			ERR_LINE;

	if (!debugfs_create_file("dtim", S_IRUSR | S_IWUSR,
		  d->debugfs_phy, hw_priv, &fops_dtim))
		ERR_LINE;

	if (!debugfs_create_file("lsn_intv", S_IRUSR | S_IWUSR,
		  d->debugfs_phy, hw_priv, &fops_listen_interval))
		ERR_LINE;

	if (!debugfs_create_file("beacon_win", S_IRUSR | S_IWUSR,
		  d->debugfs_phy, hw_priv, &fops_beacon_win))
		ERR_LINE;

	if (!debugfs_create_file("null_prd", S_IRUSR | S_IWUSR,
		  d->debugfs_phy, hw_priv, &fops_null_prd))
		ERR_LINE;

	if (!debugfs_create_file("beacon_offset", S_IRUSR | S_IWUSR,
		  d->debugfs_phy, hw_priv, &fops_beacon_offset))
		ERR_LINE;
	if (!debugfs_create_file("beacon_only_11b", S_IRUSR | S_IWUSR,
		  d->debugfs_phy, hw_priv, &fops_11b_beacon_only))
		ERR_LINE;
	...
	if (!debugfs_create_file("monitor_test_flags", S_IRUSR | S_IWUSR,
		  d->debugfs_phy, hw_priv, &fops_monitor_test_flags))
		ERR_LINE;
#ifdef XR_RRM
...
#if (XRADIO_USE_FW_SOFT_AGC_CCA_POLICY)
	if (!debugfs_create_file("fw_soft_agc_ctrl", S_IRUSR | S_IWUSR,
		d->debugfs_phy, hw_priv, &fops_fw_soft_agc))
		ERR_LINE;
#endif
...
outprint:
	...
	if (IS_FRAME_PRINT || IS_PROTO_PRINT) {
		...
		xradio_dbg(XRADIO_DBG_ERROR, "%s buff is null\n", __func__);
}
...
#if (FW_DEBUG_PRINT)
#define DBG_ELM_FLAGS__TIME      (0x1<<4)
#define DBG_ELM_FLAGS__NONE      (0x1<<5)
#define DBG_ELM_FLAGS__STRING    (0x1<<6)
#define DBG_ELM_FLAGS__CHECKSUM  (0x1<<7)
#define DBG_ELM_FLAGS__ARGNUM    (0xf)

typedef struct DBG_ELEMENT_s {
	u8  Debug_Flags;
	u8  CheckSum;
	u8  ArgOffset;
	u8  Reserved;
	u32 DebugInfoStart[1];
} DBG_ELM;

typedef struct DBG_OUTPUT_CTRL_s {
	volatile u32 CtrlMask;
	volatile u32 DataGet;
	volatile u32 DataPut;
	volatile u32 State;
	u32 OutBuffer[1];
} DBG_OUT;

typedef struct DBG_INPUT_CTRL_s {
	volatile u32 CtrlMask;
	volatile u32 DataGet;
	volatile u32 DataPut;
	volatile u32 State;
...
#define DBG_CTRL_MAGIC 0x12345678

int etf_rx_data;
u32 fw_debug_rx;
DBG_CTRL fw_dbg_ctrl;

void fw_debug_print(struct xradio_common *hw_priv)
{
	if (fw_dbg_ctrl.Magic && fw_debug_rx) {
		DBG_OUT *pOut = (DBG_OUT *)fw_dbg_ctrl.OutBuff;
		while (INDEX_BEFORE(pOut->DataGet, pOut->DataPut)) {
			const char *fmt;
			u32 offset = (pOut->DataGet & (fw_dbg_ctrl.OutBuffSize - 1));
			DBG_ELM *pElm = (DBG_ELM *)((u8 *)&pOut->OutBuffer + offset);
			u32 arg_num = (pElm->Debug_Flags & DBG_ELM_FLAGS__ARGNUM);
			if (!(pElm->Debug_Flags & DBG_ELM_FLAGS__NONE)) {
				u32 *Arg = (u32 *)&pElm + pElm->ArgOffset;
				if (pElm->Debug_Flags & DBG_ELM_FLAGS__STRING) {
					fmt = (const char *)(&pElm->DebugInfoStart);
				} else {
					fmt = (const char *)FW_ADDR_TO_APP_ADDR(pElm->DebugInfoStart[0]);
				}
				if (pElm->Debug_Flags & DBG_ELM_FLAGS__TIME) {
					xradio_dbg(XRADIO_DBG_ALWY, "[FW:%010d]\n", *Arg++);
				} else {
					xradio_dbg(XRADIO_DBG_ALWY, "[FW]\n");
				}
				switch (arg_num) {
				case 0:
				...
				case 1:
					...
					break;
				case 2:
					xradio_dbg(XRADIO_DBG_ALWY, "%s,%d,%d\n", fmt, Arg[0], Arg[1]);
					break;
				case 3:
					xradio_dbg(XRADIO_DBG_ALWY, "%s,%d,%d,%d\n", fmt, Arg[0], Arg[1], Arg[2]);
					break;
				default:
					xradio_dbg(XRADIO_DBG_ALWY, "FW PRINT unsupported, arg num =%d\n", arg_num);
				}
			}
			if (pElm->Debug_Flags & DBG_ELM_FLAGS__TIME)
				arg_num++;
			pOut->DataGet += ((pElm->ArgOffset + arg_num) << 2);
			if (atomic_read(&hw_priv->bh_rx)  || atomic_read(&hw_priv->bh_tx) || etf_rx_data)
				return;
		}
		fw_debug_rx = 0;
	}
...
int fw_debug_ctrl_init(const DBG_CTRL *pFW_DBG_CTRL)
{
	if (pFW_DBG_CTRL->Magic == DBG_CTRL_MAGIC) {
		DBG_OUT *pOut;
		fw_dbg_ctrl.Magic   = DBG_CTRL_MAGIC;
		fw_dbg_ctrl.OutBuff = FW_ADDR_TO_APP_ADDR(pFW_DBG_CTRL->OutBuff);
		fw_dbg_ctrl.OutBuffSize = pFW_DBG_CTRL->OutBuffSize;
		fw_dbg_ctrl.InBuff = FW_ADDR_TO_APP_ADDR(pFW_DBG_CTRL->InBuff);
		fw_dbg_ctrl.InBuffSize = pFW_DBG_CTRL->InBuffSize;
		pOut = (DBG_OUT *)fw_dbg_ctrl.OutBuff;
		xradio_dbg(XRADIO_DBG_ALWY, "FW DBG CTRL Supported, OutBuff=0x%08x, size=%d, InBuff=0x%08x, size=%d\n",
					fw_dbg_ctrl.OutBuff, fw_dbg_ctrl.OutBuffSize, fw_dbg_ctrl.InBuff, fw_dbg_ctrl.InBuffSize);
		xradio_dbg(XRADIO_DBG_ALWY, "FW OutPut CTRL Mask=0x%08x, Get=%d, Put=%d, State=0x%08x\n",
					pOut->CtrlMask, pOut->DataGet, pOut->DataPut, pOut->State);
#if (CONFIG_HIF_BY_PASS)
		REG_SET_BIT(HIF_OV_RX2_INT_CTRL, HIF_OV_INT_EN);
#endif
		return 0;
	}
	fw_dbg_ctrl.Magic = 0;
	xradio_dbg(XRADIO_DBG_ALWY, "FW DBG CTRL Not Supported = 0x%08x!\n", pFW_DBG_CTRL->Magic);
