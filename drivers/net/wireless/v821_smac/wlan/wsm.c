		      ...
		      int if_id)
{
	...
	buf->data -= arg->dpdData_size;
	xradio_sdd_update_frequency_ie(hw_priv, buf->data, arg->dpdData_size);
	buf->data += arg->dpdData_size;
#endif
			...
			size_t buf_size, int if_id)
{
	...
	if ((hw_priv->fw_txrxbuf_switch_buf_status == FW_TXRXBUF_SWITCH_BUF_ST_HOST_SWING) &&
		(mibId == WSM_MIB_ID_SWITCH_FW_TXRX_BUFFER) && (!atomic_read(&hw_priv->fw_txrxbuf_switch_buf_lock_cmd_tx)))
		atomic_set(&hw_priv->fw_txrxbuf_switch_buf_lock_cmd_tx, 1);
#endif
...
/* ******************************************************************** */
#if XRADIO_USER_POWER_LEVEL_TAB
int wsm_set_output_power_tab(struct xradio_common *hw_priv, int power_level, int if_id)
{
	...
	if (!priv) {
		wsm_printk(XRADIO_DBG_ERROR, "%s priv is NULL! if_id:%d\n", __func__, if_id);
		return -1;
	}
	spin_unlock(&priv->vif_lock);

	wsm_printk(XRADIO_DBG_TRC, "%s id = %d priv = 0x%x\n", __func__, if_id, (u32)priv);

	for (i = 0; i < POWER_LEVEL_TAB_USE_LENGTH; i++) {
		if (priv->user_config.power_level_tab[i] > power_level) {
			plt[i] = (u16)power_level;
			wsm_printk(XRADIO_DBG_WARN, "power level too big, max power: 0x%x\n", power_level);
		} else {
			plt[i] = priv->user_config.power_level_tab[i];
		}
	}

#ifdef P2P_MULTIVIF
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_POWER_LEVEL_TAB, (void *)plt, sizeof(plt), if_id ? 1 : 0);
#else
	return wsm_write_mib(hw_priv, WSM_MIB_ID_SET_POWER_LEVEL_TAB, (void *)plt, sizeof(plt), if_id);
#endif
}
#endif
			  ...
			  int interface_link_id)
{
	...
	tx_confirm.pktdebug = WSM_GET32(buf);
	tx_confirm.rsv[0] = WSM_GET32(buf);
	tx_confirm.rsv[1] = WSM_GET32(buf);
	tx_confirm.rsv[2] = WSM_GET32(buf);
#endif
					...
					struct wsm_buf *buf)
{
...
#if (defined CONFIG_FW_TXRXBUF_USE_COEXIST_RAM) || FW_RECORD_READ
	hw_priv->wsm_caps.firmwareConfig[3] = WSM_GET32(buf);
#endif
#ifdef CONFIG_FW_TXRXBUF_USE_COEXIST_RAM
	if (!get_etf_mode() && (xr_get_chip_type() == XR_CHIP_TYPE_V821B)) {
		hw_priv->fw_txrxbuf_epta_sec_start = WSM_GET32(buf);
		hw_priv->fw_txrxbuf_epta_sec_end = WSM_GET32(buf);
	}
#endif
	...
	wsm_printk(XRADIO_DBG_ALWY, "%s\n"
		...
		"   Hardware: %d.%d\n"
		"   %s firmware ver: 0x%x, build: 0x%x,"
		    " api: 0x%x, cap: 0x%.4X\n",
		__func__,
	...
	if ((hw_priv->wsm_caps.firmwareBuildNumber & ENABLE_P2P_KEEP_CODE) == 0)
		wsm_printk(XRADIO_DBG_WARN, "fw not support p2p mode!\n");
#endif
...
#if (CONFIG_WLAN_FUNC_CALL_SUPPORT)
static int
wsm_wlan_func_call_info_indication(struct xradio_common *hw_priv, struct wsm_buf *buf)
{
#ifdef CONFIG_WIRELESS_PM
#if (defined(CONFIG_PM_WAKELOCKS) && defined(CONFIG_CHIP_XRADIO))
	if (hw_priv->pm_state.wake_step == 1) {
		HAL_Wakeup_AddEvent(PM_WAKEUP_SRC_INTN_WLAN);
		PM_WAKE_LOCK(&hw_priv->pm_state.wake_lock);
	}
#elif (defined(CONFIG_DRIVER_R128) || (defined CONFIG_DRIVER_V821))
	PM_WAKE_LOCK(&hw_priv->pm_state.wake_lock);
#endif
#endif /* CONFIG_WIRELESS_PM */

	/* First check if the phy.a verison is match with wlan_fw */
	xradio_func_call_version_check(&hw_priv->wsm_caps.fw_label[0]);
	xradio_func_call_info_indication_handle(buf);

#ifdef CONFIG_WIRELESS_PM
#if (defined(CONFIG_PM_WAKELOCKS) && defined(CONFIG_CHIP_XRADIO))
	if (hw_priv->pm_state.wake_step == 1) {
		PM_WAKE_UNLOCK(&hw_priv->pm_state.wake_lock);
	}
#elif (defined(CONFIG_DRIVER_R128) || (defined CONFIG_DRIVER_V821))
	PM_WAKE_UNLOCK(&hw_priv->pm_state.wake_lock);
#endif
...
static int
wsm_wlan_func_call_info_indication_etf(struct xradio_common *hw_priv, struct wsm_buf *buf)
{
#ifdef CONFIG_WIRELESS_PM
#if (defined(CONFIG_PM_WAKELOCKS) && defined(CONFIG_CHIP_XRADIO))
	if (hw_priv->pm_state.wake_step == 1) {
	...
	/* First check if the phy.a verison is match with wlan_fw */
	xradio_func_call_version_check_etf(&hw_priv->wsm_caps.fw_label[0]);
	xradio_func_call_info_indication_handle_etf(buf);
...
#ifdef CONFIG_FW_TXRXBUF_USE_COEXIST_RAM
	if (xr_get_chip_type() == XR_CHIP_TYPE_V821B) {
		unsigned long timeout = jiffies + WSM_CMD_LOCK_TIMEOUT;

		while (atomic_read(&hw_priv->fw_txrxbuf_switch_buf_lock_cmd_tx) &&
			time_before(jiffies, timeout)) {
			if (hw_priv->bh_error) {
				wsm_printk(XRADIO_DBG_ERROR, "[WSM] %s bh_error:%d\n", __func__, hw_priv->bh_error);
				ret = -EBUSY;
				wsm_buf_reset(buf);
				return ret;
			}
		...
		if (atomic_read(&hw_priv->fw_txrxbuf_switch_buf_lock_cmd_tx) && time_after(jiffies, timeout)) {
			wsm_printk(XRADIO_DBG_ERROR, "[WSM] %s fw switch buf timeout\n", __func__);
			hw_priv->bh_error = 1;
#ifdef BH_USE_SEMAPHORE
			up(&hw_priv->bh_sem);
#else
...
#ifdef CONFIG_FW_TXRXBUF_USE_COEXIST_RAM
static int
wsm_fw_txrxbuf_switch_cmpl_indication(struct xradio_common *hw_priv, struct wsm_buf *buf)
{
	u32 status = 0;
	u8 tx_buf_num, rx_fifo, epta_stat;

	status = WSM_GET32(buf);
	tx_buf_num = WSM_GET8(buf);
	rx_fifo = WSM_GET8(buf);
	epta_stat = WSM_GET8(buf);
	hw_priv->fw_txrxbuf_switch_buf_status = status;
	hw_priv->wsm_caps.numInpChBufs = tx_buf_num;
	hw_priv->wsm_tx_seq = 0;
	wsm_printk(XRADIO_DBG_MSG, "%s host wsm_seq=%d.\n", __func__, hw_priv->wsm_rx_seq);
	wsm_printk(XRADIO_DBG_ALWY, "[SW_BUF] status:%d, tx buf:%d, rx fifo:%d, epta stat:%d\n", status, tx_buf_num, rx_fifo, epta_stat);

	up(&hw_priv->fw_txrxbuf_switch_buf_wq);
...
#if FW_DEBUG_PRINT
static int
wsm_fw_dbg_ctrl_init_indication(struct xradio_common *hw_priv, struct wsm_buf *buf)
{
	DBG_CTRL FW_DBG_CTRL;
	FW_DBG_CTRL.Magic = WSM_GET32(buf);
	FW_DBG_CTRL.OutBuff = WSM_GET32(buf);
	FW_DBG_CTRL.OutBuffSize = WSM_GET32(buf);
	FW_DBG_CTRL.InBuff = WSM_GET32(buf);
	FW_DBG_CTRL.InBuffSize = WSM_GET32(buf);
	fw_debug_ctrl_init(&FW_DBG_CTRL);
	return 0;
...
#if (defined(CONFIG_DRIVER_V821))
			add = hw_priv->wsm_caps.firmwareConfig[2];
			net_addr = FW_ADDR_TO_APP_ADDR(hw_priv->wsm_caps.firmwareConfig[2]);
			wsm_printk(XRADIO_DBG_NIY, "add:%x %x\n", add, net_addr);
			if (net_addr)
				xradio_efuse_info((void *)net_addr);
#endif
#if (CONFIG_WLAN_FUNC_CALL_SUPPORT)
		} else if (id == 0x0814) {
			ret = wsm_wlan_func_call_info_indication_etf(hw_priv, &wsm_buf);
			spin_lock(&hw_priv->wsm_cmd.lock);
...
#if FW_DEBUG_PRINT
			ret = wsm_fw_dbg_ctrl_init_indication(hw_priv, &wsm_buf);
#endif
#ifndef CONFIG_DRIVER_V821
			wlan_reg_enable();
#endif
...
/***************************for HWT ********************************/
	...
	if (id == 0x404) {
		...
		case 0x0425:
			...
			if (ret)
				wsm_printk(XRADIO_DBG_ERROR, "[0x%04x] ret(%d): Confirm Error, msg_len:%d\n",
					id, ret, wsm->len);
			break;
		...
		case 0x041C: /* map_link */
			...
			break;
		default:
			wsm_printk(XRADIO_DBG_ERROR, "Unknow confirm:0x%.4X.\n", id);
			SYS_BUG(1);
		}
		...
		case 0x080A:
			...
			break;
		case 0x0809:
			wsm_printk(XRADIO_DBG_ALWY, "wsm_set_pm indicaton\n");
			break;
		case 0x0806:
#ifdef ROAM_OFFLOAD
...
#if (CONFIG_WLAN_FUNC_CALL_SUPPORT)
		case 0x0814:
			ret = wsm_wlan_func_call_info_indication(hw_priv, &wsm_buf);
			break;
#endif
		case 0x0815:
			wsm_printk(XRADIO_DBG_MSG, "wsm_wlan_reg_enable_indication\n");
...
#ifdef CONFIG_FW_TXRXBUF_USE_COEXIST_RAM
		case 0x0818:
			ret = wsm_fw_txrxbuf_switch_cmpl_indication(hw_priv, &wsm_buf);
			break;
#endif
		case 0x0825:
			ret = wsm_fw_dbg_indicate(hw_priv, &wsm_buf);
			if (ret)
				wsm_printk(XRADIO_DBG_ERROR, "[0x%04x] ret(%d): indicate Error, msg_len:%d\n",
						id, ret, wsm->len);
			break;
		default:
	...
	case NL80211_IFTYPE_P2P_DEVICE:
		if (unlikely(priv->bss_loss_status == XRADIO_BSS_LOSS_CHECKING &&
			...
			if (ieee80211_is_auth(fctl))
...
#ifdef P2P_MULTIVIF
				if ((priv->join_status < XRADIO_JOIN_STATUS_MONITOR) && (!p2p_if_vif)) {
					wsm_printk(XRADIO_DBG_WARN, "%s:vif is NULL, fctl=0x%04x\n", __func__, fctl);
					action = doDrop;
				} else if ((priv->join_status < XRADIO_JOIN_STATUS_MONITOR) &&
						(p2p_if_vif->join_status < XRADIO_JOIN_STATUS_MONITOR)) {
					wsm_printk(XRADIO_DBG_WARN, "%s: join_status: %d %d fctl=0x%04x\n", __func__,
						priv->join_status, p2p_if_vif->join_status, fctl);
					action = doDrop;
				} else {
#endif
					action = doOffchannel;
				...
				if (action == doDrop && TES_P2P_0002_state == TES_P2P_0002_STATE_GET_PKTID) {
					TES_P2P_0002_state = TES_P2P_0002_STATE_IDLE;
					wsm_printk(XRADIO_DBG_MSG, "%s [TES_P2P_0002_state set to Idle]\n", __func__);
				}
#endif
			}
		}
