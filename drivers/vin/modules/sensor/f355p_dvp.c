...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for f355p sensors");
MODULE_LICENSE("GPL");
...
/*
 * The f355p i2c address
 */
#define I2C_ADDR 0x80

#define SENSOR_NUM 0x1
#define SENSOR_NAME "f355p_dvp"
#define SENSOR_NAME_2 "f355p_dvp_2"
#define		_SOI_MIRROR_FLIP
///#define	_SOI_BSUN_OPT
///#define	_SOI_PRECHG_OPT

static data_type		sensor_flip_status;
static data_type		Reg27;
static data_type		Reg28;
static data_type		Reg46;
static data_type		Reg80;
...
static int sensor_s_exp(struct v4l2_subdev *sd, unsigned int exp_val)
{
	...
	sensor_write(sd, 0x02, (exp_val >> 12) & 0xFF);
	sensor_write(sd, 0x01, (exp_val >> 4) & 0xFF);
...
{
	struct sensor_info *info = to_state(sd);
	//	printk("f355p sensor gain value is %d\n",gain_val);
...
static int f355p_sensor_vts;
static int f355p_fps_change_flag;
...
//	data_type read_high = 0, read_low = 0;
	int f355p_sensor_target_vts = 0;

	f355p_fps_change_flag = 1;
	f355p_sensor_target_vts = wsize->pclk / fps->fps / wsize->hts;

	if (f355p_sensor_target_vts <= wsize->vts) {// the max fps = 20fps
		f355p_sensor_target_vts = wsize->vts;
	} else if (f355p_sensor_target_vts >= (wsize->pclk/wsize->hts)) {// the min fps = 1fps
		f355p_sensor_target_vts = (wsize->pclk/wsize->hts) - 4;
	}

	f355p_sensor_vts = f355p_sensor_target_vts;
	sensor_dbg("target_fps = %d, f355p_sensor_target_vts = %d, 0x320e = 0x%x, 0x320f = 0x%x\n", fps->fps,
		f355p_sensor_target_vts, f355p_sensor_target_vts >> 8, f355p_sensor_target_vts & 0xff);
	sensor_write(sd, 0x22, (f355p_sensor_target_vts & 0xff));
	sensor_write(sd, 0x23, (f355p_sensor_target_vts >> 8));
//	sensor_read(sd, 0x320e, &read_high);
//	sensor_read(sd, 0x320f, &read_low);
//	sensor_print("[get write_times] 0x41 = 0x%x, 0x42 = 0x%x\n", read_high, read_low);
	f355p_fps_change_flag = 0;
...
static int sensor_g_fps(struct v4l2_subdev *sd, struct sensor_fps *fps)
{
	...
	sensor_read(sd, 0x22, &frame_length);
	act_vts = frame_length;
	sensor_read(sd, 0x23, &frame_length);
...
static int sensor_get_fmt_mbus_core(struct v4l2_subdev *sd, int *code)
{
	...
	data_type flip_mirror = (get_value >> 4) & 0x3;
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
    ...
    printk("------------------------\r\n");
    printk("this is f355p sensor init\r\n");
    printk("------------------------\r\n");
    printk("**********************************\r\n");
    printk("***********sensor init************\r\n");
    printk("**********************************\r\n");
...
static int sensor_reg_init(struct sensor_info *info)
{
	...
	f355p_sensor_vts = wsize->vts;
	sensor_dbg("f355p_sensor_vts = %d\n", f355p_sensor_vts);
	f355p_fps_change_flag = 0;
