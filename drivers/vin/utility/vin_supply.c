...
/*
 *set the gpio to irq status
 */
int vin_gpio_to_irq(struct v4l2_subdev *sd, enum gpio_type gpio_id, int *gpio_irq)
{
#ifndef FPGA_VER
	struct modules_config *modules = sd_to_modules(sd);
	int gc;
	...
	vin_log(VIN_LOG_POWER, "[%s] pin%d, set status to irq\n", __func__, gc);
	gpio_direction_input(gc);
	*gpio_irq = gpio_to_irq(gc);
	if (gpio_irq <= 0) {
		vin_err("gpio %d get irq err\n", gc);
		return -1;
	} else
		return 0;
#else
...
EXPORT_SYMBOL_GPL(vin_gpio_to_irq);


/*
 *set the gpio to mipisw
 */
int vin_gpio_to_mipisw(struct v4l2_subdev *sd, enum gpio_type gpio_id)
{
	int gc = 0;
#ifndef FPGA_VER
	struct modules_config *modules = sd_to_modules(sd);
...
EXPORT_SYMBOL_GPL(vin_gpio_to_mipisw);
