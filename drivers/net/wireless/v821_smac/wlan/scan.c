		   ...
		   struct ieee80211_scan_request *reqs)
{
	...
	if (work_pending(&priv->offchannel_work) || (hw_priv->roc_if_id != -1)) {
#ifdef TES_P2P_0002_ROC_RESTART
		struct timeval now;
		bool timeout;

		xr_do_gettimeofday(&now);
		timeout = ((now.tv_sec - TES_P2P_0002_roc_end_tmval.tv_sec) * 1000 +
		    (now.tv_usec - TES_P2P_0002_roc_end_tmval.tv_usec) / 1000) >= 0 ? true : false;

		if (TES_P2P_0002_state != TES_P2P_0002_STATE_IDLE || !timeout)
#endif
		{
			...
			scan_printk(XRADIO_DBG_NIY, "ROC end, scan permitted\n");
#endif
	}
	...
	if (!hw_priv->scan.req || (hw_priv->scan.curr == hw_priv->scan.end)) {
		...
		if (hw_priv->enable_advance_scan &&
			 ...
			 * is available for output power also */
				...
				WARN_ON(wsm_set_output_power_tab(hw_priv,
						hw_priv->scan.output_power * 10, priv->if_id));
#else
