...
#include "./de21x/de_lti_type.h"
#include "./de21x/de_peak_type.h"
	...
	struct disp_enhance_config *config)
{
	...
	csc_config.csc_id = 0;
	csc_config.hw_type = CSC_TYPE_DISP;
	de_csc2_get_config(&csc_config);

	csc_config.disp = disp;
	csc_config.csc_id = 0;
	csc_config.hw_type = CSC_TYPE_DISP;
	csc_config.flags = CSC_DIRTY_ENHANCE;
	csc_config.enhance_mode = (config->info.mode >> 16);
	csc_config.in_fmt = ctx->output.px_fmt_space;
	csc_config.out_fmt = ctx->output.px_fmt_space;
	csc_config.in_mode = ctx->output.color_space;
	csc_config.out_mode = ctx->output.color_space;
	csc_config.out_color_range = ctx->output.color_range;
	csc_config.in_color_range = ctx->output.color_range;
