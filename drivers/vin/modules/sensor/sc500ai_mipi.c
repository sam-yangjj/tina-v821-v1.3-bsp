...
static int sensor_s_gain(struct v4l2_subdev *sd, int gain_val)
{
	...
	if (++DPC_frame_cnt >= 3) {
		...
		// R = (R_high << 8) | R_low;
		if ((gainana >= 30 * 64) && (3 != DPC_Flag)) {
			sensor_write(sd, 0x5799, 0x07);
			DPC_Flag = 3;
		} else if ((gainana <= 20 * 64) && (2 != DPC_Flag)) {
...
{
	struct sensor_info *info = to_state(sd);
	int exp_val, gain_val, short_exp_val;
...
static data_type sc500ai_flip_status;
...
static int sensor_g_flip(struct v4l2_subdev *sd, struct sensor_flip *flip)
{
	...
	if (sc500ai_flip_status & 0x06)
		flip->hflip = 1;
	else
		flip->hflip = 0;

	if (sc500ai_flip_status & 0x60)
		flip->vflip = 1;
	else
		flip->vflip = 0;
#else
...
{
	*code = MEDIA_BUS_FMT_SBGGR10_1X10; // sc500ai support change the rgb format by itself
