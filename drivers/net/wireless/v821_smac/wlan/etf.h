...
#define  ETF_GENERIC_BASE                 (ETF_REQ_BASE + 0x0020)
#define  ETF_MSG_STARTUP_IND_ID           0x0801
#define  ETF_DOWNLOAD_SDD                 (0x20 + 22)
#define  ETF_HWT_REQ                      0x0004
#define  ETF_READ_PRCM_REG_REQ_ID         0x0110
#define  ETF_WRITE_PRCM_REG_REQ_ID        0x0111
#define  ETF_READ_EFUSE_ELEMENT_REQ_ID    (ETF_GENERIC_BASE + 0x0B)
#define  ETF_READ_EFUSE_ELEMENT_CNF_ID    (ETF_READ_EFUSE_ELEMENT_REQ_ID | ETF_CNF_BASE)
#define  ETF_WRITE_EFUSE_ELEMENT_REQ_ID   (ETF_GENERIC_BASE + 0x0C)
#define  ETF_WRITE_EFUSE_ELEMENT_CNF_ID   (ETF_WRITE_EFUSE_ELEMENT_REQ_ID | ETF_CNF_BASE)
...
typedef struct ETF_READ_EFUSE_ELEMENT_REQ_S {
	u16 MsgLen;
	u16 MsgId;
	u16 Type;
	u16 Addr;
	...
	u8  Reserved;
	u8  CheckSum;
} ETF_READ_EFUSE_ELEMENT_REQ;

#define EFUSE_DATA_LEN_MAX 83
#define EFUSE_READ_CNF_DEFAULT_LEN 12
typedef struct ETF_READ_EFUSE_ELEMENT_CNF_S {
	u16 MsgLen;
	u16 MsgId;
	u32 Result;
	u16 Type;
	u8  AreaInUse;
	u8  CheckSum;
	u8  EfuseData[EFUSE_DATA_LEN_MAX];
} ETF_READ_EFUSE_ELEMENT_CNF;

typedef struct ETF_WRITE_EFUSE_ELEMENT_REQ_S {
	u16 MsgLen;
	u16 MsgId;
	u16 Type;
	u16 Addr;
	...
	u8  CheckSum;
	u8  EfuseData[EFUSE_DATA_LEN_MAX];
} ETF_WRITE_EFUSE_ELEMENT_REQ;

#define EFUSE_WRITE_CNF_DEFAULT_LEN 11
typedef struct ETF_WRITE_EFUSE_ELEMENT_CNF_S {
	u16 MsgLen;
	u16 MsgId;
	u32 Result;
	u16 Type;
	u8  CheckSum;
	u8  EfuseData[EFUSE_DATA_LEN_MAX];
} ETF_WRITE_EFUSE_ELEMENT_CNF;

typedef struct RW_Efuse_Settings_Struct {
	u16 type;
	u16 satrt_bit;
	u16 bit_num;
	u16 efuse_data_len;
...
typedef struct RW_Efuse_Result_Struct {
	u32 u32ResultCode;
	u8  u8AreaInUse;
	u8  u8EfuseDataRet[EFUSE_DATA_LEN_MAX];
} RW_EfuseResult;

typedef enum _RW_EFUSE_TYPE {
	RW_EFUSE_NONE				= 0x0,
	RW_EFUSE_CHIPID				= 0x1,
	RW_EFUSE_DCXO_TRIM			= 0x2,
	RW_EFUSE_POUT_CAL			= 0x3,
	RW_EFUSE_MAC				= 0x4,
	RW_EFUSE_USER_AREA			= 0x5,
	RW_EFUSE_POUT_CAL_BT		= 0x6
} RW_EFUSE_T;
