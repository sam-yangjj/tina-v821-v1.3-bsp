...
#define EMAC_HASH_TABLE_SIZE	64
...
static __maybe_unused void emac_skb_debug_pkt(const char *prefix, struct sk_buff *skb)
{
	int len = skb_headlen(skb) > 96 ? 96 : skb_headlen(skb);
	pr_info("%s: protocol:%#x, len:%d\n", prefix, skb->protocol, skb_headlen(skb));
	print_hex_dump(KERN_INFO, "", DUMP_PREFIX_OFFSET, 16, 1, skb_mac_header(skb), len, true);
}
...
static void emac_hash_filter(void *iobase, unsigned long low, unsigned long high)
{
	writel(high, iobase + EMAC_RX_HASH1_REG);
	writel(low, iobase + EMAC_RX_HASH0_REG);
}
...
static void emac_set_rx_mode(struct net_device *ndev)
{
	...
	if (ndev->flags & IFF_PROMISC) {
		...
		reg_val &= ~EMAC_RX_CTL_PASS_ALL_EN;
		if ((netdev_mc_count(ndev) > EMAC_HASH_TABLE_SIZE) || (ndev->flags & IFF_ALLMULTI)) {
			emac_hash_filter(db->membase, ~0UL, ~0UL);
		} else if (!netdev_mc_empty(ndev)) {
			u32 mc_filter[2];
			struct netdev_hw_addr *ha;

			reg_val |= EMAC_RX_CTL_HASH_FILTER_EN;
			...
			netdev_for_each_mc_addr(ha, ndev) {
				...
				*/
				netdev_dbg(ndev, "%s: mcasts addr %pM\n", __func__, ha->addr);
			...
			emac_hash_filter(db->membase, mc_filter[0], mc_filter[1]);
		}
	}
...
static int emac_rx(struct net_device *dev, int limit)
{
	...
	unsigned int reg_val, reg_val_bk;
	...
	while (work_done < limit) {
		...
		if (reg_val != EMAC_UNDOCUMENTED_MAGIC) {
			/* disable RX */
			reg_val_bk = readl(db->membase + EMAC_RX_CTL_REG);
			writel(0, db->membase + EMAC_RX_CTL_REG);

			/* flush fifo */
			writel(BIT(3), db->membase + EMAC_RX_CTL_REG);
			do {
				reg_val = readl(db->membase + EMAC_RX_CTL_REG);
			} while (reg_val & (1 << 3));

			/* enable RX */
			writel(reg_val_bk, db->membase + EMAC_RX_CTL_REG);
