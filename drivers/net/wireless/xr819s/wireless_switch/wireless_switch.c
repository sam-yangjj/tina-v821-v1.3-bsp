...
bool wlan_reg_ready(void)
{
	return btlpm_state;
}
EXPORT_SYMBOL_GPL(wlan_reg_ready);

void wlan_set_reset_pin(bool state)
{
	sunxi_wlan_set_power(state);

	if (state == WLAN_STATE_OFF && bt_state == BT_STATE_OFF && btlpm_state == true)
...
static int sw_platform_init(void)
{
	...
	if (!np) {
		btlpm_state = false;
		WIRELESS_SWITCH_DEBUG("can't find compatible named sunxi-btlpm\n");
		return 0;
	}
	...
	if (!gpio_is_valid(wireless->wk_pin)) {
		btlpm_state = false;
		WIRELESS_SWITCH_DEBUG("get gpio bt_wakeup_pin failed\n");
		return 0;
	}
