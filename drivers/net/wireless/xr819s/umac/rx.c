...
ieee80211_rx_h_sta_process(struct ieee80211_rx_data *rx)
{
	...
	struct ieee80211_rx_status *status = IEEE80211_SKB_RXCB(skb);
	struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)skb->data;
#ifdef SIGNAL_SMOOTHING
	static s8 signal_buf[SIGNAL_WIN_SIZE] = {0};
	static int signal_num_p = 0;
	int i = 0, sum = 0, min = 0;
#endif
...
#ifdef SIGNAL_SMOOTHING
	if(++signal_num_p == SIGNAL_WIN_SIZE) {
		signal_num_p = 0;
	}
	signal_buf[signal_num_p] = (s8)status->signal;
	for (i = 0, sum = 0, min = 0; i < SIGNAL_WIN_SIZE; i++) {
		if(signal_buf[i] <= min) {
			min = signal_buf[i];
		}
		sum += signal_buf[i];
	}
	sta->last_signal = (s8)((sum - min) / (SIGNAL_WIN_SIZE - 1));
#else
...
ieee80211_rx_h_amsdu(struct ieee80211_rx_data *rx)
{
	...
	if (ieee80211_data_to_8023_exthdr(skb, &ethhdr,
					  rx->sdata->vif.addr,
					  rx->sdata->vif.type,
#if (LINUX_VERSION_CODE > KERNEL_VERSION(5, 4, 123))
					  0, true))
#else
					  0))
#endif
