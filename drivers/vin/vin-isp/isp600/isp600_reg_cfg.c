...
struct isp600_reg {
	...
	ISP_RANDOM_SEED_REG_t *isp_random_seed1;
...
void bsp_isp_map_save_load_dram_addr1(unsigned long id, vin_dma_addr_t base)
{
	isp_regs[id].isp_random_seed1 = (ISP_RANDOM_SEED_REG_t *) (base + ISP_RANDOM_SEED_REG);
}
...
void bsp_isp_set_input_fmt1(unsigned long id, unsigned int fmt)
{
	isp_regs[id].isp_global_cfg0->bits.input_fmt = fmt;
	isp_regs[id].isp_random_seed1->bits.input_fmt = fmt;
}
