...
/*
		//alloc buf with CONFIG_VIN_USE_ION_SIZE_POOL_HEAP
		for (j = 0; j < nplanes; j++) {
			CLEAR(exp);
			exp.type = V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE;
			exp.index = i;
			...
			if (-1 == ioctl(fd, VIDIOC_EXPBUF, &exp)) {
				...
				printf("buffer %d plane 0x%x DMABUF fd is %d\n", i, buf.m.planes[j].m.mem_offset, exp.fd);
			//close(exp.fd) when exit
		}
*/
		free(buf.m.planes);
	}
...
static int camera_init(int sel, int mode)
{
	...
	buffer_mode = 1;
	req_frame_num = 1;
	if (-1 == ioctl(fd, VIDIOC_VIN_SET_BUFFER_MODE, &buffer_mode)) {
		printf("VIDIOC_VIN_SET_BUFFER_MODE!\n");
		return -1;
	}
#endif
