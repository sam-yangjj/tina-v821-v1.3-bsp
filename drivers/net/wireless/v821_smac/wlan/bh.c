...
#if (CONFIG_WLAN_FUNC_CALL_SUPPORT)
	atomic_set(&hw_priv->bh_func_call, 0);
#endif
	hw_priv->buf_id_tx = 0;
	hw_priv->buf_id_rx = 0;
...
void xradio_irq_handler(void *priv)
{
	...
	int i = 0;
	u32 rx_flag = 0;
	u32 reg = HIF_OV_RX0_INT_CTRL;
#endif
	bh_printk(XRADIO_DBG_TRC, "%s\n", __func__);
	DBG_INT_ADD(irq_count);
#if (CONFIG_HIF_BY_PASS)
	/*Test Rx0~Rx3 interrupt*/
	while (i < 4) {
		if (REG_GET_BIT(reg, HIF_OV_INT_EN) &&
			REG_GET_BIT(reg, HIF_OV_INT_RDY_FLAG)) {
			REG_CLR_BIT(reg, HIF_OV_INT_RDY_FLAG);
			rx_flag |= (1 << i);
			xradio_dbg(XRADIO_DBG_MSG, "[BH] hifbypass irq%d.\n", i);
		}
		reg += 4;
		i++;
	}
	if (REG_GET_BIT(HIF_OV_RDY_INT_CTRL, HIF_OV_INT_EN) &&
		REG_GET_BIT(HIF_OV_RDY_INT_CTRL, HIF_OV_INT_RDY_FLAG)) {
		REG_CLR_BIT(HIF_OV_RDY_INT_CTRL, HIF_OV_INT_EN);
		xradio_dbg(XRADIO_DBG_MSG, "[BH] hifbypass rdy irq.\n");
	}
	...
	if (/* SYS_WARN */(hw_priv->bh_error)) {
		xradio_dbg(XRADIO_DBG_ERROR, "%s bh_error = %d\n", __func__, hw_priv->bh_error);
		return;
	}
#if (FW_DEBUG_PRINT)
	if (rx_flag & (0x1 << 0)) {
...
#if (CONFIG_WLAN_FUNC_CALL_SUPPORT)
	if (rx_flag & (0x1 << 1)) {
		if (atomic_add_return(1, &hw_priv->bh_func_call) == 1)
...
int xradio_device_wakeup(struct xradio_common *hw_priv)
{
	...
	bh_printk(XRADIO_DBG_MSG, "Device wakeup.\n");
...
#endif
	bh_printk(XRADIO_DBG_MSG, "RDY_INT = 0x%08x.\n", REG_READ_U32(HIF_OV_RDY_INT_CTRL));
	while (!REG_GET_BIT(HIF_WLAN_STATE, WLAN_STATE_ACTIVE) && (i < 500)) {
		msleep(1);
		i++;
	}
	...
	if (i >= 500) {
		bh_printk(XRADIO_DBG_ERROR, "Device cannot wakeup during 500ms.\n");
		return -1;
	} else {
		bh_printk(XRADIO_DBG_MSG, "awake\n");
	}
	return 1;
}
#else
...
static int xradio_bh(void *arg)
{
	...
	unsigned long minitor_rx_timeout = 0;
#endif
	...
	for (;;) {
...
#if (CONFIG_WLAN_FUNC_CALL_SUPPORT)
			&& atomic_read(&hw_priv->bh_func_call) == 0
#endif
...
#else
			REG_CLR_BIT(HIF_OV_CTRL, OV_WLAN_WAKE_UP);
#endif
#endif
		...
		status = wait_event_interruptible_timeout(hw_priv->bh_wq, ({
			...
			suspend = pending_tx ? 0 : atomic_read(&hw_priv->bh_suspend);
#if (CONFIG_WLAN_FUNC_CALL_SUPPORT)
			fw_func_call = atomic_xchg(&hw_priv->bh_func_call, 0);
#endif
		...
		if (!status) {
			...
			if (!is_device_awake()) {
				...
				if (ret > 0)
					fix_wakeup_device = true;
			}
#endif
			xradio_bh_read_ctrl_reg(hw_priv, &ctrl_reg);
#ifdef SUPPORT_XR_COEX
			if (fix_wakeup_device == true) {
				xradio_func_call_xr_coex_wakeup(0);
			}
#endif
...
#ifdef MONITOR_RX_FIX
			} else if (hw_priv->monitor_running && time_after(jiffies, minitor_rx_timeout)) {
				bh_printk(XRADIO_DBG_MSG, "monitor rx timeout, time_now:%u, pre_time = %u\n",
						(u32)jiffies, (u32)(minitor_rx_timeout - MONITOR_RX_TMO));
				queue_work(hw_priv->workqueue, &hw_priv->monitor_rx_restart_work);
#endif
			} else if (!pending_tx) {
				down(&hw_priv->device_wkup_lock);
...
#ifdef MONITOR_RX_FIX
			if (hw_priv->monitor_running)
				minitor_rx_timeout = jiffies + MONITOR_RX_TMO;
#endif
		} else {
			PERF_INFO_STAMP(&bh_start_time, &bh_others, 0);
		}
