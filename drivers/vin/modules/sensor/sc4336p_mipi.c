...
/*
 * A V4L2 driver for SC4336P Raw cameras.
 *
...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for SC4336P sensors");
MODULE_LICENSE("GPL");

#define MCLK				(27 * 1000 * 1000)
#define V4L2_IDENT_SENSOR	0x9c42
#define SENSOR_HOR_VER_CFG0_REG1 1

/*
 * The SC4336P i2c address
 */
#define I2C_ADDR			0x60      //0x60   0x64

#define SENSOR_NUM			0x1
#define SENSOR_NAME			"sc4336p_mipi"

static int SC4336P_sensor_vts;
...
/*Cleaned_0x01_FT_SC4336P_MIPI_27Minput_2lane_10bit_630Mbps_2560x1440_30fps_LDO_1.35v_continue*/
	{0x0103, 0x01},
	{0x36e9, 0x80},
	{0x37f9, 0x80},
	{0x301f, 0x01},
...
static int SC4336P_sensor_vts;
static int SC4336P_fps_change_flag;
...
//	data_type read_high = 0, read_low = 0;
	...
	if (!SC4336P_fps_change_flag) {
		shutter = exp_val >> 4;
		if (shutter > SC4336P_sensor_vts - 8) {//12
			frame_length = shutter + 8;
		} else
			frame_length = SC4336P_sensor_vts;
...
//	data_type read_high = 0, read_low = 0;
	int SC4336P_sensor_target_vts = 0;

	SC4336P_fps_change_flag = 1;
	SC4336P_sensor_target_vts = wsize->pclk/fps->fps/wsize->hts;

	if (SC4336P_sensor_target_vts <= wsize->vts) {// the max fps = 20fps
		SC4336P_sensor_target_vts = wsize->vts;
	} else if (SC4336P_sensor_target_vts >= (wsize->pclk/wsize->hts)) {// the min fps = 1fps
		SC4336P_sensor_target_vts = (wsize->pclk/wsize->hts) - 8;
	}

	SC4336P_sensor_vts = SC4336P_sensor_target_vts;
	sensor_dbg("target_fps = %d, SC4336P_sensor_target_vts = %d, 0x320e = 0x%x, 0x320f = 0x%x\n", fps->fps,
		SC4336P_sensor_target_vts, SC4336P_sensor_target_vts >> 8, SC4336P_sensor_target_vts & 0xff);
	sensor_write(sd, 0x320f, (SC4336P_sensor_target_vts & 0xff));
	sensor_write(sd, 0x320e, (SC4336P_sensor_target_vts >> 8));
//	sensor_read(sd, 0x320e, &read_high);
//	sensor_read(sd, 0x320f, &read_low);
//	sensor_print("[get write_times] 0x41 = 0x%x, 0x42 = 0x%x\n", read_high, read_low);
	SC4336P_fps_change_flag = 0;
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	sensor_print("ready to detect, write 0x440d(0x10) and 0x4400(0x11) first\n");
	sensor_write(sd, 0x440d, 0x10);
	sensor_write(sd, 0x4400, 0x11);
	/* Smartsens recommends adding 10ms delay */
