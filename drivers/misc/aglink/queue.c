...
int aglink_queue_init(struct aglink_queue_t *queue, size_t capacity, char *name, xr_free_skb_t cb)
{
	...
	memset(queue, 0, sizeof(struct aglink_queue_t));
	...
	aglink_k_spin_lock_init(&queue->lock);

	queue->pool = aglink_k_zmalloc(sizeof(struct aglink_queue_item_t) * capacity);
	if (!queue->pool)
		return -ENOMEM;
...
{
	aglink_queue_clear(queue);
	INIT_LIST_HEAD(&queue->free_pool);
	aglink_k_free(queue->pool);
	queue->pool = NULL;
	queue->capacity = 0;
	queue->free_skb = NULL;
}
...
int aglink_queue_put(struct aglink_queue_t *queue, struct sk_buff *skb, u8 check_q_full)
{
	...
	aglink_k_spin_lock_bh(&queue->lock);

	ret = list_empty(&queue->free_pool);
	if (!ret) {
		struct aglink_queue_item_t *item;

		item = list_first_entry(&queue->free_pool, struct aglink_queue_item_t, head);
		if (!item) {
			txrx_printk(AGLINK_DBG_ERROR, "push queue, find first entry error\n");
			ret = 2;
			goto end;
		}
		...
		if ((queue->num_queued > (queue->capacity * check_q_full / 100))) {
			txrx_printk(AGLINK_DBG_MSG, "%s nearly full, queued:%d, cap:%d\n",
					queue->name, queue->num_queued, queue->capacity);
			aglink_k_spin_unlock_bh(&queue->lock);
			return -ENOMEM;
		}
		ret = 0;
	} else {
		ret = 1;
		txrx_printk(AGLINK_DBG_NIY, "%s push queue fail, free_pool empty\n", queue->name);
	}
...
{
	struct aglink_queue_item_t *item = NULL;
	struct sk_buff *skb = NULL;

	aglink_k_spin_lock_bh(&queue->lock);
	if (!list_empty(&queue->queue)) {
		item = list_first_entry(&queue->queue, struct aglink_queue_item_t, head);
		if (!item) {
			txrx_printk(AGLINK_DBG_ERROR, "%s pop queue,find fist entry error\n", queue->name);
			goto end;
		}
...
int aglink_queue_clear(struct aglink_queue_t *queue)
{
	...
	while (!list_empty(&queue->queue)) {
		item = list_first_entry(&queue->queue, struct aglink_queue_item_t, head);
		if (!item) {
			txrx_printk(AGLINK_DBG_ERROR, "%s pop queue error\n", queue->name);
			ret = -1;
			goto end;
		}
		...
		if (queue->free_skb && skb)
			...
			txrx_printk(AGLINK_DBG_WARN, "%s skb %p not free\n", queue->name, skb);
	}
