...
struct disp_manager_private_data {
	...
	struct clk *clk_de_hb;
	struct clk *clk_disp_ahb;
	struct clk *clk_disp_mbus;
...
static s32 disp_mgr_clk_init(struct disp_manager *mgr)
{
	...
	/*
	  normally, the sun300iw1 uses the peripheral 307M to
	  divide the frequency into two and get 153.6M.
	 */
	if (mgrp->cfg->config.de_freq < 120 * 1000000 ||
	    mgrp->cfg->config.de_freq > 307 * 1000000) {
		DE_WARN("maybe de freq set err, freq = %d\n", mgrp->cfg->config.de_freq);
	}
#else
...
static s32 disp_mgr_clk_enable(struct disp_manager *mgr)
{
	...
	if (mgrp->clk_disp_ahb) {
		ret = clk_prepare_enable(mgrp->clk_disp_ahb);
		if (ret) {
			DE_WARN("clk_prepare_enable for clk_ahb failed\n");
			return ret;
		}
	}

	if (mgrp->clk_disp_mbus) {
		ret = clk_prepare_enable(mgrp->clk_disp_mbus);
		if (ret) {
			DE_WARN("clk_prepare_enable for clk_mbus failed\n");
			return ret;
		}
	}

	if (mgrp->clk_de_hb) {
		ret = clk_prepare_enable(mgrp->clk_de_hb);
		if (ret) {
			DE_WARN("clk_prepare_enable for clk_hb failed\n");
			return ret;
		}
	}
...
static s32 disp_mgr_clk_disable(struct disp_manager *mgr)
{
	...
	if (mgrp->clk_de_hb)
		clk_disable_unprepare(mgrp->clk_de_hb);
	if (mgrp->clk_disp_mbus)
		clk_disable_unprepare(mgrp->clk_disp_mbus);
	if (mgrp->clk_disp_ahb)
		clk_disable_unprepare(mgrp->clk_disp_ahb);
...
#if IS_ENABLED(CONFIG_AW_KSC)
	ksc_online_enable(false, 0, 0, 0, 0);
#endif
...
s32 disp_init_mgr(struct disp_bsp_init_para *para)
{
	...
	for (disp = 0; disp < num_screens; disp++) {
		...
		if (para->clk_de_hb)
			mgrp->clk_de_hb = para->clk_de_hb;
		if (para->clk_disp_ahb)
			mgrp->clk_disp_ahb = para->clk_disp_ahb;
		if (para->clk_disp_mbus)
			mgrp->clk_disp_mbus = para->clk_disp_mbus;
...
s32
disp_mgr_set_rtwb_sync_fd(struct disp_manager *mgr,
			  struct disp_capture_info2 *p_cptr_info)
{
	struct disp_manager_private_data *mgrp;
	struct dmabuf_item *wb_item;
	int ret = -1;
...
/* for rtwb unmap all dmabuf, test in ahb updata platform */
s32 disp_mgr_force_disable_layer_and_unmap_dmabuf(struct disp_manager *mgr)
{
	struct disp_manager_private_data *mgrp = disp_mgr_get_priv(mgr);
	struct disp_layer_config disable_config = {0};
	struct dmabuf_item *item, *tmp;
	struct disp_layer *lyr = NULL;
	unsigned long flags;
	int layer_cnt, channel_cnt, layer_per_channel, i, j;

	if (!mgr)
		return -1;

	layer_cnt = bsp_disp_feat_get_num_layers(mgr->disp);
	channel_cnt = bsp_disp_feat_get_num_channels(mgr->disp);

	mutex_lock(&mgr_mlock);
	for (i = 0; i < channel_cnt; i++) {
		layer_per_channel = bsp_disp_feat_get_num_layers_by_chn(mgr->disp, i);
		for (j = 0; j < layer_per_channel; j++) {
			lyr = disp_get_layer(mgr->disp, i, j);
			if (lyr == NULL)
				continue;

			if (!lyr->check(lyr, &disable_config)) {
				lyr->save_and_dirty_check(lyr, &disable_config);
			}
		}
	}
