			       ...
			       struct ieee80211_sched_scan_ies *ies)
{
	...
	frame.skb = mac80211_probereq_get(hw, priv->vif, NULL, 0, ies->ie[0], ies->len[0]);
	if (!frame.skb) {
		scan_printk(XRADIO_DBG_ERROR, "%s: mac80211_probereq_get failed!\n", __func__);
		return -ENOMEM;
	}
...
void xradio_scan_work(struct work_struct *work)
{
	struct xradio_common *hw_priv =
		container_of(work, struct xradio_common, scan.work);
	...
	if (!priv) {
		scan_printk(XRADIO_DBG_WARN, "interface removed, ignoring scan work\n");
		return;
	}
	...
	if (!hw_priv->scan.req || (hw_priv->scan.curr == hw_priv->scan.end)) {
		...
		if (hw_priv->enable_advance_scan) {
			...
			scan.numOfProbeRequests = (first->flags &
				(IEEE80211_CHAN_NO_IR | IEEE80211_CHAN_RADAR)) ? 0 : 4;
#ifdef CONFIG_XRADIO_TESTMODE
		}
#endif /* CONFIG_XRADIO_TESTMODE */
...
{
	struct xradio_common *hw_priv = container_of(work, struct xradio_common, scan.swork);
	struct wsm_scan scan;
...
static void xradio_scan_restart_delayed(struct xradio_vif *priv)
{
	...
	if (priv->delayed_link_loss) {
		...
		/* Restart beacon loss timer and requeue
		   BSS loss work. */
		scan_printk(XRADIO_DBG_WARN, "[CQM] Requeue BSS loss in %d beacons.\n", tmo);
		cancel_delayed_work_sync(&priv->bss_loss_work);
		queue_delayed_work(hw_priv->workqueue, &priv->bss_loss_work, tmo * HZ / 10);
...
{
	struct xradio_vif *priv = xrwl_hwpriv_to_vifpriv(hw_priv, hw_priv->scan.if_id);
	scan_printk(XRADIO_DBG_TRC, "%s\n", __func__);
	...
	if (hw_priv->scan.status == -ETIMEDOUT) {
		up(&hw_priv->scan.status_lock);
		scan_printk(XRADIO_DBG_WARN, "Scan timeout already occured. Don't cancel work");
	} else {
...
void xradio_advance_scan_timeout(struct work_struct *work)
{
	...
	if (hw_priv->advanceScanElems.scanMode ==
		...
		wsm_unlock_tx(hw_priv);
		up(&hw_priv->conf_lock);
		mac80211_scan_completed(hw_priv->hw, hw_priv->scan.status ? true : false);
		up(&hw_priv->scan.lock);
	}
}
#endif
...
void xradio_probe_work(struct work_struct *work)
{
	...
	scan.band = (hw_priv->channel->band ==
		NL80211_BAND_5GHZ) ? WSM_PHY_BAND_5G : WSM_PHY_BAND_2_4G;
	if (priv->join_status == XRADIO_JOIN_STATUS_STA) {
		scan.scanType  = WSM_SCAN_TYPE_BACKGROUND;
		scan.scanFlags = WSM_SCAN_FLAG_FORCE_BACKGROUND;
		if (priv->if_id)
