...
#define DCXO_CLK_40M     (40000000)
static void sun300iw1_nvmem_xlate(u32 *versions, char *name)
{
	u32 index = 0x0000;
	u32 clock_rate;

	/* The AW1882 does not have a reserved area for burning DVFS code values, */
	/* so there is no need to read them. */

	sunxi_set_vf_index(index);

	/* Determine whether to use 40m or 24m */
	clock_rate = determine_dcxo_clk_source();
	if (clock_rate == DCXO_CLK_40M)
		snprintf(name, MAX_NAME_LEN, "40m-vf%04x", index);
	else
		snprintf(name, MAX_NAME_LEN, "vf%04x", index);

	get_vf_table_version();
	sunxi_debug(NULL, "dvfs: %s, %s\n", ver_data.vf_ver, name);
}

static struct cpufreq_soc_data sun300iw1_soc_data = {
	.nvmem_xlate = sun300iw1_nvmem_xlate,
};
...
static const struct of_device_id sun50i_cpufreq_match_list[] = {
	...
	{.compatible = "riscv,sun300iw1p1", .data = &sun300iw1_soc_data,},
	{}
};
