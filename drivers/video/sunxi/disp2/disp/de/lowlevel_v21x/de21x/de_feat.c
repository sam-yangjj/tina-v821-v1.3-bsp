...
extern struct de_feat sun55iw6_de_features;
#elif IS_ENABLED(CONFIG_ARCH_SUN251IW1)
extern struct de_feat sun251iw1_de_features;
#endif
...
s32 de_feat_get_num_csc2s(u32 disp)
{
	s32 num_csc2s = 0;
	if (disp < de_feat_get_num_screens()) {
		u32 i, index = 0, num_channels = 0;
		...
		num_channels = de_feat_get_num_chns(disp);
		for (i = 0; i < num_channels; i++, index++)
			num_csc2s += de_cur_features->num_csc2_chns[index];
		num_csc2s += de_cur_features->num_csc2_disps[disp];
	}
	return num_csc2s;
}

s32 de_feat_get_num_csc2s_by_disp(u32 disp)
{
	if (disp < de_feat_get_num_screens())
		return de_cur_features->num_csc2_disps[disp];
	else
		return 0;
}

s32 de_feat_get_num_csc2s_by_chn(u32 disp, u32 chn)
{
	if ((disp < de_feat_get_num_screens())
		&& (chn < de_feat_get_num_chns(disp))) {
		u32 i, index = 0;
		for (i = 0; i < disp; i++)
			index += de_feat_get_num_chns(i);
		index += chn;
		return de_cur_features->num_csc2_chns[index];
	}
	return 0;
}
...
s32 de_feat_is_support_lti_by_chn(u32 disp, u32 chn)
{
	if ((disp < de_feat_get_num_screens())
		&& (chn < de_feat_get_num_chns(disp))) {
		u32 i, index = 0;
		for (i = 0; i < disp; i++)
			index += de_feat_get_num_chns(i);
		index += chn;
		return de_cur_features->is_support_lti[index];
	}
	return 0;
}

s32 de_feat_is_support_peak_by_chn(u32 disp, u32 chn)
{
	if ((disp < de_feat_get_num_screens())
		&& (chn < de_feat_get_num_chns(disp))) {
		u32 i, index = 0;
		for (i = 0; i < disp; i++)
			index += de_feat_get_num_chns(i);
		index += chn;
		return de_cur_features->is_support_peak[index];
	}
	return 0;
}
...
s32 de_feat_is_support_lbc_by_layer(u32 disp,
	u32 chn, u32 layer)
{
	if ((disp < de_feat_get_num_screens())
		&& (chn < de_feat_get_num_chns(disp))
...
{
#if IS_ENABLED(CONFIG_ARCH_SUN55IW6)
	de_cur_features = &sun55iw6_de_features;
#elif IS_ENABLED(CONFIG_ARCH_SUN251IW1)
	de_cur_features = &sun251iw1_de_features;
#else
