...
static struct ispstat_buffer *isp_stat_buf_get(struct isp_stat *stat, struct vin_isp_stat_data *data)
{
	...
	if (NULL != data) {
		...
		*/
		...
		if (stat->isp->id == SOFTWARE_TDM_ID) {
			if (*(unsigned char *)(buf->virt_addr + ISP_SAVE_BUF_SIGN0_OFFSET) == ISP_SAVE_BUF_SIGN0 &&
					*(unsigned char *)(buf->virt_addr + ISP_SAVE_BUF_SIGN1_OFFSET) == ISP_SAVE_BUF_SIGN1) {
				if ((data->config_counter & 0x2) && buf->frame_number > 60) {
					rval = copy_to_user(data->buf + buf->buf_size + ISP_SAVE_LOAD_PLTM_PKX_SIZE, stat->isp->isp_save_load1.vir_addr + ISP_SAVE_LOAD_REG_SIZE + ISP_SAVE_LOAD_PLTM_PKX_SIZE, ISP_SAVE_LOAD_D3D_K_SIZE);
				} else {
					rval = copy_to_user(data->buf + buf->buf_size, stat->isp->isp_save_load1.vir_addr + ISP_SAVE_LOAD_REG_SIZE, ISP_SAVE_LOAD_STATISTIC_SIZE);
				}
			} else {
				if ((data->config_counter & 0x2) && buf->frame_number > 60) {
...
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
			if (stat->isp->id == SOFTWARE_TDM_ID)
				stat->active_buf->embed_mask = stat->isp->isp_handle_embed;
#endif
		}
	}
...
#if IS_ENABLED(CONFIG_SOFTWARE_TDM_FOR_ISP)
			if (stat->isp->id == SOFTWARE_TDM_ID)
				stat->buf[i].embed_mask =  stat->isp->isp_handle_embed;
#endif
	...
	for (i = 0; i < stat->buf_cnt; i++) {
		if (stat->buf[i].state == ISPSTAT_LOAD_SET)
			...
			if (stat->isp->id == SOFTWARE_TDM_ID) {
				...
				if (stat->buf[i].embed_mask) {
					*(unsigned char *)(stat->buf[i].virt_addr + ISP_SAVE_BUF_SIGN0_OFFSET) = ISP_SAVE_BUF_SIGN0;
					*(unsigned char *)(stat->buf[i].virt_addr + ISP_SAVE_BUF_SIGN1_OFFSET) = ISP_SAVE_BUF_SIGN1;
				} else {
					*(unsigned char *)(stat->buf[i].virt_addr + ISP_SAVE_BUF_SIGN0_OFFSET) = 0;
					*(unsigned char *)(stat->buf[i].virt_addr + ISP_SAVE_BUF_SIGN1_OFFSET) = 0;
				}
			}
#endif
...
#if IS_ENABLED(CONFIG_AW_ISP_PARAM)
...
{
	sunxi_isp_param_save_isp_info(to, len, buf);
	return 1;
}
#elif IS_ENABLED(CONFIG_AW_MTD)
...
static void __isp_save_ae_handle(struct work_struct *work)
{
	struct isp_dev *isp = container_of(work, struct isp_dev, isp_save_ae_task);
	unsigned int rpmag_data;

	rpmag_data = VIN_SET_SAVE_AE_ACK;
	isp_rpmsg_send(isp, &rpmag_data, sizeof(unsigned int));
}
...
void isp_save_ae(struct isp_dev *isp, void *data)
{
	...
	ret = isp_write_nor_flash(to, 0, 5*4, data);
	if (ret < 0) {
		schedule_work(&isp->isp_save_ae_task);
	}
}
...
int vin_isp_h3a_init(struct isp_dev *isp)
{
	...
	stat->isp = isp;
	mutex_init(&stat->ioctl_lock);
	INIT_WORK(&isp->isp_save_ae_task, __isp_save_ae_handle);
