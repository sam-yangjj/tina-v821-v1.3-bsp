...
static int sun8iw20_ccu_really_probe(struct device_node *node)
{
	void __iomem *reg;
	u32 val;
	int i, ret;
	...
	ret = sunxi_ccu_probe(node, reg, &sun8iw20_ccu_desc);
	if (ret)
		return ret;
...
static void __init of_sun8iw20_ccu_init(struct device_node *node)
{
	int ret;

	ret = sun8iw20_ccu_really_probe(node);
	if (ret)
		sunxi_err(NULL, "sun8iw20 ccu really probe failed\n");
}

CLK_OF_DECLARE(sun8iw20_ccu_init, "allwinner,sun20iw1-ccu", of_sun8iw20_ccu_init);
#else
