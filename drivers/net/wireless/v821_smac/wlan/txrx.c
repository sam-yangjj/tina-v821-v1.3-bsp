...
struct timeval TES_P2P_0002_roc_end_tmval;
	...
	struct ieee80211_tx_rate *rates, size_t count)
{
	...
	if (WSM_CAPS_11N_TO_11BG(hw_priv->wsm_caps) && !priv->vif->p2p &&
		...
		txrx_printk(XRADIO_DBG_MSG,
			"[TX policy]WSM_CAPS to11b=%d, p2p=%d, MSC=%d, rates=0x%08x, count(%u)=%d\n",
...
void tx_policy_init(struct xradio_common *hw_priv)
{
	...
	for (i = 0; i < hw_priv->tx_policy_limit; ++i)
		  ...
		  u8 use_bg_rate, bool *renew)
{
	...
	if (unlikely(list_empty(&cache->free)) &&
		!cache->queue_locked) {
		/* Lock TX queues. */
		txrx_printk(XRADIO_DBG_MSG, "[TX policy] policy cache used up\n");
...
void xradio_tx(struct ieee80211_hw *dev, struct ieee80211_tx_control *control, struct sk_buff *skb)
{
	...
	if (ieee80211_is_probe_req(frame->frame_control)) {
		printk("%s  cfg send scan correct\n", __func__);
	}
#ifdef CONFIG_XRADIO_DEBUGFS
...
#ifdef ROC_DEBUG
	if (ieee80211_is_action(frame->frame_control)) {
		txrx_printk(XRADIO_DBG_ALWY, "%s [ROC] Tx action if_id:%d join_status:%d\n", __func__,
	...
	*/
	...
	if (hw_priv->tx_policy_limit > 8) {
		wsm->reserved |= WSM_TX_PATCH_POLICY_MAX;
		if (flags & WSM_TX_2BYTES_SHIFT) {
			wsm->reserved |= WSM_TX_2BYTES_SHIFT;
			flags &= (~WSM_TX_2BYTES_SHIFT) & 0xff;
		}
	}
			  ...
			  struct wsm_tx_confirm *arg)
{
...
#endif
	{
		...
		if ((arg->status == WSM_STATUS_TX_LIFETIME_EXCEEDED) || (arg->status == WSM_STATUS_FLUSH)
			|| (arg->status == WSM_STATUS_FLUSH_END_OF_LIFE)) {
			txrx_printk(XRADIO_DBG_ERROR, "status=%d, retry=%d, lastRate=%d, mediaDelay=%d, txQueueDelay=%d, pktdebug=0x%x, pktId:0x%x\n", \
					  arg->status, arg->ackFailures, arg->txedRate, \
					  arg->mediaDelay & (1<<31) ? -(arg->mediaDelay &(~(1<<31))) : (arg->mediaDelay &(~(1<<31))), \
					  arg->txQueueDelay, arg->pktdebug, arg->packetID);
		} else if (arg->status) {
			txrx_printk(XRADIO_DBG_NIY, "status=%d, retry=%d, lastRate=%d, mediaDelay=%d, txQueueDelay=%d, pktdebug=0x%x, pktId:0x%x\n", \
					  arg->status, arg->ackFailures, arg->txedRate, \
					  arg->mediaDelay & (1<<31) ? -(arg->mediaDelay &(~(1<<31))) : (arg->mediaDelay &(~(1<<31))), \
					  arg->txQueueDelay, arg->pktdebug, arg->packetID);
		} else {
			txrx_printk(XRADIO_DBG_MSG, "status=%d, retry=%d, lastRate=%d, mediaDelay=%d, txQueueDelay=%d, pktdebug=0x%x, pktId:0x%x\n", \
					  arg->status, arg->ackFailures, arg->txedRate, \
					  arg->mediaDelay, arg->txQueueDelay, arg->pktdebug, arg->packetID);
		}
	}
#endif
	...
	if ((TES_P2P_0002_state == TES_P2P_0002_STATE_GET_PKTID) &&
		...
		if (arg->status == 0x00) {
			...
			if (TES_P2P_0002_roc_time < 100) {
				...
				TES_P2P_0002_roc_end_tmval.tv_sec = TES_P2P_0002_now_sec + TES_P2P_0002_roc_time / 1000;
				TES_P2P_0002_roc_end_tmval.tv_usec = TES_P2P_0002_now_usec + TES_P2P_0002_roc_time * 1000;
				TES_P2P_0002_roc_rst_need = true;
				txrx_printk(XRADIO_DBG_WARN, "%s roc time too small.\n", __func__);
			}
...
#ifdef CONFIG_XRADIO_DEBUGFS
u8 monitor_rx_cnt;
u8 monitor_test_flag;
#endif

#if XRADIO_FIX_P2P_RX_SAME_NGO_REQ
static u8 ngo_request_sa[6];
static u16 ngo_request_seq;
#define ACTION_IS_P2P_FRAME(action) \
	(action[2] == 0x50 && action[3] == 0x6F && \
	    action[4] == 0x9A && action[5] == 0x09)
#endif
		  ...
		  struct sk_buff **skb_p)
{
	...
	if ((ieee80211_is_action(frame->frame_control))
		...
		xradio_check_go_neg_conf_success(hw_priv, action);
#if XRADIO_FIX_P2P_RX_SAME_NGO_REQ
		if (ACTION_IS_P2P_FRAME(action)) {
			/* action[6]: 0 neg request, 1 neg response, 2 neg confirmation */
			if (!action[6] && mgmt->seq_ctrl == ngo_request_seq && !memcmp(mgmt->sa, ngo_request_sa, ETH_ALEN)) {
				txrx_printk(XRADIO_DBG_NIY, "recv duplicate ngo request seq %x in scan, drop it\n", mgmt->seq_ctrl);
				action_drop = 1;
			} else {
				txrx_printk(XRADIO_DBG_MSG, "first ngo req, storege it %x\n", mgmt->seq_ctrl);
				ngo_request_seq = mgmt->seq_ctrl;
				memcpy(ngo_request_sa, mgmt->sa, ETH_ALEN);
			}
		}
#endif
	}
	...
	if (!hw_priv->monitor_running) { /* In monitor mode, all frames will be received */
		if (skb->len < sizeof(struct ieee80211_pspoll)) {
			txrx_printk(XRADIO_DBG_WARN, "Mailformed SDU rx'ed. "
				    "Size is lesser than IEEE header. len:%d, fctl:0x%x\n", skb->len, frame->frame_control);
			goto drop;
		}
	}
	...
	if (rxparse_flags) {
		if (monitor_test_flag && (priv->join_status == XRADIO_JOIN_STATUS_MONITOR)) {
			if (monitor_rx_cnt == 0) {
				printk("---------------[Monitor test mode]---------------\n");
...
#ifdef ROC_DEBUG
	if (ieee80211_is_action(frame->frame_control)) {
		txrx_printk(XRADIO_DBG_ALWY, "%s [ROC] Rx action if_id:%d join_status:%d\n", __func__,
			priv->if_id, priv->join_status);
	}
#endif
	...
	if (ieee80211_is_beacon(frame->frame_control) &&
		...
		if (ds_ie && (ds_ie[1] == 1)) {
			...
			if (ds_ie_partms_chan != hw_priv->join_chan) {
				...
				if (cancel_delayed_work(&priv->connection_loss_work))
					queue_delayed_work(hw_priv->workqueue, &priv->connection_loss_work, 0);
			}
		}
	}
