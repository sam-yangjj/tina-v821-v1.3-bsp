...
#define SUN300IW1_OFFSET (-2813)
#define SUN300IW1_SCALE (-66)
#endif
#if IS_ENABLED(CONFIG_CHIP_V821B)
#define SUN300IW1_SENSOR_DATA_CODE (1500)
#define SUN300IW1_OFFSET_BELOW (-2436)
#define SUN300IW1_SCALE_BELOW (-66)
#define SUN300IW1_OFFSET_ABOVE (-2372)
#define SUN300IW1_SCALE_ABOVE (-71)
#endif

static int sun300iw1_calc_temp(struct ths_device *tmdev, int reg)
{
#if IS_ENABLED(CONFIG_CHIP_V821)
	return (reg + SUN300IW1_OFFSET) * SUN300IW1_SCALE;
#endif

#if IS_ENABLED(CONFIG_CHIP_V821B)
	if (reg > SUN300IW1_SENSOR_DATA_CODE)
		return ((reg + SUN300IW1_OFFSET_ABOVE) * SUN300IW1_SCALE_BELOW);
	else
		return ((reg + SUN300IW1_OFFSET_BELOW) * SUN300IW1_SCALE_BELOW);
#endif
}

static int sun300iw1_ths_get_temp(void *data, int *temp)
{
	struct tsensor *s = data;
	struct ths_device *tmdev = s->tmdev;
	int val = 0;
	...
	*temp = sun300iw1_calc_temp(tmdev, val);
...
#define SUN60IW2_SENSOR_DATA_CODE (2000)
#define SUN60IW2_OFFSET_BELOW (-2825)
#define SUN60IW2_SCALE_BELOW (-67)
#define SUN60IW2_OFFSET_ABOVE (-2868)
#define SUN60IW2_SCALE_ABOVE (-60)
...
static int sun300iw1_ths_calibrate(struct ths_device *tmdev)
{
	...
	for (i = 0; i < tmdev->chip->sensor_num; i++) {
		...
		delta = (ft_temp * 100 + SUN300IW1_CAL_COM - sun300iw1_calc_temp(tmdev, reg))
			/ tmdev->chip->scale;
		cdata = CALIBRATE_DEFAULT - delta;
		if (cdata & ~TEMP_CALIB_MASK) {
			sunxi_warn(dev, "sensor%d is not calibrated.\n", i);
...
static int sun300iw1_thermal_init(struct ths_device *tmdev)
{
	int val;
...
static const struct ths_thermal_chip sun300iw1p1_ths = {
	...
	.init = sun300iw1_thermal_init,
	.get_temp = sun300iw1_ths_get_temp,
};
