...
static void xradio_itp_handle(struct xradio_common *priv, struct sk_buff *skb);
static void xradio_itp_err(struct xradio_common *priv, int err, int arg);
...
void xradio_itp_release(struct xradio_common *priv)
{
	...
	wait_event_interruptible(itp->close_wait, !atomic_read(&itp->open_count));
...
static int __xradio_itp_open(struct xradio_common *priv)
{
	...
	if (!priv->channel)
		priv->channel = &priv->hw->wiphy->bands[NL80211_BAND_2GHZ]->channels[0];
...
static void xradio_itp_tx_start(struct xradio_common *priv)
{
	...
	itp_printk(XRADIO_DBG_MSG,
		   "[ITP] TX start: band = %d, ch = %d, rate = %d, preamble = %d, number = %d, data_mode = %d, interval = %d, power = %d, data_len = %d\n",
	...
	xradio_itp_fill_pattern(&itp->data[itp->hdr_len], itp->data_len, itp->data_mode);

	xradio_disable_listening(priv);
	priv->channel = &priv->hw->wiphy->bands[itp->band]->channels[itp->ch];
	SYS_WARN(wsm_set_output_power(priv, itp->power));
	if (itp->preamble == ITP_PREAMBLE_SHORT || itp->preamble == ITP_PREAMBLE_LONG)
		SYS_WARN(wsm_set_association_mode(priv, &assoc_mode));
	wsm_set_bssid_filtering(priv, false);
	xradio_enable_listening(priv, priv->channel);
				   ...
				   enum xradio_itp_version_type type)
{
	...
	switch (type) {
	case ITP_CHIP_ID:
		len = xradio_print_fw_version(priv, buf + size, ITP_BUF_SIZE - size);
			      ...
			      struct sk_buff *skb)
{
	...
	case 1: /* RX test */
		...
		ret = sscanf(skb->data, "%d,%u,%u", &cmd, &itp->band, &itp->ch);
		if (ret != 3) {
