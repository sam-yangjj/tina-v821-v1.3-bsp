...
#define SUN50I_TIMER_MODULE_VERSION	"1.1.7"

#if IS_ENABLED(CONFIG_AW_TIMER_SWAP_IVH_CVL_SPACE)
#define AW_TIMER_SWAP_IVH_CVL_SPACE     1
#else
#define AW_TIMER_SWAP_IVH_CVL_SPACE     0
#endif
...
#define TIMER_CVL_REG(val)	(0x20 * (val) + 0x20 + 0x08 + 0x04 * AW_TIMER_SWAP_IVH_CVL_SPACE)
#define TIMER_IVH_REG(val)	(0x20 * (val) + 0x20 + 0x0c - 0x04 * AW_TIMER_SWAP_IVH_CVL_SPACE)
...
static void timer_sun50i_clk_deinit(struct sun50i_timer *chip)
{
	...
	if (!IS_ERR_OR_NULL(chip->timer0_clk))
		clk_disable_unprepare(chip->timer0_clk);
	if (!IS_ERR_OR_NULL(chip->timer1_clk))
...
static irqreturn_t sun50i_timer_interrupt(int irq, void *dev_id)
{
	...
	struct timer_of *to = to_timer_of(evt);
	u32 enable = 0, irq_state = 0;

	enable = readl(timer_of_base(to) + TIMER_IRQ_REG);
	irq_state = readl(timer_of_base(to) + TIMER_STA_REG);
...
static struct timer_of to = {
	...
	/*
	 * Because the value of the rating variable for riscv_timer_clockevent is 100,
	 * and the value of the rating variable for arch_sys_timer is 450, in order to
	 * ensure that soctimer is not selected as the tick device, the value of the
	 * rating for sun50i_tick needs to be less than 100.
	 */
	.clkevt = {
		.name = "sun50i_tick",
		.rating = 50,
...
static int sun50i_timer_resource_get(struct sun50i_timer *chip, struct device_node *np)
{
	...
	if (IS_ERR(chip->timer0_clk)) {
		sunxi_warn(NULL, "request timer0 clock failed\n");
	}

	chip->timer1_clk = of_clk_get_by_name(np, "timer1-mod");
	if (IS_ERR(chip->timer1_clk)) {
		sunxi_warn(NULL, "request timer1 clock failed\n");
	}
...
static int sunxi_timer_init(struct device_node *node)
{
	...
	ret = clocksource_mmio_init(timer_of_base(&to) + TIMER_CVL_REG(1),
				    node->name, timer_of_rate(&to), CLOCKSOURCE_RATING, 56,
