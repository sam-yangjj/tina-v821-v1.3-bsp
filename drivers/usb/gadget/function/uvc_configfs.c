...
static ssize_t uvcg_default_control_strings_show(
	struct config_item *item, char *page)
{
	struct config_group *group = to_config_group(item);
	struct mutex *su_mutex = &group->cg_subsys->su_mutex;
...
static ssize_t uvcg_default_control_strings_store(
	struct config_item *item, const char *page, size_t len)
{
	...
	mutex_lock(&opts->lock);
	if (len > sizeof(opts->control_interface_strings))
		store_len = sizeof(opts->control_interface_strings);
	snprintf(opts->control_interface_strings, store_len, "%s", page);
	result = len;
	mutex_unlock(&opts->lock);
...
UVC_ATTR(uvcg_default_control_, strings, strings);

static struct configfs_attribute *uvcg_default_control_attrs[] = {
	&uvcg_default_control_attr_b_interface_number,
	&uvcg_default_control_attr_strings,
	NULL,
};
...
UVCG_FRAME_ATTR(b_frame_index, bFrameIndex, 8);
					   ...
					   const char *name)
{
	...
	} else if (fmt->type == UVCG_NV12) {
		h->frame.b_descriptor_subtype = UVC_VS_FRAME_UNCOMPRESSED;
		h->fmt_type = UVCG_NV12;
	} else {
		mutex_unlock(&opts->lock);
		kfree(h);
...
{
	struct uvcg_h264 *h;
	char h264GuidFormat[16] = {'H', '2', '6', '4', 0x00, 0x00, 0x10, 0x00,
					0x80, 0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71};
	...
	memcpy(h->desc.guidFormat, h264GuidFormat, sizeof(h264GuidFormat));
...
static struct uvcg_nv12 *to_uvcg_nv12(struct config_item *item)
{
	return container_of(
		container_of(to_config_group(item), struct uvcg_format, group),
		struct uvcg_nv12, fmt);
}

static struct configfs_group_operations uvcg_nv12_group_ops = {
	.make_item		= uvcg_frame_make,
	.drop_item		= uvcg_frame_drop,
};

static ssize_t uvcg_nv12_guid_format_show(struct config_item *item,
							char *page)
{
	struct uvcg_nv12 *ch = to_uvcg_nv12(item);
	struct f_uvc_opts *opts;
	struct config_item *opts_item;
	struct mutex *su_mutex = &ch->fmt.group.cg_subsys->su_mutex;
...
static ssize_t uvcg_nv12_guid_format_store(struct config_item *item,
						   const char *page, size_t len)
{
	struct uvcg_nv12 *ch = to_uvcg_nv12(item);
	struct f_uvc_opts *opts;
...
UVC_ATTR(uvcg_nv12_, guid_format, guidFormat);

#define UVCG_NV12_ATTR_RO(cname, aname, bits)			\
static ssize_t uvcg_nv12_##cname##_show(			\
	struct config_item *item, char *page)				\
{									\
	struct uvcg_nv12 *u = to_uvcg_nv12(item);	\
	struct f_uvc_opts *opts;					\
	struct config_item *opts_item;					\
	struct mutex *su_mutex = &u->fmt.group.cg_subsys->su_mutex;	\
	int result;							\
	...
	return result;							\
									...
									\
UVC_ATTR_RO(uvcg_nv12_, cname, aname);

#define UVCG_NV12_ATTR(cname, aname, bits)			\
static ssize_t uvcg_nv12_##cname##_show(			\
	struct config_item *item, char *page)				\
{									\
	struct uvcg_nv12 *u = to_uvcg_nv12(item);	\
	...
	return result;							\
									...
									\
static ssize_t								\
uvcg_nv12_##cname##_store(struct config_item *item,		\
				    const char *page, size_t len)	\
{									\
	struct uvcg_nv12 *u = to_uvcg_nv12(item);	\
	struct f_uvc_opts *opts;					\
	...
	return ret;							\
									...
									\
UVC_ATTR(uvcg_nv12_, cname, aname);

UVCG_NV12_ATTR_RO(b_format_index, bFormatIndex, 8);
UVCG_NV12_ATTR(b_bits_per_pixel, bBitsPerPixel, 8);
UVCG_NV12_ATTR(b_default_frame_index, bDefaultFrameIndex, 8);
UVCG_NV12_ATTR_RO(b_aspect_ratio_x, bAspectRatioX, 8);
UVCG_NV12_ATTR_RO(b_aspect_ratio_y, bAspectRatioY, 8);
UVCG_NV12_ATTR_RO(bm_interface_flags, bmInterfaceFlags, 8);

#undef UVCG_NV12_ATTR
#undef UVCG_NV12_ATTR_RO

static inline ssize_t
uvcg_nv12_bma_controls_show(struct config_item *item, char *page)
{
	struct uvcg_nv12 *unc = to_uvcg_nv12(item);
	return uvcg_format_bma_controls_show(&unc->fmt, page);
}

static inline ssize_t
uvcg_nv12_bma_controls_store(struct config_item *item,
				     const char *page, size_t len)
{
	struct uvcg_nv12 *unc = to_uvcg_nv12(item);
	return uvcg_format_bma_controls_store(&unc->fmt, page, len);
...
UVC_ATTR(uvcg_nv12_, bma_controls, bmaControls);

static struct configfs_attribute *uvcg_nv12_attrs[] = {
	&uvcg_nv12_attr_b_format_index,
	&uvcg_nv12_attr_guid_format,
	&uvcg_nv12_attr_b_bits_per_pixel,
	&uvcg_nv12_attr_b_default_frame_index,
	&uvcg_nv12_attr_b_aspect_ratio_x,
	&uvcg_nv12_attr_b_aspect_ratio_y,
	&uvcg_nv12_attr_bm_interface_flags,
	&uvcg_nv12_attr_bma_controls,
	NULL,
};

static const struct config_item_type uvcg_nv12_type = {
	.ct_item_ops	= &uvcg_config_item_ops,
	.ct_group_ops	= &uvcg_nv12_group_ops,
	.ct_attrs	= uvcg_nv12_attrs,
	.ct_owner	= THIS_MODULE,
};

static struct config_group *uvcg_nv12_make(struct config_group *group,
						   const char *name)
{
	static char guid[] = {
...
static struct configfs_group_operations uvcg_nv12_grp_ops = {
	.make_group		= uvcg_nv12_make,
};

static const struct uvcg_config_group_type uvcg_nv12_grp_type = {
	.type = {
		.ct_item_ops	= &uvcg_config_item_ops,
		.ct_group_ops	= &uvcg_nv12_grp_ops,
		.ct_owner	= THIS_MODULE,
	},
	.name = "nv12",
};
			   ...
			   enum uvcg_strm_type type)
{
	...
	case UVCG_FORMAT: {
		...
		if (fmt->type == UVCG_UNCOMPRESSED) {
			...
			*size += sizeof(h->desc);
			...
			struct uvcg_nv12 *h =
				container_of(fmt, struct uvcg_nv12, fmt);
	...
	case UVCG_FRAME: {
		...
		if (frm->fmt_type != UVCG_H264)
			*size += (sizeof(frm->frame)-sizeof(frm->frame.dw_bytes_perline));
		else
			*size += (sizeof(frm->frame)-sizeof(frm->frame.dw_max_video_frame_buffer_size));
			    ...
			    enum uvcg_strm_type type)
{
	...
	**array = *dest;
	...
	case UVCG_FRAME: {
		...
		if (frm->fmt_type != UVCG_H264) {
			sz = (sizeof(frm->frame) - sizeof(frm->frame.dw_bytes_perline));
			...
			*dest += sz;
			if (frm->fmt_type == UVCG_UNCOMPRESSED)
				...
				h->bLength = UVC_DT_FRAME_MJPEG_SIZE(
					frm->frame.b_frame_interval_type);
			else if (frm->fmt_type == UVCG_NV12)
				h->bLength = UVC_DT_FRAME_UNCOMPRESSED_SIZE(
					frm->frame.b_frame_interval_type);
		} else {
			sz = (size_t)((void *)&frm->frame.dw_max_video_frame_buffer_size - (void *)&frm->frame.b_length);
			memcpy(*dest, &frm->frame, sz);
			*dest += sz;
			sz = sizeof(frm->frame.dw_default_frame_interval);
			memcpy(*dest, &frm->frame.dw_default_frame_interval, sz);
			*dest += sz;
			sz = sizeof(frm->frame.b_frame_interval_type);
			memcpy(*dest, &frm->frame.b_frame_interval_type, sz);
			*dest += sz;
			sz = sizeof(frm->frame.dw_bytes_perline);
			memcpy(*dest, &frm->frame.dw_bytes_perline, sz);
			*dest += sz;
			sz = frm->frame.b_frame_interval_type *
				sizeof(*frm->dw_frame_interval);
			memcpy(*dest, frm->dw_frame_interval, sz);
			*dest += sz;
			h->bLength = UVC_DT_FRAME_FRAME_BASED_SIZE(frm->frame.b_frame_interval_type);
		}
	}
	break;
	}
...
static ssize_t uvcg_default_streaming_strings_show(
	struct config_item *item, char *page)
{
	struct config_group *group = to_config_group(item);
	struct mutex *su_mutex = &group->cg_subsys->su_mutex;
...
static ssize_t uvcg_default_streaming_strings_store(
	struct config_item *item, const char *page, size_t len)
{
	struct config_group *group = to_config_group(item);
	struct mutex *su_mutex = &group->cg_subsys->su_mutex;
	...
	if (len > sizeof(opts->control_interface_strings))
		store_len = sizeof(opts->control_interface_strings);
	snprintf(opts->streaming_interface_strings, store_len, "%s", page);
	result = len;
	mutex_unlock(&opts->lock);
...
UVCG_OPTS_ATTR(streaming_bulkep, streaming_bulkep, 1);
...
static struct configfs_attribute *uvc_attrs[] = {
	...
	&f_uvc_opts_attr_streaming_bulkep,
	NULL,
};
