...
static void LCD_panel_init(unsigned int sel)
{
	...
	sunxi_lcd_cmd_write(
		sel, 0xe6); /* Fixed SPI 50MHz and 60MHz LCD display problems */
	...
	if (info.lcd_if == LCD_FB_IF_DBI && info.dbi_if == LCD_FB_D2LI) {
		/* two data lane */
	...
	/* 55----RGB565;66---RGB666 */
	if (info.lcd_pixel_fmt == LCDFB_FORMAT_RGB_565 ||
	    info.lcd_pixel_fmt == LCDFB_FORMAT_BGR_565) {
		sunxi_lcd_para_write(sel, 0x55);
		sunxi_lcd_cmd_write(
			sel,
			0x36); /* Memory sunxi_lcd_para_write Access Control */
		if (info.lcd_pixel_fmt == LCDFB_FORMAT_RGB_565)
			sunxi_lcd_para_write(sel, rotate | 0x08);
		else
			sunxi_lcd_para_write(sel, rotate & 0xf7);
	} else if (info.lcd_pixel_fmt < LCDFB_FORMAT_RGB_888) {
		sunxi_lcd_para_write(sel, 0x77);
		if (info.lcd_pixel_fmt == LCDFB_FORMAT_RGBA_8888 ||
		    info.lcd_pixel_fmt == LCDFB_FORMAT_RGBX_8888 ||
		    info.lcd_pixel_fmt == LCDFB_FORMAT_ARGB_8888 ||
		    info.lcd_pixel_fmt == LCDFB_FORMAT_XRGB_8888) {
			sunxi_lcd_cmd_write(
				sel,
				0x36); /* Memory sunxi_lcd_para_write Access Control */
			sunxi_lcd_para_write(sel, rotate | 0x08);
...
static s32 LCD_open_flow(u32 sel)
{
	...
	LCD_OPEN_FUNC(sel, sunxi_lcd_fb_black_screen, 20);
